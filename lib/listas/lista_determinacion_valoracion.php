...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (af.CodValJurDet LIKE '%".$fBuscar."%' OR
...
$_width = 950;
?>
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lista_determinacion_valoracion" method="post" autocomplete="off">
		...
		<td>
			...
			if ($ventana == 'prorrogas') {
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <?=loadSelectGeneral("ESTADO-DETERMINACION-VALORACION", $fEstado, 1)?>
	            </select>
				<?php
			} else {
				?>
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <option value="">&nbsp;</option>
	                <?=loadSelectGeneral("ESTADO-DETERMINACION-VALORACION", $fEstado, 0)?>
	            </select>
				<?php
			}
			?>
...
<table class="tblLista" style="width:100%; min-width:2500px;">
	...
	<?php
	//	consulto todos
	$sql = "SELECT af.CodValJurDet
			FROM
				pf_determinacionvaloracion af
	...
	foreach($field as $f) {
		...
		if ($ventana == "prorrogas") {
			?><tr class="trListaBody" onClick="getActividadesProrroga(['<?=$f['CodValJurDet']?>','<?=$f['CodOrganismo']?>'], ['<?=$campo1?>','<?=$campo2?>'], '<?=$f['CodValJurDet']?>');"><?php
		}
		else {
			?><tr class="trListaBody" onClick="selLista(['<?=$f['CodValJurDet']?>'], ['<?=$campo1?>']);"><?php
		}
		?>
			<td align="center"><?=$f['CodValJurDet']?></td>
			<td><?=$Ente?></td>
			...
			<td align="center"><?=printValoresGeneral("ESTADO-DETERMINACION-VALORACION", $f['Estado'])?></td>
...
<script type="text/javascript" language="javascript">
	function getActividadesProrroga(valores, inputs, CodValJurDet) {
		//	ajax
		$.ajax({
			type: "POST",
			url: "../../pf/lib/fphp_funciones_ajax.php",
			data: "accion=getActividadesProrroga&CodValJurDet="+CodValJurDet,
			async: false,
