...
if ($fEstado != "") { 
	$cEstado = "checked";
	if ($fEstado == 'AP/TE') $filtro.=" AND (af.Estado = 'AP' OR af.Estado = 'TE')";
	else $filtro.=" AND (af.Estado = '".$fEstado."')";
...
$_width = 950;
?>
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lista_actuacion_fiscal" method="post" autocomplete="off">
		...
		<td>
			...
			if ($ventana == 'prorrogas') {
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <?=loadSelectGeneral("ESTADO-ACTUACION-PRORROGAS", $fEstado, 0)?>
	            </select>
				<?php
			} else {
				?>
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <option value="">&nbsp;</option>
	                <?=loadSelectGeneral("ESTADO-ACTUACION", $fEstado, 0)?>
	            </select>
				<?php
			}
			?>
...
<table class="tblLista" style="width:100%; min-width:2500px;">
	...
	foreach($field as $f) {
		...
		if ($ventana == "prorrogas") {
			?><tr class="trListaBody" onClick="getActividadesProrroga(['<?=$f['CodActuacion']?>','<?=$f['CodOrganismo']?>'], ['<?=$campo1?>','<?=$campo2?>'], '<?=$f['CodActuacion']?>');"><?php
		}
		else {
			?><tr class="trListaBody" onClick="selLista(['<?=$f['CodActuacion']?>'], ['<?=$campo1?>']);"><?php
		}
		?>
			<td align="center"><?=$f['CodActuacion']?></td>
			<td><?=$Ente?></td>
			...
			<td align="center"><?=printValoresGeneral("ESTADO-ACTUACION", $f['Estado'])?></td>
...
<script type="text/javascript" language="javascript">
	function getActividadesProrroga(valores, inputs, CodActuacion) {
		//	ajax
		$.ajax({
			type: "POST",
			url: "../../pf/lib/fphp_funciones_ajax.php",
			data: "accion=getActividadesProrroga&CodActuacion="+CodActuacion,
			async: false,
