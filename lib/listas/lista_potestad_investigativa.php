...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (af.CodPotestad LIKE '%".$fBuscar."%' OR
...
$_width = 950;
?>
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lista_potestad_investigativa" method="post" autocomplete="off">
		...
		<td>
			...
			if ($ventana == 'prorrogas') {
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <?=loadSelectGeneral("ESTADO-POTESTAD", $fEstado, 1)?>
	            </select>
				<?php
			} else {
				?>
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <option value="">&nbsp;</option>
	                <?=loadSelectGeneral("ESTADO-POTESTAD", $fEstado, 0)?>
	            </select>
				<?php
			}
			?>
...
<table class="tblLista" style="width:100%; min-width:2500px;">
	...
	foreach($field as $f) {
		...
		if ($ventana == "prorrogas") {
			?><tr class="trListaBody" onClick="getActividadesProrroga(['<?=$f['CodPotestad']?>','<?=$f['CodOrganismo']?>'], ['<?=$campo1?>','<?=$campo2?>'], '<?=$f['CodPotestad']?>');"><?php
		}
		else {
			?><tr class="trListaBody" onClick="selLista(['<?=$f['CodPotestad']?>'], ['<?=$campo1?>']);"><?php
		}
		?>
			<td align="center"><?=$f['CodPotestad']?></td>
			<td><?=$Ente?></td>
			...
			<td align="center"><?=printValoresGeneral("ESTADO-POTESTAD", $f['Estado'])?></td>
...
<script type="text/javascript" language="javascript">
	function getActividadesProrroga(valores, inputs, CodPotestad) {
		//	ajax
		$.ajax({
			type: "POST",
			url: "../../pf/lib/fphp_funciones_ajax.php",
			data: "accion=getActividadesProrroga&CodPotestad="+CodPotestad,
			async: false,
