...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (f.CodFamilia LIKE '%".$fBuscar."%' OR
					  f.CodLinea LIKE '%".$fBuscar."%' OR
					  f.Descripcion LIKE '%".$fBuscar."%')";
} else $dBuscar = "disabled";
if ($fEstado != "") { $cEstado = "checked"; $filtro.=" AND (f.Estado = '".$fEstado."')"; } else $dEstado = "disabled";
if ($fCodLinea != "") { $cCodLinea = "checked"; $filtro.=" AND (f.CodLinea = '".$fCodLinea."')"; } else $dCodLinea = "visibility:hidden;";
//	------------------------------------
?>
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lista_at_familia" method="post">
...
<table class="tblFiltro" style="width:100%; min-width:<?=$_width?>px;">
	<tr>
		...
		<td>
			<input type="checkbox" <?=$cCodLinea?> onclick="ckLista(this.checked, ['fCodLinea'], ['btCodLinea']);" />
        	<input type="text" name="fCodLinea" id="fCodLinea" value="<?=$fCodLinea?>" style="width:50px; font-weight:bold;" readonly="readonly" />
            <a href="javascript:" onclick="window.open('gehen.php?anz=lista_at_linea&filtrar=default&campo1=fCodLinea&ventana=selListaOpener','lista_at_linea','width=950, height=430, toolbar=no, menubar=no, location=no, scrollbars=yes, left=0, top=0, resizable=no')" style=" <?=$dCodLinea?>" id="btCodLinea">
...
<center>
<div style="overflow:scroll; height:200px; width:100%; min-width:<?=$_width?>px;">
<table class="tblLista" style="width:100%; min-width:800px;">
	<thead>
    <tr>
        <th scope="col" width="60" onclick="order('CodFamilia')">C&oacute;digo</th>
        <th scope="col" align="left" onclick="order('Descripcion')">Descripci&oacute;n</th>
        <th scope="col" width="60" onclick="order('CodLinea,CodFamilia')">Linea</th>
	...
	foreach($field as $f) {
		?>
        <tr class="trListaBody" onClick="<?=$ventana?>(['<?=$f['CodLinea']?>','<?=$f['CodFamilia']?>','<?=$f['Linea']?>','<?=$f['Descripcion']?>'], ['<?=$campo1?>','<?=$campo2?>','<?=$campo3?>','<?=$campo4?>']);">
