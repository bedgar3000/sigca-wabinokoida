...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (af.CodDeterminacion LIKE '%".$fBuscar."%' OR
...
$_width = 950;
?>
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lista_determinacion_responsabilidad" method="post" autocomplete="off">
		...
		<td>
			...
			if ($ventana == 'prorrogas') {
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <?=loadSelectGeneral("ESTADO-DETERMINACION", $fEstado, 1)?>
	            </select>
				<?php
			} else {
				?>
	            ...
	            <select name="fEstado" id="fEstado" style="width:143px;" <?=$dEstado?>>
	                <option value="">&nbsp;</option>
	                <?=loadSelectGeneral("ESTADO-DETERMINACION", $fEstado, 0)?>
	            </select>
				<?php
			}
			?>
...
<table class="tblLista" style="width:100%; min-width:2500px;">
	...
	<?php
	//	consulto todos
	$sql = "SELECT af.CodDeterminacion
			FROM
				pf_determinacion af
	...
	foreach($field as $f) {
		$id = $f['CodDeterminacion'];
		$Ente = "<strong>".htmlentities($f['NomOrganismoExterno'])."</strong>"."<br />".htmlentities($f['NomDependenciaExterna']);
		if ($ventana == "prorrogas") {
			?><tr class="trListaBody" onClick="getActividadesProrroga(['<?=$f['CodDeterminacion']?>','<?=$f['CodOrganismo']?>'], ['<?=$campo1?>','<?=$campo2?>'], '<?=$f['CodDeterminacion']?>');"><?php
		}
		else {
			?><tr class="trListaBody" onClick="selLista(['<?=$f['CodDeterminacion']?>'], ['<?=$campo1?>']);"><?php
		}
		?>
			<td align="center"><?=$f['CodDeterminacion']?></td>
			<td><?=$Ente?></td>
			...
			<td align="center"><?=printValoresGeneral("ESTADO-DETERMINACION", $f['Estado'])?></td>
...
<script type="text/javascript" language="javascript">
	function getActividadesProrroga(valores, inputs, CodDeterminacion) {
		//	ajax
		$.ajax({
			type: "POST",
			url: "../../pf/lib/fphp_funciones_ajax.php",
			data: "accion=getActividadesProrroga&CodDeterminacion="+CodDeterminacion,
			async: false,
