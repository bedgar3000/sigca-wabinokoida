...
if ($EsEmpleado == "S") {
	if ($filtro_flag == "") $filtro_flag .= " AND (mp.EsEmpleado = 'S' ";
	else $filtro_flag .= " OR mp.EsEmpleado = 'S' ";
	$filtro_estado = " AND me.Estado = 'A'";
}
if ($EsOtros == "S") {
...
<thead>
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($opcion == "insertarLineaListado") {
			?><tr class="trListaBody" onclick="<?=$opcion?>('<?=$accion?>', '<?=$detalle?>', '<?=$php?>', '<?=$field['CodPersona']?>');"><?php
		}
		elseif ($opcion == "selListadoOC") {
			?><tr class="trListaBody" onclick="<?=$opcion?>('<?=$field['CodPersona']?>', '<?=($field["NomCompleto"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$field["CodTipoServicio"]?>', '<?=($field["NomTipoServicio"])?>', '<?=$field["FactorPorcentaje"]?>', '<?=$field["CodFormaPago"]?>');"><?php
		}
		elseif ($opcion == "selListadoOS") {
			?><tr class="trListaBody" onclick="<?=$opcion?>('<?=$field['CodPersona']?>', '<?=($field["NomCompleto"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$field["CodTipoServicio"]?>', '<?=($field["NomTipoServicio"])?>', '<?=$field["FactorPorcentaje"]?>', '<?=$field["CodFormaPago"]?>', '<?=$field["CodTipoPago"]?>', '<?=$field["DiasPago"]?>', '<?=$fhasta?>');"><?php
		}
		else {
			?><tr class="trListaBody" onclick="selListado('<?=$field['CodPersona']?>', '<?=($field["NomCompleto"])?>', '<?=$cod?>', '<?=$nom?>');"><?php
		}
		?>
