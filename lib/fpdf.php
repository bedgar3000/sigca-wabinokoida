...
class FPDF
{
    ...
    var $CurPageFormat;
     //current page format
    var $PageSizes;
     //array storing non-default page sizes
    var $wPt, $hPt;
    ...
    function Row($data) {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            //Draw the border
            if (!$this->fill) $this->Rect($x, $y, $w, $h, "DF");
            else $this->Rect($x, $y, $w, $h, "D");
    ...
    function FPDF($orientation = 'P', $unit = 'mm', $format = 'A4') {
        ...
        //Page format
        $this->PageFormats = array('a3' => array(841.89, 1190.55), 'a4' => array(595.28, 841.89), 'a5' => array(420.94, 595.28), 'letter' => array(612, 792), 'legal' => array(612, 1008));
    ...
    function SetTitle($title, $isUTF8 = false) {
        ...
        //Title of document
        if ($isUTF8) $title = $this->_UTF8toUTF16($title);
        $this->title = $title;
    }
    ...
    function SetSubject($subject, $isUTF8 = false) {
        ...
        //Subject of document
        if ($isUTF8) $subject = $this->_UTF8toUTF16($subject);
    ...
    function SetAuthor($author, $isUTF8 = false) {
        ...
        //Author of document
        if ($isUTF8) $author = $this->_UTF8toUTF16($author);
        $this->author = $author;
    }
    ...
    function SetKeywords($keywords, $isUTF8 = false) {
        ...
        if ($isUTF8) $keywords = $this->_UTF8toUTF16($keywords);
    ...
    function SetCreator($creator, $isUTF8 = false) {
        ...
        //Creator of document
        if ($isUTF8) $creator = $this->_UTF8toUTF16($creator);
    ...
    function _dochecks() {
        ...
        if (sprintf('%.1F', 1.0) != '1.0') $this->Error('This version of PHP is not supported');
    ...
    function _getpageformat($format) {
        ...
        if (!isset($this->PageFormats[$format])) $this->Error('Unknown page format: ' . $format);
    ...
    function _beginpage($orientation, $format) {
        ...
        if ($format == '') $format = $this->DefPageFormat;
        else {
            if (is_string($format)) $format = $this->_getpageformat($format);
        }
        ...
        if ($orientation != $this->DefOrientation || $format[0] != $this->DefPageFormat[0] || $format[1] != $this->DefPageFormat[1]) $this->PageSizes[$this->page] = array($this->wPt, $this->hPt);
    }
    ...
    function _parsegif($file) {
        ...
        imageinterlace($im, 0);
        $tmp = tempnam('.', 'gif');
        if (!$tmp) $this->Error('Unable to create a temporary file');
        if (!imagepng($im, $tmp)) $this->Error('Error while saving to temporary file');
        imagedestroy($im);
