...
function Cabecera($pdf, $ftiponom, $nomina, $proceso, $periodo, $periodo_fecha, $periodoant, $codempleado, $nomempleado) {
	...
	$pdf->SetFont('Arial', 'B', 6);
	$pdf->Cell(190, 10, ($codempleado.'  '.$nomempleado), 0, 1, 'L');
...
for ($i=0; $i<$rows; $i++) {
	...
	if ($i == 0) {
		$emp = 1;
		Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant, $field['Ndocumento'], $field['NomCompleto']);
	...
	elseif ($grupo != $field['CodEmpleado']) {
		...
		if ($emp == 3) {
			$emp = 1;
			Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant, $field['Ndocumento'], $field['NomCompleto']);
		} else Cabecera2($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant, $field['Ndocumento'], $field['NomCompleto']);
	}
	elseif ($linea > 275) {
		Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant, $field['Ndocumento'], $field['NomCompleto']);
	}
	...
	if ($diferencia != 0) {
		$pdf->Row(array($field['NomConcepto'], number_format($field['MontoAnterior'], 2, ',', '.'), number_format($field['MontoActual'], 2, ',', '.'), number_format($diferencia, 2, ',', '.')));
	}
