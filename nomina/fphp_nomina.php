...
function consultarSueldoNormal($anio, $empleado) {
	$sql = "SELECT Periodo, SueldoNormal FROM rh_sueldos WHERE Periodo LIKE '".$anio."-%' AND CodPersona = '".$empleado."' ORDER BY Periodo";
	$query = mysql_query($sql) or die ($sql.mysql_error());
	$sueldos = array();
	$sw = true;
	while ($field = mysql_fetch_array($query)) {
		if ($field['Orden'] == $OrdenAnt || $field['Orden'] == "1") $sueldos[] = $field;
		$OrdenAnt = $field['Orden'];
	}
	return $sueldos;
}
...
function consultarConceptoBonificacionAnual($anio, $empleado) {
	$sql = "SELECT
			...
			WHERE
				...
				c.Tipo = 'I' AND
				c.FlagBono = 'S' AND
				SUBSTRING(tne.FechaPago, 1, 4) = '".$anio."' AND
				tne.EstadoPago = 'PA'
