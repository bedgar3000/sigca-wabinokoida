...
##	proceso
if ($fCodTipoNom && $fCodTipoProceso) {
	$sql = "SELECT 
				pp.*,
				tn.Nomina,
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Cell(205, 5, strtoupper(utf8_decode('CORRESPONDIENTE A LA LEY DE REGIMEN PRESTACIONAL DE VIVIENDA Y HABITAT')), 0, 1, 'L');
		if ($fCodTipoNom && $fCodTipoProceso) {
			...
			$this->Cell(205, 5, strtoupper(utf8_decode('PERIODO '.$fPeriodo)), 0, 1, 'L');
		}
		##	-------------------
		...
		##
		$this->SetFont('Arial','B',8);
		$this->SetWidths(array(20,110,25,25,25));
