...
<input type="hidden" name="columna" id="columna" value="<?=$columna?>" />
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if (mysql_num_rows($query_sub) == 0 && strlen($field['CodCuenta']) >= 3) {
			if ($destino == "selCuentaContablePerfil") {
				?>
				<tr class="trListaBody" onclick="mClk(this, 'registro'); selCuentaContablePerfil('<?=$field['CodCuenta']?>', '<?=$columna?>');" id="<?=$field['CodCuenta']?>">
			...
			elseif ($destino == "listadoCuentaContable") {
				...
				?>
				<tr class="trListaBody" onclick="mClk(this, 'registro'); selListado('<?=($field["Descripcion"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['CodCuenta']?>">
					<td><?=$field['CodCuenta']?></td>
					<td><?=($field['Descripcion'])?></td>
					<td align="center"><?=($field['NomTipoCuenta'])?></td>
					<td align="center"><?=($naturaleza)?></td>
