...
elseif ($_POST['modulo'] == "EJECUCION-PROCESOS") {
	...
	if ($_POST['accion'] == "PROCESAR-NOMINA") {
		...
		list($_ANIO, $_MES) = split("-", $_PERIODO);
		$_NOMINA = $tiponom;
		...
		foreach ($empleado as $codpersona) {
			...
			while ($field_concepto = mysql_fetch_array($query_concepto)) {	//echo "$field_concepto[CodConcepto] \n\n";
				...
				if (trim($field_concepto['Formula']) == "") {
					...
					$_SUELDO_NORMAL_DIARIO = $_SUELDO_NORMAL / 30; $_SUELDO_NORMAL_DIARIO = round($_SUELDO_NORMAL_DIARIO, 2);
