...
$sql = "SELECT * FROM mastorganismos WHERE CodOrganismo = '$fCodOrganismo'";
...
$emp = 0;
if ($PreNomina == "S") {
	$pr_tiponominaempleado = "pr_tiponominaempleadoprenomina";
	$pr_procesoperiodo = "pr_procesoperiodoprenomina";
	$pr_tiponominaempleadoconcepto = "pr_tiponominaempleadoconceptoprenomina";
}
else {
	$pr_tiponominaempleado = "pr_tiponominaempleado";
	$pr_procesoperiodo = "pr_procesoperiodo";
	$pr_tiponominaempleadoconcepto = "pr_tiponominaempleadoconcepto";
}

//	Obtengo los empleados seleccionados
if ($empleados != "") $dato = explode("|", $empleados);
else {
	$i = 0;
	$dato = "";
	$sql = "SELECT tne.CodPersona 
			FROM 
	...
	while($field_empleados = mysql_fetch_array($query_empleados)) {
		$dato[$i] = $field_empleados['CodPersona'];
		++$i;
	}
}

//	Imprimo los datos
foreach ($dato as $CodPersona) {
	...
	if (mysql_num_rows($query_valido) != 0) {
		...
		$sql = "SELECT 
						...
						CONCAT(ss.CodClaSectorial, pr.CodPrograma, a.CodActividad) AS CodUnidad
				FROM 
						mastempleado me 
		...
		if ($field_resto['CodBeneficiario'] != "") {
			...
			if ($fCodTipoNom == "04") $cargo = ("Pensión por Inválidez");
			elseif ($fCodTipoNom == "03") $cargo = "Jubilado";
		...
		$pdf->Cell(148, 5, substr(utf8_decode($field_resto['CodUnidad'].'-'.$field_resto['UnidadEjecutora']), 0, 50), 0, 0, 'L');
		...
		$sql = "SELECT 
				...
				WHERE 
						...
						tnec.CodTipoProceso = '".$fCodTipoProceso."' 
		...
		while ($field_conceptos = mysql_fetch_array($query_conceptos)) {
			$concepto = substr($field_conceptos['NomConcepto'], 0, 25);
			$monto = number_format($field_conceptos['Monto'], 2, ',', '.');
			$cantidad = number_format($field_conceptos['Cantidad'], 2, ',', '.'); //if ($cantidad == 0) $cantidad = "";
			
			$pdf->SetDrawColor(255, 255, 255); $pdf->SetFillColor(255, 255, 255); $pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont('Courier', 'B', 10);
			$pdf->SetWidths(array(60, 15, 27, 1));
			...
			elseif ($field_conceptos['Tipo'] == "D") {
				$pdf->SetXY(107, $yd); $pdf->Row(array(utf8_decode($concepto), $cantidad, $monto, ''));
				$yd += 5;
				$linead++;
			}
		}
