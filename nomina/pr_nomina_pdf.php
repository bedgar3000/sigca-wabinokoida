...
$pdf->Cell(100, 10, utf8_decode($NomProceso), 0, 1, 'R');


//	Imprimo el tiupo de nomina
$sql = "SELECT * FROM tiponomina WHERE CodTipoNom = '".$fCodTipoNom."'";
...
foreach ($dato as $CodPersona) {
	...
	if (mysql_num_rows($query_valido) != 0) {
		...
		while ($field_conceptos = mysql_fetch_array($query_conceptos)) {
			...
			$pdf->SetWidths(array(58, 14, 30, 1));
...
$pdf->SetFont('Courier', 'B', 11);
$pdf->Cell(195, 5, 'NRO DE TRABAJADORES: '.$emp, 0, 1, 'L');
