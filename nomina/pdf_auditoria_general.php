...
Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant);
...
while ($field = mysql_fetch_array($query)) {
	...
	$pdf->Row(array($field['Ndocumento'], $field['Busqueda'], number_format($field['TotalIngresosAnterior'], 2, ',', '.'), number_format($field['TotalIngresosActual'], 2, ',', '.'), number_format($field['TotalEgresosAnterior'], 2, ',', '.'), number_format($field['TotalEgresosActual'], 2, ',', '.'), number_format($VariacionIngresos, 2, ',', '.'), number_format($VariacionEgresos, 2, ',', '.')));
	if ($pdf->GetY() > 260) Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $fperiodo, $periodo_fecha, $periodoant);
}
//---------------------------------------------------
$pdf->Output();
?>  
