...
elseif ($accion == "insertarCajaChicaDetalle") {
    ...
    <td align="center">
		...
		$query = mysql_query($sql) or die($sql.mysql_error());
		while ($field = mysql_fetch_array($query)) {
			?><option value="<?=$field['CodTipoServicio']?>"><?=$field['Descripcion']?></option><?php
		}
		?></select>
    </td>
...
elseif ($accion == "setTipoServicioCajaChica") {
	connect();
	?><select name="tiposervicio" id="tiposervicio_<?=$candetalles?>" style="width:100%;" class="cell" onBlur="this.className='cell';" onFocus="this.className='cellFocus';" onchange="limpiarMontosCajaChicaDetalle(<?=$candetalles?>);"><?php		
