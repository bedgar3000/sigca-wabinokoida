...
case "TIPO-CUENTA":
	...
	if ($accion == "GUARDAR") {
		$sql = "SELECT * FROM pv_tipocuenta WHERE cod_tipocuenta = '".$codigo."' OR descp_tipocuenta = '".($descripcion)."'";
	...
	elseif ($accion == "ACTUALIZAR") {
		$sql = "SELECT * FROM pv_tipocuenta WHERE cod_tipocuenta <> '".$codigo."' AND descp_tipocuenta = '".($descripcion)."'";
		$query = mysql_query($sql) or die ($sql.mysql_error());
		if (mysql_num_rows($query) != 0) die("¡REGISTRO YA INGRESADO!");
		else {
			$sql = "UPDATE pv_tipocuenta SET descp_tipocuenta = '".($descripcion)."',
												Estado = '".$status."',
...
case "CLASIFICADOR-PRESUPUESTARIO":
	...
	//	MODIFICAR REGISTRO...
	elseif ($accion == "ACTUALIZAR") {
		$sql = "UPDATE pv_partida SET denominacion = '".($descripcion)."',
										tipo = '".$tipo."',
										Estado = '".$status."',
										CodCuenta = '".$codcuenta."',
										UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."',
