...
<table width="910" class="tblLista">
    ...
    if ($registros!=0) {
        ...
        for ($i=0; $i<$rows; $i++) {
			...
			elseif ($ventana == "autorizacion_empleados") {	
				?>
				<tr class="trListaBody" onclick="mClk(this, 'registro'); selListadoAutorizacion('<?=$field["NomCompleto"]?>', '<?=$field['CodPersona']?>', '<?=$field['CodEmpleado']?>');" id="<?=$field["CodPersona"]?>">
					<td align="center"><?=$field["EsReg"]?></td>
					<td align="center"><?=$field["SitTra"]?></td>
					<td align="center">&nbsp;</td>
					<td align="center"><?=$field["CodEmpleado"]?></td>
