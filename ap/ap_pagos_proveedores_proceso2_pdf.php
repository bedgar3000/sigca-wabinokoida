...
if ($fNroProceso != "") $filtro.=" AND (pa.NroProceso = '".$fNroProceso."')";
//---------------------------------------------------
...
$sql = "SELECT
		...
		FROM
			...
			INNER JOIN ap_obligaciones o ON (
				o.NroProceso = pa.NroProceso
				AND o.ProcesoSecuencia = pa.Secuencia
				AND o.Estado = 'PA'
			)
			INNER JOIN ap_ordenpago op ON (
				op.CodProveedor = o.CodProveedor
				AND op.CodTipoDocumento = o.CodTipoDocumento
				AND op.NroDocumento = o.NroDocumento
				AND op.Estado = 'PA'
			)
			INNER JOIN mastpersonas p ON (p.CodPersona = pa.CodProveedor)
			INNER JOIN masttipopago tp ON (tp.CodTipoPago = pa.CodTipoPago)
		WHERE pa.Estado = 'IM' $filtro
		ORDER BY NroCuenta, NroProceso, Secuencia";
$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
while ($field = mysql_fetch_array($query)) {
	##	imprimo proveedor
	$agrupador = $field['NroProceso'].$field['NroCuenta'].$field['CodProveedor'];
	if ($grupo != $agrupador) {
		$grupo = $agrupador;
		$_sub = 1;
		$i = 0;
		...
		$pdf->Row(array(utf8_decode($field['NomProveedorPagar']),
