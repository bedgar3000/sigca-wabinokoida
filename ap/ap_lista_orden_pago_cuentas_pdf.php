...
if ($fFechaTransferenciad != "" || $fFechaTransferenciah != "") {
	if ($fFechaTransferenciad != "") $filtro.=" AND (op.FechaTransferencia >= '".formatFechaAMD($fFechaTransferenciad)."')";
	if ($fFechaTransferenciah != "") $filtro.=" AND (op.FechaTransferencia <= '".formatFechaAMD($fFechaTransferenciah)."')";
}
...
if ($fMostrar != "") {
	if ($fMostrar == 'P') $filtro.=" AND (o.FlagPresupuesto = 'S')";
	elseif ($fMostrar == 'F') $filtro.=" AND (o.FlagPresupuesto = 'N')";
}
//---------------------------------------------------
	...
	if ($verDistribucion == "S") {
		$sql = "SELECT
				...
				FROM
					...
					INNER JOIN ac_mastplancuenta pc ON (pc.CodCuenta = opd.CodCuenta)
				WHERE
		...
		while ($field_distribucion = mysql_fetch_array($query_distribucion)) {
			...
			$pdf->Row(array('',
							...
							utf8_decode($field_distribucion['Descripcion']),
...
$pdf->Cell(100, 5, 'Cantidad:        ' . number_format($contador, 2, ',', '.'), 0, 0, 'L');
$pdf->Cell(100, 5, 'Total:        ' . number_format($MontoTotal, 2, ',', '.'), 0, 0, 'R');
//---------------------------------------------------
