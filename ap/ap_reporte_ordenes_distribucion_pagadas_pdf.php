		...
		FROM
			(SELECT
			 ...
			 FROM
				...
				INNER JOIN ap_ordenpago op ON (op.CodOrganismo = do.CodOrganismo AND
											   op.Anio = do.Anio AND
											   op.NroOrden = do.NroOrden)
	...
	if ($fFlagMostrar == "S") {
		$sql = "(SELECT
				...
				(SELECT
					...
					do.NroDocumento,
					do.Monto,
					o.FechaTransaccion AS FechaOrdenPago,
					p.NomCompleto AS NomProveedor,
					'TB' AS Origen
				 FROM
					ap_ordenpagodistribucion do
		...
		foreach($field_obligaciones as $fo) {
			...
			$pdf->Cell(30, 5, formatFechaDMA($fo['FechaOrdenPago']), 0, 0, 'C');
