...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(10, 20); $this->Cell(190, 5, utf8_decode('LIBRO BANCOS'), 0, 1, 'C', 0);
		$this->SetXY(10, 26); $this->Cell(190, 5, 'DEL '.$fFechaTransacciond.' AL '.$fFechaTransaccionh, 0, 1, 'C', 0);
		##
...
$sql = "SELECT
			...
			(SELECT SUM(bt2.Monto)
			 ...
			 WHERE
				...
				bt2.CodOrganismo = bt.CodOrganismo AND
				bt2.NroCuenta = cb.NroCuenta AND
				(bt2.Estado = 'AP' OR bt2.Estado = 'CO')) AS SaldoCuenta
		FROM
			ap_ctabancaria cb
...
while ($field = mysql_fetch_array($query)) {	++$i;
	...
	$pdf->Cell(30, 5, utf8_decode('CUENTA. '.$field['NroCuenta']), 0, 0, 'L', 0);
