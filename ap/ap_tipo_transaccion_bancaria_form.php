...
if ($opcion == "nuevo") {
	...
	$visible_flagingresos = "visibility:hidden;";
	$visible_flagotro = "visibility:hidden;";
...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	if ($opcion == "modificar") {
		...
		if ($field['TipoTransaccion'] != "I") $visible_flagingresos = "visibility:hidden;";
...
<table width="800" class="tblForm">
	...
	<tr>
		...
		<td>
			<select id="TipoTransaccion" style="width:100px;" onchange="setFlagIngresos(this.value)" <?=$disabled_ver?>>
            	<option value="">&nbsp;</option>
				<?=loadSelectValores("TIPO-TRANSACCION-BANCARIA", $field['TipoTransaccion'], 0)?>
			</select>
            &nbsp; 
            <span id="flagingresos" style=" <?=$visible_flagingresos?>">
			<input type="checkbox" id="FlagDeposito" <?=chkOpt($field['FlagDeposito'], "S")?> onclick="setFlagTipo($(this))" <?=$disabled_ver?> /> Dep&oacute;sito 
			<input type="checkbox" id="FlagNotaCredito" <?=chkOpt($field['FlagNotaCredito'], "S")?> onclick="setFlagTipo($(this))" <?=$disabled_ver?> /> Nota de Cr&eacute;dito 
			<input type="checkbox" id="FlagOtroIngreso" <?=chkOpt($field['FlagOtroIngreso'], "S")?> onclick="setFlagTipo($(this))" <?=$disabled_ver?> /> Otro 
            </span>
            <span id="flagotro" style=" <?=$visible_flagotro?>">
			<input type="checkbox" id="FlagOtroAjuste" <?=chkOpt($field['FlagOtroAjuste'], "S")?> onclick="setFlagTipo($(this))" <?=$disabled_ver?> /> Otros Ajustes
            </span>
		</td>
	</tr>
    <tr>
