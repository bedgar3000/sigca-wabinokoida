...
if ($fCodFormaPago != "") { $cCodFormaPago = "checked"; $filtro.=" AND (oc.CodFormaPago = '".$fCodFormaPago."')"; } else $dCodFormaPago = "disabled";
//	------------------------------------
?>
...
<table width="1050" class="tblFiltro">
	...
	<tr>
		...
		<td class="gallery clearfix">
            ...
            <input type="text" id="fCodProveedor" name="fCodProveedor" style="width:50px;" class="disabled" value="<?=$fCodProveedor?>" readonly="readonly" />
			<input type="text" id="fNomProveedor" name="fNomProveedor" style="width:235px;" class="disabled" value="<?=$fNomProveedor?>" readonly="readonly" />
            <a href="../lib/listas/listado_personas.php?filtrar=default&cod=fCodProveedor&nom=fNomProveedor&iframe=true&width=100%&height=100%" rel="prettyPhoto[iframe1]" id="btProveedor" style=" <?=$dProveedor?>">
...
<table width="2050" class="tblLista">
    ...
    <tbody>
    <?php
	if ($fCodProveedor != "" && ($fDocumentoClasificacion == "RCD" || $fDocumentoClasificacion == "ROC" || $fDocumentoClasificacion == "SER")) {
		//	CONSULTO LA TABLA
		if ($fDocumentoClasificacion == "RCD") {
			$sql = "SELECT
						...
						d.MontoTotal,
						d.Estado,
						d.TransaccionTipoDocumento,
						t.NroInterno AS TransaccionNroDocumento,
		...
		while ($field_documento = mysql_fetch_array($query_documento)) {
			...
			<tr class="trListaBody" onclick="mClkMulti(this); mostrarDocumentosObligacion();" id="row_<?=$id?>">
				...
				<td align="center"><?=($field_documento['Anio'])?></td>
