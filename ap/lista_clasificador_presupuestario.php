...
<table width="100%" class="tblLista">
	<tr class="trListaHead">
		<th width="125" scope="col"><?=$destino?> Partida</th>
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($field['especifica'] == "00" && $field['subespecifica'] == "00") {
			?>
		...
		elseif ($destino == "selPartidaCuenta2") {
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selPartidaCuenta2('<?=htmlentities($field["denominacion"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$cod2?>', '<?=$nom2?>', '<?=$field['CodCuenta']?>', '<?=htmlentities($field["NomCuenta"])?>');" id="<?=$field['cod_partida']?>">
				<td align="center"><span <?=$style?>><?=$field['cod_partida']?></span></td>
				<td><span <?=$style?>><?=htmlentities($field['denominacion'])?></span></td>
				<td align="center"><span <?=$style?>><?=$status?></span></td>
			</tr>
		...
		elseif ($destino == "selListado2") {
			?>
			<tr class="trListaBody" onclick="selListado2('<?=$cod?>', '<?=$nom?>', '<?=$field['cod_partida']?>', '<?=htmlentities($field["denominacion"])?>', '<?=$cod2?>', '<?=$nom2?>', '<?=$field['CodCuenta']?>', '<?=htmlentities($field["NomCuenta"])?>');" id="<?=$field['cod_partida']?>">
				<td align="center"><span <?=$style?>><?=$field['cod_partida']?></span></td>
				<td><span <?=$style?>><?=htmlentities($field['denominacion'])?></span></td>
				<td align="center"><span <?=$style?>><?=$status?></span></td>
			</tr>
