...
class PDF extends FPDF {
	...
	function Footer() {
		...
		if ($_HOJA == "ORDEN DE PAGO") {
			...
			$this->SetXY(10, 241); $this->MultiCell(50, 5, substr($_PREPARADO['Nivel'].' '.$_PREPARADO['Nombre'], 0, 35), 0, 'L');
			$this->SetXY(60, 241); $this->MultiCell(50, 5, substr($_REVISADO['Nivel'].' '.$_REVISADO['Nombre'], 0, 35), 0, 'L');
			$this->SetXY(106, 241); $this->MultiCell(50, 5, substr($_APROBADO['Nivel'].' '.$_APROBADO['Nombre'], 0, 35), 0, 'L');
			$this->SetXY(156, 241); $this->MultiCell(50, 5, substr($_PAGUESE['Nivel'].' '.$_PAGUESE['Nombre'], 0, 35), 0, 'L');

			$this->SetXY(10, 249); $this->MultiCell(50, 5, substr($_PREPARADO['Cargo'], 0, 35), 0, 'C');
			$this->SetXY(60, 249); $this->MultiCell(50, 5, substr($_REVISADO['Cargo'], 0, 35), 0, 'C'); 
			$this->SetXY(106, 249); $this->MultiCell(50, 5, substr($_APROBADO['Cargo'], 0, 35), 0, 'C'); 
			$this->SetXY(156, 249); $this->MultiCell(50, 5, substr($_PAGUESE['Cargo'], 0, 35), 0, 'C'); 
...
$pdf->SetFont('Arial', '', 9);
$pdf->MultiCell(195, $h, utf8_decode('POR CONCEPTO DE: '.$field_orden['Concepto']), 0, 'J');
$y+=$h*5;
