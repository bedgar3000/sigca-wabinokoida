...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetWidths(array(15, 15, 8, 40, 20, 20, 15, 15, 15, 100));
		$this->SetAligns(array('C', 'C', 'C', 'L', 'C', 'R', 'C', 'C', 'C', 'L'));
...
$sql = "SELECT
		...
		WHERE
			...
			$filtro
		ORDER BY CodOrganismo, NomProveedorPagar, CodProveedor, Banco, CodBanco, NroCuenta";
$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
while ($field = mysql_fetch_array($query)) {	++$i;
	$g = "$field[CodProveedor]";
	##
	if ($grupo != $g) {
		$grupo = $g;
		if ($i > 1) {
			...
			$pdf->Cell(68, 5, 'Total Proveedor. ', 0, 0, 'R', 0);
		...
		$pdf->SetFont('Arial', 'B', 6);
		$pdf->Cell(25, 5, 'Proveedor.', 0, 0, 'L', 0);
		$pdf->Cell(80, 5, utf8_decode($field['NomProveedorPagar']), 0, 1, 'L', 0);
	}
	$TotalCheques++;
