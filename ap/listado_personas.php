...
$filtro_flag = "";
if ($flagcliente == "S") { 
	$filtro_flag .= " AND (mp.EsCliente = 'S' ";
}
if ($flagproveedor == "S") {
	if ($filtro_flag == "") $filtro_flag .= " AND (mp.EsProveedor = 'S' ";
	else $filtro_flag .= " OR mp.EsProveedor = 'S' ";
}
if ($flagempleado == "S") {
	if ($filtro_flag == "") $filtro_flag .= " AND ((mp.EsEmpleado = 'S' AND me.Estado = 'A') ";
	else $filtro_flag .= " OR (mp.EsEmpleado = 'S' AND me.Estado = 'A') ";
}
if ($flagotros == "S") {
	if ($filtro_flag == "") $filtro_flag .= " AND (mp.EsOtros = 'S' ";
	else $filtro_flag .= " OR mp.EsOtros = 'S' ";
}
	...
	if ($registros!=0) {
		...
		for ($i=0; $i<$rows; $i++) {
			...
			if ($ventana == "orden_compras") {
				...
				echo "
				<tr class='trListaBody' onclick='mClk(this, \"registro\"); selListadoOrdenCompra(\"".htmlentities($field['NomCompleto'])."\", \"".$cod."\", \"".$nom."\", \"".$field['CodFormaPago']."\", \"".$field['CodTipoServicio']."\", \"".$field['NomTipoServicio']."\", \"".$field['FactorPorcentaje']."\", \"".$field['CodTipoPago']."\", \"".$field['DiasPago']."\", \"".$fhasta."\");' id='".$field['CodPersona']."'>
					<td align='center'>".$field['CodPersona']."</td>
					<td align='left'>".htmlentities($field['NomCompleto'])."</td>
			...
			elseif ($ventana == "caja_chica") {
				echo "
				<tr class='trListaBody' onclick='mClk(this, \"registro\"); selPersonaCajaChica(\"".($cod)."\", \"".($nom)."\", \"".($cod2)."\", \"".($field['CodPersona'])."\", \"".htmlentities($field['NomCompleto'])."\", \"".$field['Ndocumento']."\");' id='".$field['CodPersona']."'>
					<td align='center'>".$field['CodPersona']."</td>
					<td align='left'>".htmlentities($field['NomCompleto'])."</td>
					<td align='center'><input type='checkbox' $escliente disabled /></td>
					<td align='center'><input type='checkbox' $esproveedor disabled /></td>
				...
				echo "
				<tr class='trListaBody' onclick='mClk(this, \"registro\"); selListado(\"".htmlentities($field['NomCompleto'])."\", \"".$cod."\", \"".$nom."\");' id='".$field['CodPersona']."'>
					<td align='center'>".$field['CodPersona']."</td>
					<td align='left'>".htmlentities($field['NomCompleto'])."</td>
					<td align='center'><input type='checkbox' $escliente disabled /></td>
					<td align='center'><input type='checkbox' $esproveedor disabled /></td>
