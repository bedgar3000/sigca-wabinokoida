...
<table width="800" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN ac_subgrupocentrocosto sgcc ON (mcc.CodSubGrupoCentroCosto = sgcc.CodSubGrupoCentroCosto AND mcc.CodGrupoCentroCosto = sgcc.CodGrupoCentroCosto)
				LEFT JOIN ac_grupocentrocosto gcc ON (mcc.CodGrupoCentroCosto = gcc.CodGrupoCentroCosto)
			WHERE 1 $filtro";
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($ventana == "lista") {
			...
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selListado('<?=htmlentities($field["Abreviatura"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['CodCentroCosto']?>">
				<td align="center"><?=$field['CodCentroCosto']?></td>
				<td><?=htmlentities($field['Descripcion'])?></td>
				<td><?=htmlentities($field['NomGrupo'])?></td>
				<td><?=htmlentities($field['NomSubGrupo'])?></td>
