...
include("lib/fphp.php");
//---------------------------------------------------
$_PATHLOGO = $_PARAMETRO["PATHLOGO"];
//---------------------------------------------------
//	obtengo la informacion general
$codigo = explode('_', $sel_registros);
$nrotransaccion = (isset($codigo[0])?$codigo[0]:'');
$secuencia = (isset($codigo[1])?$codigo[1]:'');
$estado = (isset($codigo[2])?$codigo[2]:'');
//list($nrotransaccion, $secuencia, $estado) = explode("_", $sel_registros);
$sql = "SELECT 
			bt.*,
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$sql = "SELECT a.Denominacion
				...
				WHERE cp.CategoriaProg = '$_PARAMETRO[CATADM]'";
		$query = mysql_query($sql) or die($sql.mysql_error());
		if (mysql_num_rows($query) != 0) $field_unidad = mysql_fetch_array($query);
		$UnidadEjecutora = $field_unidad['Denominacion'];
		...
		$this->SetXY(20, 10); $this->Cell(100, 5, utf8_decode($UnidadEjecutora), 0, 0, 'L');
		$this->SetFont('Arial', '', 8);
