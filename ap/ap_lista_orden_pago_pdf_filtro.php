...
if ($filtrar == "default") {
	...
	$fFechaTransferenciad = "01-$Mes-$Anio";
	$fFechaTransferenciah = "$Dia-$Mes-$Anio";
}
?>
...
<table width="1000" class="tblFiltro">
	...
	<tr>
		...
		<td>
			<input type="checkbox" onclick="chkCampos2(this.checked, ['fCodBanco','fNroCuenta']);" />
			<select name="fCodBanco" id="fCodBanco" style="width:300px;" disabled onChange="getOptionsSelect(this.value, 'cuentas_bancarias', 'fNroCuenta', true)">
            	<option value="">&nbsp;</option>
	...
	<tr>
		...
		<td>
			<input type="checkbox" onclick="chkFiltro_2(this.checked, 'fFechaTransferenciad', 'fFechaTransferenciah');" checked />
			<input type="text" name="fFechaTransferenciad" id="fFechaTransferenciad" value="<?=$fFechaTransferenciad?>" maxlength="10" style="width:60px;" class="datepicker" onkeyup="setFechaDMA(this);" />-
            <input type="text" name="fFechaTransferenciah" id="fFechaTransferenciah" value="<?=$fFechaTransferenciah?>" maxlength="10" style="width:60px;" class="datepicker" onkeyup="setFechaDMA(this);" />
        </td>
		<td>&nbsp;</td>
	...
	<tr>
		...
		<td>
        	<input type="checkbox" onclick="chkFiltro(this.checked, 'fMostrar');" />
			<select name="fMostrar" id="fMostrar" style="width:200px;" disabled>
            	<option value="">&nbsp;</option>
                <?=loadSelectValores("ordenes-pago")?>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td><input type="checkbox" name="FlagMostrarCheque" id="FlagMostrarCheque" value="S" checked="checked" /> Mostrar Nro. Cheque</td>
	</tr>
</table>
</div>
