...
<center>
	<table class="tblBotones" style="width:100%; min-width:<?=$_width?>px;">
		<tr>
			...
			<td align="right">
	        	<?php
				if ($_PARAMETRO['CONTORDENDIS'] == "T") $pdf = "ap_orden_pago_pdf";
				elseif ($_PARAMETRO['CONTORDENDIS'] == "F") $pdf = "ap_orden_pago_pub20_pdf";
				?>
				<input type="button" id="btModificar" class="update" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=orden_pago_modificar', 'gehen.php?anz=ap_orden_pago_form&opcion=modificar', 'SELF', '');" />
	            <input type="button" id="btVer" class="ver" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=ap_orden_pago_form&opcion=ver', 'SELF', '', $('#registro').val());" />
	            <input type="button" id="btAnular" class="update" value="Anular" style="width:75px; <?=$btAnular?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=orden_pago_anular', 'gehen.php?anz=ap_orden_pago_form&opcion=anular', 'SELF', '');" />
	            <input type="button" id="btImprimir" class="ver" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="cargarOpcion2(this.form, '<?=$pdf?>.php?', 'BLANK', 'height=800, width=750, left=200, top=200, resizable=no', $('#registro').val());" />
				<input type="button" id="btImprimirVoucher" class="ver" value="Imprimir Voucher" style="width:100px;" onclick="abrirReporte(document.getElementById('frmentrada'), 'aVoucher', 'ap_orden_pago_voucher_pdf')" /> | 
	            <input type="button" id="btPrepago" class="update" value="Generar Pre-Pago" style="width:125px; <?=$btPrepago?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=ap_orden_pago_prepago&accion=prepago', 'SELF', '', $('#registro').val());" />
			</td>
		</tr>
	</table>
	<div class="scroll" style="overflow:scroll; height:300px; width:100%; min-width:<?=$_width?>px;">
	<table class="tblLista" style="width:100%; min-width:2700px;">
		<thead>
			<th width="25">Sist.</th>
			<th width="85">Nro. Orden</th>
			<th>Pagar A</th>
			...
			if ($_PARAMETRO['CONTORDENDIS'] == "T") {
				?><th width="100">Voucher</th><?php
			}
			elseif ($_PARAMETRO['CONTORDENDIS'] == "F") {
				?><th width="100">Voucher (Pub.20)</th><?php
			}
			?>
			...
			<th width="90">Fecha Anulaci&oacute;n</th>
		...
		$sql = "SELECT
				...
				FROM
					ap_ordenpago op
					INNER JOIN ap_obligaciones o ON (o.CodTipoDocumento = op.CodTipoDocumento AND o.CodProveedor = op.CodProveedor AND o.NroDocumento = op.NroDocumento)
		...
		while ($field = mysql_fetch_array($query)) {
			...
			<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
				...
				<td align="center"><?=formatFechaDMA($field['FechaAnulacion'])?></td>
...
if ($mostrar == "vouchers") {
	...
	$(document).ready(function() {
		var url = "gehen.php?anz=<?=$accion?>&registro=<?=$registro?>&accion=ver&origen=orden-anulacion&iframe=true&width=100%&height=100%";
