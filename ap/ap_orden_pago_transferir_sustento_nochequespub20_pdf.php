...
$sql = "SELECT
			...
			p.NroCuenta,
			p.Periodo,
			p.PeriodoPagoPub20 AS VoucherPeriodo,
...
function chequeVenezuela($pdf) {
	...
	$pdf->Cell(60, 5, utf8_decode($field['Banco']), 0, 0, 'L');
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(16, 5, 'Nro. Pago', 0, 0, 'L');
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(100, 5, $field['NroPago'], 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(27, 5, 'Pagar a la orden de ', 0, 0, 'L');
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(160, 5, utf8_decode($field['NomProveedorPagar']), 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(17, 5, 'La suma de ', 0, 0, 'L');
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(160, 5, utf8_decode($monto_letras), 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(15, 5, 'Proceso ', 0, 0, 'L');
	$pdf->Cell(30, 5, $field['NroProceso'], 0, 0, 'R');
	$pdf->Cell(5, 5, $field['Secuencia'], 0, 0, 'R');
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(15, 5, 'Cta. Cte. ', 0, 0, 'L');
	$pdf->Cell(35, 5, $field['CtaBanco'], 0, 0, 'R');
	$pdf->Ln(10);
}
//---------------------------------------------------
...
$sql = "SELECT
		...
		WHERE
			CodOrganismo = '".$field['CodOrganismo']."' AND
			Periodo = '".$field['VoucherPeriodo']."' AND
			Voucher = '".$field['VoucherPago']."' AND
			CodContabilidad = 'F'
		ORDER BY CodCuenta";
