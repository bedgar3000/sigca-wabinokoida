...
function getGrupoCentroCosto($codigo, $opt) {
	...
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$codigo) echo "<option value='".$field[0]."' selected>".($field[1])."</option>"; 
				else echo "<option value='".$field[0]."'>".($field[1])."</option>";
			}
			break;
	...
	switch ($opt) {
		case 0:
			foreach ($value AS $valor) {
				if ($campo == $valor) { ?> <option value="<?=$valor?>" selected="selected"><?=($text[$i])?></option> <?php }
				else { ?> <option value="<?=$valor?>"><?=($text[$i])?></option> <?php }
				$i++;
			}
			break;
