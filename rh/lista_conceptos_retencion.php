...
$filtro=trim($filtro); 
if ($filtro!="") $filtrado="AND (CodConcepto LIKE '%".$filtro."%' OR Descripcion LIKE '%".$filtro."%') "; else $filtrado="";
//	----------------------------
?>
...
<table width="100%" class="tblLista">
	...
	<?php 
	//	CONSULTO LA TABLA
	$sql="SELECT * FROM pr_concepto WHERE Tipo = 'D' $filtrado ORDER BY CodConcepto ASC";
	...
	for ($i=0; $i<$rows; $i++) {
		...
		//---------------------------------------------------
		echo "
		<tr class='trListaBody' onclick='mClk(this, \"registro\"); selConceptoRetencion(\"".($field['Descripcion'])."\");' id='".$field['CodConcepto']."'>
