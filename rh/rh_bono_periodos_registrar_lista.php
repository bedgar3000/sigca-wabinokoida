...
//	------------------------------------
if (!empty(trim($return))) $click = "cargarPagina(document.getElementById('frmentrada'), '../framemain.php');"; else $click = "cargarPagina(document.getElementById('frmentrada'), 'gehen.php?anz=rh_bono_periodos_lista');";
?>
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Registro de Eventos</td>
		<td align="right"><a class="cerrar" href="javascript:" onclick="<?php echo $click; ?>">[cerrar]</a></td>
	</tr>
...
<table width="1500" class="tblLista">
    ...
    <tr>
        ...
        <th scope="col" width="60">Descanso</th>
    </tr>
    </thead>
    ...
    $sql = "SELECT
				...
				bad.DiasPeriodo,
				bad.DiasPago,
				(bad.DiasPeriodo - bad.DiasFeriados - bad.DiasInactivos - bad.DiasDescuento) AS DiasTrabajados,
				bad.DiasDescuento,
				bad.DiasInactivos,
				bad.DiasFeriados,
				p.NomCompleto,
    ...
    while ($field_empleados = mysql_fetch_array($query_empleados)) {
        ...
        <tr class="trListaBody" onclick="mClk(this, 'dregistro');" id="<?=$id?>">
			...
			<td align="right">
                <?=number_format($field_empleados['DiasFeriados'], 2, ',', '.')?>
			</td>
			<td align="right">
                <?=number_format($field_empleados['DiasInactivos'], 2, ',', '.')?>
			</td>
        </tr>
        <?php
    }
