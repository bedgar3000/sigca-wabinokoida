...
$filtro_perfil = "";
foreach ($_REQUEST['fCodPerfil'] as $fCodPerfil) {
	if ($filtro_perfil == "") $filtro_perfil .= " AND (e.CodPerfil = '".$fCodPerfil."'";
	else $filtro_perfil .= " OR e.CodPerfil = '".$fCodPerfil."'";
}
if ($filtro_perfil != "") { $filtro_perfil .= ")"; $filtro .= $filtro_perfil; }
//---------------------------------------------------
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(10, 20); $this->Cell(190, 5, utf8_decode('Vacaciones Pendientes de Disfrute'), 0, 1, 'C', 0);
		$this->Ln(5);
		##
		$this->SetDrawColor(0, 0, 0);
		$this->SetWidths(array(30, 15, 25, 25, 25, 25));
...
while ($field = mysql_fetch_array($query)) {	++$i;
	...
	##
	$Pendientes = $field['Derecho'] - $field['TotalUtilizados'];
