...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Cell(125, 5, utf8_decode('INTERRUPCIÓN DE VACACIONES'), 0, 0, 'C');
		##
		$this->SetFont('Arial', 'B', 8);
		...
		$this->SetXY(10, 55); 
		$this->Cell(165, 10, utf8_decode('INTERRUPCIÓN DE VACACIONES'), 1, 0, 'C');
...
while ($field_detalles = mysql_fetch_array($query_detalles)) {
	...
	$pdf->Cell(195, 5, utf8_decode('DIAS A INTERRUMPIR'), 1, 1, 'C');
	$pdf->Cell(76, 5, utf8_decode('DESDE:'), 1, 0, 'C');
	$pdf->Cell(76, 5, utf8_decode('HASTA:'), 1, 0, 'C');
	$pdf->Cell(43, 5, utf8_decode('TOTAL DIAS'), 1, 1, 'C');
	$pdf->Cell(25, 5, utf8_decode('DIA'), 1, 0, 'C');
	$pdf->Cell(25, 5, utf8_decode('MES'), 1, 0, 'C');
	$pdf->Cell(26, 5, utf8_decode('AÑO'), 1, 0, 'C');
	$pdf->Cell(25, 5, utf8_decode('DIA'), 1, 0, 'C');
	$pdf->Cell(25, 5, utf8_decode('MES'), 1, 0, 'C');
	$pdf->Cell(26, 5, utf8_decode('AÑO'), 1, 0, 'C');
	$pdf->SetFont('Arial', 'B', 10);
	$pdf->Cell(43, 10, $field_detalles['NroDias'], 1, 1, 'C');
	##
	$pdf->SetY($pdf->GetY()-5);
	$pdf->Cell(25, 5, $DiaDesde, 1, 0, 'C');
	$pdf->Cell(25, 5, $MesDesde, 1, 0, 'C');
	$pdf->Cell(26, 5, $AnioDesde, 1, 0, 'C');
	$pdf->Cell(25, 5, $DiaHasta, 1, 0, 'C');
	$pdf->Cell(25, 5, $MesHasta, 1, 0, 'C');
	$pdf->Cell(26, 5, $AnioHasta, 1, 1, 'C');
	##
	$pdf->SetFont('Arial', 'B', 10);
	...
	$pdf->SetX(108.5);
	$pdf->Row(array(utf8_decode("DIRECTORA DE RECURSOS HUMANOS")));
	##
