...
?>
<form name="frmlista" id="frmlista" method="post" action="lista_participantes.php?">
<input type="text" name="capacitacion" id="capacitacion" value="<?=$capacitacion?>" />
<input type="text" name="organismo" id="organismo" value="<?=$organismo?>" />
...
<table width="700" class="tblLista">
	...
	if ($registros!=0) {
		//	CONSULTO LA TABLA
		if ($filtro!="") $sql="SELECT mastempleado.CodEmpleado, mastempleado.CodPersona, mastempleado.CodDependencia, mastempleado.CodDivision, mastempleado.CodOrganismo, mastpersonas.Busqueda, mastpersonas.EsCliente, mastpersonas.EsProveedor, mastpersonas.EsEmpleado, mastpersonas.EsOtros, mastpersonas.Ndocumento, mastpersonas.DocFiscal FROM mastempleado, mastpersonas WHERE (mastempleado.CodPersona LIKE '%".$filtro."%' OR mastpersonas.Busqueda LIKE '%".$filtro."%' OR mastpersonas.Ndocumento LIKE '%".$filtro."%' OR mastpersonas.DocFiscal LIKE '%".$filtro."%') AND (mastpersonas.CodPersona=mastempleado.CodPersona) AND (mastpersonas.EsEmpleado='S') AND (mastempleado.CodOrganismo='".$organismo."') ORDER BY mastempleado.CodPersona LIMIT ".$limit.", $MAXLIMIT";
		else $sql="SELECT mastempleado.CodEmpleado, mastempleado.CodPersona, mastempleado.CodDependencia, mastempleado.CodDivision, mastempleado.CodOrganismo, mastpersonas.Busqueda, mastpersonas.EsCliente, mastpersonas.EsProveedor, mastpersonas.EsEmpleado, mastpersonas.EsOtros, mastpersonas.Ndocumento, mastpersonas.DocFiscal FROM mastempleado, mastpersonas WHERE (mastpersonas.CodPersona=mastempleado.CodPersona) AND (mastpersonas.EsEmpleado='S') ORDER BY mastempleado.CodPersona LIMIT ".$limit.", $MAXLIMIT";
		$query=mysql_query($sql) or die ($sql.mysql_error());
		$rows=mysql_num_rows($query);	echo $sql;
		//	MUESTRO LA TABLA
		for ($i=0; $i<$rows; $i++) {
			...
			echo "
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); selParticipante(\"".$capacitacion."\", \"".$organismo."\", \"".$field['CodPersona']."\", \"".$field['CodDependencia']."\", \"".$field['CodDivision']."\");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['CodPersona']."'>
