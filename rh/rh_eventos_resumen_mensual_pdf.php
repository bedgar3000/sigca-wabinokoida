...
$TotalHorasMensuales = $field_resumen['HorasDiaria'] * $field_resumen['TotalDiasPago'];
$TotalDiasHabiles = getDiasHabiles($FechaInicio, $FechaFin);
$TotalDiasDescanso = $field_resumen['TotalDiasPago'] - $field_resumen['TotalFeriados'] - $TotalDiasHabiles;
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(10, 20); $this->Cell(190, 5, utf8_decode('RESUMEN MENSUAL'), 0, 1, 'C', 0);
...
$pdf->Row(array($field_resumen['HorasDiaria'],
				...
				$TotalDiasHabiles,
				$TotalDiasDescanso,
				$field_resumen['TotalFeriados'],
				number_format($field_resumen['ValorDia'], 2, ',', '.'),
				number_format($field_resumen['ValorMes'], 2, ',', '.')));
...
while ($field = mysql_fetch_array($query)) {	$sw++;
	...
	while($swSemana) {
		...
		$fechaf[$ns] = $ff;
		//	
		$_dias_semanas = getFechaDias($fi, $ff) + 1;
		$_feriados_semanas = getDiasFeriados($fi, $ff);
		$_dias_semanas_habiles[$ns] = $_dias_semanas - $_feriados_semanas;
		//	
		$fi = obtenerFechaFin($ff, 2);
	}
	//	imprimo las semanas
	$TotalHorasFaltantes = '00:00';
	$TotalHorasTrabajadas = '00:00';
	for($ns=1;$ns<=$nro_semanas;$ns++) {
		...
		for($i=$_DiffDias;$i<($_DiffDias+$TotalDiasSemana);$i++) {
			...
			elseif ($field_total['Dia'] == "I") ++$DiasInactivos;
		}
		$_DiffDias += $TotalDiasSemana;
		$ValorDescuento = $field_resumen['ValorDia'] * $DiasDescuento;
		$TotalPagar = $field_resumen['ValorDia'] * ($DiasPago + $DiasInactivos + $DiasFeriados);
		$_HorasSemanal = $field_resumen['HorasDiaria'] * ($DiasPago + $DiasInactivos + $DiasFeriados);
		...
		$pdf->Row(array($HorasFaltantes,
						...
						$DiasInactivos,
						$DiasDescuento,
						($DiasPago + $DiasInactivos + $DiasFeriados),
						number_format($ValorDescuento, 2, ',', '.'),
						number_format($TotalPagar, 2, ',', '.')));
		##	
		$TotalHorasFaltantes = sumarHoras($HorasFaltantes, $TotalHorasFaltantes);
		$TotalHorasTrabajadas = sumarHoras($HorasTrabajadas, $TotalHorasTrabajadas);
		$TotalDiasFeriados += $DiasFeriados;
		$TotalDiasInactivos += $DiasInactivos;
		$TotalDiasDescuento += $DiasDescuento;
		$TotalDiasPago += ($DiasPago + $DiasInactivos + $DiasFeriados);
	...
	$pdf->Row(array($TotalHorasFaltantes,
					...
					number_format($field['TotalPagar'], 2, ',', '.')));
	$pdf->Ln(5);
