...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	$sql = "SELECT * FROM rh_profesiones WHERE CodProfesion = '".$sel_registros."'";
	$field = getRecord($sql);
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=rh_profesiones_lista" method="POST" enctype="multipart/form-data" onsubmit="return form(this, '<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
	...
	<tr>
		<td class="tagForm">Cod.:</td>
		<td>
        	<input type="text" id="CodProfesion" value="<?=$field['CodProfesion']?>" style="width:75px;" disabled />
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
            <select id="CodGradoInstruccion" style="width:320px;" <?=$disabled_ver?>>
                <option value="">&nbsp;</option>
                <?=loadSelect2("rh_gradoinstruccion", "CodGradoInstruccion", "Descripcion", $field['CodGradoInstruccion'], 0)?>
            </select>
		</td>
	</tr>
	<tr>
		...
		<td>
            <select id="Area" style="width:320px;" <?=$disabled_ver?>>
                <option value="">&nbsp;</option>
...
function form(form, accion) {
	//	valido
	var error = "";
	if ($("#Descripcion").val().trim() == "" || $("#CodGradoInstruccion").val() == "" || $("#Area").val() == "") error = "Debe llenar los campos obligatorios";
	...
	if (error != "") {
		...
		$.ajax({
			...
			data: "modulo=profesiones&accion="+accion+"&"+post,
			async: false,
