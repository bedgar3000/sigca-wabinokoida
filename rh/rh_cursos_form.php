...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	$sql = "SELECT * FROM rh_cursos WHERE CodCurso = '".$sel_registros."'";
	$field = getRecord($sql);
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=rh_cursos_lista" method="POST" enctype="multipart/form-data" onsubmit="return form(this, '<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
	...
	<tr>
		...
		<td>
        	<input type="text" id="CodCurso" value="<?=$field['CodCurso']?>" style="width:50px;" disabled />
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
            <select id="AreaCurso" style="width:320px;" <?=$disabled_ver?>>
                <option value="">&nbsp;</option>
                <?=getMiscelaneos($field['AreaCurso'], "AREACURSO", 0)?>
            </select>
		</td>
	</tr>
	<tr>
...
function form(form, accion) {
	...
	if (error != "") {
		...
		$.ajax({
			type: "POST",
			url: "rh_cursos_ajax.php",
			data: "modulo=cursos&accion="+accion+"&"+post,
			async: false,
