...
if ($opcion == "nuevo") {
	...
	$FechaTermino = getFechaFinHabiles($FechaSalida, $field['NroDias']);
	$FechaIncorporacion = getFechaFinHabiles($FechaSalida, $field['NroDias']+1);
	...
	$visible_interrumpir = "visibility:hidden;";
	$tdSolicitud = "visibility:hidden;";
	$vacaciones_periodos_insertar = "vacaciones_periodos_insertar()";
	$obtenerFechaTerminoVacacion = "obtenerFechaTerminoVacacion()";
}
elseif ($opcion == "modificar" || $opcion == "ver" || $opcion == "revisar" || $opcion == "conformar" || $opcion == "aprobar" || $opcion == "anular") {
	...
	elseif ($opcion == "ver") {
		...
		$visible_modificar = "visibility:hidden;";
		$visible_interrumpir = "visibility:hidden;";
		if ($origen == "blank") $display_ver_blank = "display:none;";
	}
	
	elseif ($opcion == "revisar") {
		$titulo = "Autorizar Solicitud";
		$label_submit = "Autorizar";
		$accion = "revisar";
		$disabled_nuevo = "disabled";
		$disabled_modificar = "disabled";
		$disabled_ver = "disabled";
	...
	if ($field['Tipo'] == "G") {
		...
		$vacaciones_periodos_insertar = "vacaciones_periodos_insertar_interrupcion()";
		$obtenerFechaTerminoVacacion = "obtenerFechaTerminoVacacion()";
	}
	list($AnioRelacionada, $CodSolicitudRelacionada) = split("[-]", $field['SolicitudRelacionada']);
}	
elseif ($opcion == "interrumpir") {
	list($Anio, $CodSolicitud) = split("[.]", $registro);
	$AnioRelacionada = $Anio;
	$CodSolicitudRelacionada = $CodSolicitud;
	//	consulto datos generales
	$sql = "SELECT
				vs.CodPersona,
				vs.FechaSalida,
...
//	------------------------------------
if (!empty(trim($origen))) $action = "gehen.php?anz=$origen"; else $action = "../framemain.php";
?>
<div style=" <?=$display_ver_blank?>">
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo"><?=$titulo?></td>
		<td align="right"><a class="cerrar" href="<?php echo $action ?>">[cerrar]</a></td>
	</tr>
...
<form name="frmentrada" id="frmentrada" action="<?=$action?>" method="POST" onsubmit="return vacaciones(this, '<?=$accion?>');">
...
<div id="tab1" style="display:block;">
	<table width="800" class="tblForm">
	    ...
	    <tr>
			...
			<td class="gallery clearfix" colspan="3">
	            ...
	            <input type="text" id="CodEmpleado" style="width:49px;" value="<?=$field['CodEmpleado']?>" disabled="disabled" />
				<input type="text" id="NomPersona" style="width:235px;" value="<?=$field['NomPersona']?>" disabled="disabled" />
	    ...
	    <tr>
			...
			<td>
	        	<input type="hidden" id="CodSolicitud" value="<?=$field['CodSolicitud']?>" />
	        	<input type="text" id="NroSolicitud" value="<?=$field['NroSolicitud']?>" style="width:60px;" class="codigo" disabled="disabled" />
	        </td>
			<td class="tagForm" width="150">Nro. Otorgamiento:</td>
			<td width="300"><input type="text" id="NroOtorgamiento" value="<?=$field['NroOtorgamiento']?>" style="width:75px;" class="codigo" disabled="disabled" /></td>
		</tr>
	    <tr>
			<td class="tagForm">Documento:</td>
			<td><input type="text" id="Documento" maxlength="50" style="width:100px;" value="<?=$field['Documento']?>" <?=$disabled_conformar?> /></td>
			<td class="tagForm">Estado:</td>
			<td>
	            <input type="hidden" id="Estado" value="<?=$field['Estado']?>" />
	            <input type="text" style="width:75px;" class="codigo" value="<?=strtoupper(printValores("ESTADO-VACACIONES", $field['Estado']))?>" disabled="disabled" />
			</td>
		</tr>
	    <tr>
	        ...
	        <td>
	            <select id="Tipo" style="width:106px;" <?=$disabled_modificar?>>
	                <?=loadSelectValores("TIPO-VACACIONES", $field['Tipo'], 1)?>
	            </select>
	        </td>
	    ...
	    <tr>
	        ...
	        <td>
	            ...
	            <input type="text" id="NomCreadoPor" value="<?=htmlentities($field['NomCreadoPor'])?>" style="width:245px;" class="disabled" disabled="disabled" />
	        </td>
	    </tr>
	    <tr>
			<td class="tagForm">* F.T&eacute;rmino:</td>
			<td><input type="text" id="FechaTermino" value="<?=formatFechaDMA($field['FechaTermino'])?>" maxlength="10" style="width:60px;" class="datepicker" onkeyup="setFechaDMA(this);" disabled="disabled" /></td>
	        <td class="tagForm">Autorizado Por:</td>
	        <td>
	            <input type="hidden" id="RevisadoPor" value="<?=$field['RevisadoPor']?>" />
	            <input type="text" id="NomRevisadoPor" value="<?=htmlentities($field['NomRevisadoPor'])?>" style="width:245px;" class="disabled" disabled="disabled" />
	        </td>
	    </tr>
	    <tr>
			<td class="tagForm">* F.Incorporaci&oacute;n:</td>
			<td><input type="text" id="FechaIncorporacion" value="<?=formatFechaDMA($field['FechaIncorporacion'])?>" maxlength="10" style="width:60px;" class="datepicker" onkeyup="setFechaDMA(this);" disabled="disabled" /></td>
	        <td class="tagForm">Revissdo por:</td>
	        <td>
	            <input type="hidden" id="ConformadoPor" value="<?=$field['ConformadoPor']?>" />
	            <input type="text" id="NomConformadoPor" value="<?=htmlentities($field['NomConformadoPor'])?>" style="width:245px;" class="disabled" disabled="disabled" />
	        </td>
	    </tr>
	    <tr>
	        <td class="tagForm" style=" <?=$tdSolicitud?>">Solicitud Relacionada:</td>
	        <td style=" <?=$tdSolicitud?>" class="gallery clearfix">
	            <input type="hidden" id="AnioRelacionada" value="<?=$AnioRelacionada?>" />
	            <input type="text" id="CodSolicitudRelacionada" value="<?=$CodSolicitudRelacionada?>" style="width:60px;" class="codigo" disabled="disabled" />
	            <a class="link" href="gehen.php?anz=rh_vacaciones_form&opcion=ver&origen=blank&registro=<?=$registro?>&iframe=true&height=525&width=850" rel="prettyPhoto[iframe3]" style=" <?=$visible_interrumpir?>">
	            	Ver Solicitud
	            </a>
	        </td>
	        ...
	        <td>
	            ...
	            <input type="text" id="NomAprobadoPor" value="<?=htmlentities($field['NomAprobadoPor'])?>" style="width:245px;" class="disabled" disabled="disabled" />
	        </td>
	    </tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3"><textarea id="Observaciones" style="width:95%; height:45px;" <?=$disabled_conformar?>><?=htmlentities($field['Observaciones'])?></textarea></td>
		</tr>
		<tr>
	...
	<center>
	<input type="submit" value="<?=$label_submit?>" style="width:65px; <?=$display_submit?>" />
	<input type="button" value="Cancelar" style="width:65px; <?=$display_ver_blank?>" onclick="this.form.submit();" /> | 
	<input type="button" value="Mostrar Periodos" style=" <?=$display_ver_blank?>" onclick="abrir_listado_periodos_vacaciones('mostrar');" />
	</center>
	...
	<table width="1200" class="tblLista">
		...
		while($field_detalles = mysql_fetch_array($query_detalles)) {
	        ...
	        <tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="<?=$field_detalles['NroPeriodo']?>">
	            ...
	            <td>
	               <textarea name="Observaciones" class="cell" style="height:20px;" onfocus="$(this).css('height', '60px');" onblur="$(this).css('height', '20px');"><?=$field_detalles['Observaciones']?></textarea>
