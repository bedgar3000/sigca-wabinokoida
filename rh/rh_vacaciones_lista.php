...
/*elseif ($lista == "revisar") {
	$titulo = "Autorizar Solicitudes de Vacaciones";
	$btNuevo = "display:none;";
	$btModificar = "display:none;";
	$btAprobar = "display:none;";
	$btConformar = "display:none;";
	$btInterrumpir = "display:none;";
	$fEstado = "PE";
}
...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (CONCAT(vs.Anio, '-', vs.NroSolicitud) LIKE '%".$fBuscar."%' OR
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			...
			<input type="button" id="btInterrumpir" value="Interrumpir" style="width:75px; <?=$btInterrumpir?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=vacaciones_interrumpir', 'gehen.php?anz=rh_vacaciones_form&opcion=interrumpir&origen=rh_vacaciones_lista', 'SELF', '');" />
            
			<input type="button" id="btRevisar" value="Autorizar" style="width:75px; <?=$btRevisar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_vacaciones_form&opcion=revisar&origen=rh_vacaciones_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btConformar" value="Revisar" style="width:75px; <?=$btConformar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_vacaciones_form&opcion=conformar&origen=rh_vacaciones_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btAprobar" value="Conformar" style="width:75px; <?=$btAprobar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_vacaciones_form&opcion=aprobar&origen=rh_vacaciones_lista', 'SELF', '', 'registro');" />
			...
			<input type="button" id="btImprimir" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="vacaciones_imprimir();" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	while ($field = mysql_fetch_array($query)) {
		$id = "$field[Anio].$field[CodSolicitud].$field[Tipo]";
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			<td align="center"><?=$field['Anio']?>-<?=$field['NroSolicitud']?></td>
