...
if ($fBuscar != "") {
	$cBuscar = "checked";
	$filtro .= " AND (ea.Evaluacion LIKE '%".$fBuscar."%' OR
...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Grupos de Competencia</td>
		<td align="right"><a class="cerrar" href="../framemain.php">[cerrar]</a></td>
	</tr>
</table><hr width="100%" color="#333333" /><br />

<form name="frmentrada" id="frmentrada" action="gehen.php?anz=rh_competencias_grupo_lista" method="post">
...
<table width="700" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" id="btNuevo" value="Nuevo" style="width:75px;" onclick="cargarPagina(this.form, 'gehen.php?anz=rh_competencias_grupo_form&opcion=nuevo');" />
            
			<input type="button" id="btModificar" value="Modificar" style="width:75px;" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_competencias_grupo_form&opcion=modificar', 'SELF', '', $('#registro').val());" />
            
			<input type="button" id="btVer" value="Ver" style="width:75px;" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_competencias_grupo_form&opcion=ver', 'SELF', '', $('#registro').val());" />
            
			<input type="button" id="btEliminar" value="Eliminar" style="width:75px;" onclick="opcionRegistro2(this.form, $('#registro').val(), 'competencias_grupo', 'eliminar');" />
		</td>
	</tr>
</table>
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($Grupo != $field['TipoEvaluacion']) {
			...
			<tr class="trListaBody2">
				...
				<td><?=htmlentities($field['NomTipoEvaluacion'])?></td>
			</tr>
			<?php
		}
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			<td align="center"><?=$field['Area']?></td>
