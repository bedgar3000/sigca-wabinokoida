...
function Cabecera($pdf) {
	...
	$pdf->SetXY(166, 24); $pdf->SetFont('Arial', 'B', 9); $pdf->Cell(18, 5, utf8_decode('Nº'), 0, 1, 'C');
...
$sql = "SELECT 
			...
			(SELECT DescripCargo 
			 ...
			 WHERE 
			 	CodCargo = (SELECT 
							...
							WHERE 
								CodPersona=rh_permisos.Aprobador AND Secuencia = (SELECT 
																					MAX(Secuencia) 
																				  FROM 
																				  	rh_empleadonivelacion 
																				  WHERE 
																				  	CodPersona=rh_permisos.Aprobador)
						   )
			) AS CargoAprobador, 
			(SELECT CodCargo 
		...
		WHERE 
			...
			rh_permisos.CodPermiso='".$sel_registros."'";
...
$pdf->SetXY(51, $y+5); $pdf->SetFont('Arial', '', 9); $pdf->Cell(153, 5, utf8_decode($field['NomCompleto']), 0, 1, 'L');
$y=62;
...
$pdf->SetXY(11, $y+5); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(153, 5, utf8_decode($field['DescripCargo']), 0, 'L');
$pdf->SetXY(166, $y+5); $pdf->SetFont('Arial', '', 9); $pdf->Cell(38, 5, utf8_decode($field['CodEmpleado']), 0, 1, 'C');
$y=75;
...
$pdf->SetXY(11, $y+6); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(193, 4, utf8_decode($field['Dependencia']), 0, 'L');
$y=92;
...
$y=105;
$pdf->Rect(10, $y, 195, $h1);
$pdf->SetXY(11, $y); $pdf->SetFont('Arial', 'B', 10); $pdf->Cell(195, 5, 'DATOS DE INCIDENCIA', 0, 1, 'C');
$y=110;
$pdf->Rect(10, $y, 50, $h1); $pdf->Rect(60, $y, 145, $h1);
$pdf->SetXY(11, $y); $pdf->SetFont('Arial', 'B', 9); $pdf->Cell(50, 5, 'TIPO:', 0, 1, 'C');
$pdf->SetXY(61, $y); $pdf->SetFont('Arial', 'B', 9); $pdf->Cell(140, 5, 'MOTIVO:', 0, 1, 'C');
$pdf->Rect(10, $y+5, 50, $h4); $pdf->Rect(60, $y+5, 145, $h4);
$pdf->SetXY(11, $y+11); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(50, 4, utf8_decode($field['TipoFalta']), 0, 'C');
$pdf->SetXY(61, $y+6); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(143, 4, utf8_decode($field['ObsMotivo']), 0, 'L');

//--
$y=132;
$pdf->Rect(10, $y, 195, $h1);
$pdf->SetXY(11, $y); $pdf->SetFont('Arial', 'B', 10); $pdf->Cell(195, 5, 'DURACION', 0, 1, 'C');
...
$sql = "SELECT * FROM mastdependencias WHERE CodDependencia = '".$field['CodDependencia']."'";
$field_empleado_dependencia = getRecord($sql);
##	consulto la dependencia del que revisa
$sql = "SELECT * FROM mastempleado WHERE CodPersona = '".$field['Aprobador']."'";
$field_revisa = getRecord($sql);
##	
if ($field_revisa['CodDependencia'] == $field['CodDependencia'] && $field_empleado_dependencia['CodPersona'] == $field['Aprobador'])
	list($AutorizadoNombre, $AutorizadoCargo, $AutorizadoNivel) = getFirmaxDependencia($field['CodDependencia']);
else
	list($AutorizadoNombre, $AutorizadoCargo, $AutorizadoNivel) = getFirma($field['Aprobador']); 
//	
$y=157;
$pdf->Rect(10, $y, 100, $h1); $pdf->Rect(110, $y, 95, $h1);
...
$pdf->SetXY(11, $y+7); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(98, 4, utf8_decode($AutorizadoNombre), 0, 'L');
$pdf->SetXY(111, $y+7); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(93, 5, utf8_decode($AutorizadoCargo), 0, 'L');
$y=174;
$pdf->Rect(10, $y, 100, $h1); $pdf->Rect(110, $y, 95, $h1);
...
$pdf->Rect(10, $y+5, 195, $h3);
$pdf->SetXY(11, $y+5); $pdf->SetFont('Arial', 'B', 10); $pdf->MultiCell(193, 4, utf8_decode($field['ObsAprobado']), 0, 'L');
...
$pdf->Rect(10, $y+5, 100, $h3); $pdf->Rect(110, $y+5, 95, $h3);
//	
list($VerificadoNombre, $VerificadoCargo, $VerificadoNivel) = getFirmaxDependencia($_PARAMETRO["DEPRHPR"]);
//	
$pdf->SetXY(11, $y+7); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(98, 4, utf8_decode($VerificadoNombre), 0, 'L');
$pdf->SetXY(111, $y+7); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(93, 5, utf8_decode($VerificadoCargo), 0, 'L');
$y=226;
$pdf->Rect(10, $y, 100, $h1); $pdf->Rect(110, $y, 95, $h1);
$pdf->SetXY(11, $y); $pdf->SetFont('Arial', 'B', 9); $pdf->Cell(98, 5, 'FIRMA Y SELLO:', 0, 1, 'C');
$pdf->SetXY(111, $y); $pdf->SetFont('Arial', 'B', 9); $pdf->Cell(93, 5, 'FECHA:', 0, 1, 'C');
...
$pdf->SetXY(155, $y+9); $pdf->SetFont('Arial', '', 9); $pdf->MultiCell(48, 4, utf8_decode($justificativo), 0, 'L');
//---------------------------------------------------
if ($field['Estado'] != "A") {
	$pdf->SetXY(20, 170); 
