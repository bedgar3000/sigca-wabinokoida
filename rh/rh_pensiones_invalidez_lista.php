...
//	------------------------------------
if ($lista == "todos") {
	$titulo = "Lista de Pensiones x Invalidez";
	$btConformar = "display:none;";
	$btAprobar = "display:none;";
}
elseif ($lista == "conformar") {
	$titulo = "Conformar Pensiones x Invalidez";
...
elseif ($lista == "aprobar") {
	$titulo = "Aprobar Pensiones x Invalidez";
	$btNuevo = "display:none;";
	$btModificar = "display:none;";
	$btConformar = "display:none;";
	$fEstado = "CN";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=rh_pensiones_invalidez_lista" method="post" autocomplete="off">
...
<table width="950" class="tblBotones">
    <tr>
        ...
        <td align="right">
            <input type="button" id="btNuevo" value="Nuevo" style="width:75px; <?=$btNuevo?>" onclick="cargarPagina(this.form, 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=nuevo');" />
            
            <input type="button" id="btModificar" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#sel_registros').val(), 'accion=pensiones_modificar', 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=modificar', 'SELF', '');" />
            
            <input type="button" id="btConformar" value="Conformar" style="width:75px; <?=$btConformar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=conformar', 'SELF', '', $('#sel_registros').val());" />
            
            <input type="button" id="btAprobar" value="Aprobar" style="width:75px; <?=$btAprobar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=aprobar', 'SELF', '', $('#sel_registros').val());" />
            
            <input type="button" id="btAnular" value="Anular" style="width:75px; <?=$btAnular?>" onclick="cargarOpcionValidar2(this.form, $('#sel_registros').val(), 'accion=pensiones_anular', 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=anular', 'SELF', '');" />
            
            <input type="button" id="btVer" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=rh_pensiones_invalidez_form&opcion=ver', 'SELF', '', $('#sel_registros').val());" />
        </td>
    </tr>
</table>
