...
$sql = "SELECT
			...
			d1.CodPersona AS DepCodPersona,
			d1.CodCargo AS DepCodCargo,
...
$sql = "SELECT * FROM mastempleado WHERE CodPersona = '".$field['RevisadoPor']."'";
$field_revisa = getRecord($sql);
##	
if ($field_revisa['CodDependencia'] == $field['CodDependencia'] && $field_empleado_dependencia['CodPersona'] == $field['RevisadoPor'])
	list($_REVISADO['Nombre'], $_REVISADO['Cargo'], $_REVISADO['Nivel']) = getFirmaxDependencia($field['CodDependencia']);
else
...
if ($field['AprobadoPor'] == $field_empleado_dependencia['CodPersona'])
	list($_APROBADO['Nombre'], $_APROBADO['Cargo'], $_APROBADO['Nivel']) = getFirmaxDependencia($_PARAMETRO["DEPRHPR"]);
else 
...
while ($field_detalles = mysql_fetch_array($query_detalles)) {
	...
	$pdf->Cell(110, 5, utf8_decode(intval($field_detalles['DiasDerecho']).' DIAS HÁBILES '), 0, 0, 'L');
	$pdf->Ln();
	$pdf->Cell(110, 5, utf8_decode('CANTIDAD DE DIAS DISFRUTADOS (ANTERIOR Y ACTUAL):'), 0, 0, 'L');
	$pdf->Cell(110, 5, utf8_decode(intval($field_detalles['DiasUsados']+$field_detalles['NroDias']).' DIAS HÁBILES '), 0, 0, 'L');
	$pdf->Ln();
	$pdf->Cell(110, 5, utf8_decode('CANTIDAD DE DIAS PENDIENTES:'), 0, 0, 'L');
	$pdf->Cell(110, 5, utf8_decode(intval($field_detalles['DiasDerecho']-$field_detalles['DiasUsados']-$field_detalles['NroDias']).' DIAS HÁBILES '), 0, 0, 'L');
	##
	$pdf->Ln();
	$pdf->SetFillColor(200, 200, 200);
	...
	$pdf->Cell(97.5, 5, utf8_decode('APROBADOR POR / JEFE INMEDIATO'), 1, 0, 'C');
	$pdf->SetXY(10, 230);
