...
if ($rows!=0) {
	...
	$primas_letras = convertir_a_letras($primas, "moneda");
	$_PRIMAS = utf8_decode(strtoupper("$primas_letras (Bs. $total_primas)"));
	
	$sueldo_normal = number_format($total_ingresos, 2, ',', '.');
	$sueldo_normal_letras = convertir_a_letras($total_ingresos, "moneda");
	...
	$dia_letras=convertir_a_letras($dia_actual, "moneda");
	$dia="$dia_letras ($dia_actual)";
	$anio_letras=convertir_a_letras($anio_actual, "moneda");
	...
	if ($archivo) {
		...
		<table width='100%' cellspacing='0' cellpadding='0'>
			<tr>
				...
				<td class='tagForm'><a class='cerrar' href='javascript:' onclick='window.close();'>[cerrar]</a></td>
			</tr>
		...
		header('Content-Disposition: inline; filename='.$registro.'.rtf');
		$output="{\\rtf1";
		$output.=$texto;
		$output.="}";
		echo $output;
