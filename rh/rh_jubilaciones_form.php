...
elseif ($opcion == "modificar" || $opcion == "ver" || $opcion == "conformar" || $opcion == "aprobar" || $opcion == "anular") {
	...
	$sql = "SELECT
				pj.*,
				(pj.TotalSueldo + pj.TotalPrimas) AS Total,
				((pj.TotalSueldo + pj.TotalPrimas) / 24) AS SueldoBase,
			...
			FROM
				...
				LEFT JOIN mastpersonas p2 ON (p2.CodPersona = pj.ProcesadoPor)
				LEFT JOIN mastpersonas p3 ON (p3.CodPersona = pj.ConformadoPor)
				LEFT JOIN mastpersonas p4 ON (p4.CodPersona = pj.AprobadoPor)
			WHERE pj.CodProceso = '".$sel_registros."'";
...
<table width="800" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td>
            ...
            <li id="li2" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 2, 4);">Antecedentes de Servicio</a></li>
            <li id="li3" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 3, 4);">Relaci&oacute;n de Sueldos</a></li>
            <li id="li4" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 4, 4);">Jubilaci&oacute;n</a></li>
            </ul>
            </div>
        </td>
    </tr>
...
<form name="frmgeneral" id="frmgeneral" action="gehen.php?anz=rh_jubilaciones_lista" method="POST" enctype="multipart/form-data" onsubmit="return false;" autocomplete="off">
...
<input type="hidden" id="TipoProceso" value="J" />
...
<table width="800" class="tblForm">
    ...
    <tr>
		...
		<td class="gallery clearfix">
			...
			<a href="../lib/listas/listado_empleados.php?filtrar=default&ventana=jubilaciones_empleados_sel&iframe=true&width=950&height=525" rel="prettyPhoto[iframe1]" style=" <?=$display_modificar?>">
    ...
    <tr>
		...
		<td>
            <input type="text" id="SueldoActual" style="width:100px; text-align:right;" value="<?=number_format($field['SueldoActual'], 2, ',', '.')?>" disabled="disabled" />
		</td>
...
<form name="frmjubilacion" id="frmjubilacion" action="gehen.php?anz=rh_jubilaciones_lista" method="POST" enctype="multipart/form-data" onsubmit="return jubilaciones('<?=$accion?>');" autocomplete="off">
...
<table width="800" class="tblForm">
    ...
    <tr>
		...
		<td colspan="3">
            ...
            <input type="text" style="width:94px;" class="codigo" value="<?=strtoupper(printValores("ESTADO-JUBILACION", $field['Estado']))?>" disabled="disabled" />
		</td>
    </tr>
    <tr>
...
<center>
<input type="button" value="<?=$label_submit?>" style="width:75px; <?=$display_submit?>" onclick="jubilaciones('<?=$accion?>');" />
