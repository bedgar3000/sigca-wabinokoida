...
function Cabecera($pdf) {
	...
	$pdf->SetXY(20, 10); $pdf->Cell(190, 5, utf8_decode('Alcaldía de Angostura'), 0, 1, 'L');
...
Cabecera($pdf);
//	Cuerpo
$sql="SELECT me.CodEmpleado, m.NomCompleto, m.Ndocumento, m.Fnacimiento, me.CodTipoTrabajador, SUBSTRING(m.Fnacimiento, 0, 4) AS AnioNac, SUBSTRING(m.Fnacimiento, 6, 2) AS MesNac, SUBSTRING(m.Fnacimiento, 9, 2) AS DiaNac FROM mastpersonas m INNER JOIN mastempleado me ON (m.CodPersona=me.CodPersona) $filtro ORDER BY MesNac, DiaNac";
...
for ($i=1; $i<=$rows; $i++) {
	...
	$pdf->Cell(20, 5); $pdf->Row(array($field['CodEmpleado'], $field['Ndocumento'], $field['NomCompleto'], $Fnacimiento, $edad));
	$con++;
	if ($i==$rows) {
		$pdf->Ln(2);
		$pdf->SetFont('Arial', 'BI', 8);
