...
if ($ficha == "competencia")
	$sql="SELECT * FROM rh_gradoscompetencia ORDER BY PuntajeMin";
elseif ($ficha == "objetivos")
	$sql="SELECT * FROM rh_evaluacionperiodovalor WHERE CodOrganismo='".$organismo."' AND Periodo='".$periodo."' AND Secuencia='".$secuencia."'";
elseif ($ficha == "funciones")
	$sql="SELECT * FROM rh_evaluacionperiodovalor WHERE CodOrganismo='".$organismo."' AND Periodo='".$periodo."' AND Secuencia='".$secuencia."'";
else
	$sql="SELECT * FROM rh_evaluacionperiodovalor WHERE CodOrganismo='".$organismo."' AND Periodo='".$periodo."' AND Secuencia='".$secuencia."'";
...
<table width="300" class="tblLista">
	...
	if ($registros!=0) {
		//	CONSULTO LA TABLA
		if ($ficha == "competencia")
			$sql="SELECT *, PuntajeMin AS Valor FROM rh_gradoscompetencia ORDER BY PuntajeMin LIMIT $limit, $MAXLIMIT";
		elseif ($ficha == "objetivos")
