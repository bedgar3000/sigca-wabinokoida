...
function vacaciones_periodos_insertar() {
	...
	if (isNaN(NroDias) || NroDias <= 0 || !valFecha(FechaSalida)) $("#lista_detalles").html("");
		...
		$.ajax({
			...
			async: true,
			success: function(resp) {
				var datos = resp.split(";char:tr;");
				$("#lista_detalles").html(datos[0]);
				totalizarPeriodoVacaciones1();
				$("#FechaSalida").val(datos[1]);
				$("#FechaTermino").val(datos[2]);
				$("#FechaIncorporacion").val(datos[3]);
			}
		});
	}
}
...
//	INSERTAR LOS PERIODOS VACACIONALES VENCIDOS 
function vacaciones_periodos_insertar_interrupcion() {
	var CodPersona = $("#CodPersona").val();
	var AnioRelacionada = $("#AnioRelacionada").val();
	var CodSolicitudRelacionada = $("#CodSolicitudRelacionada").val();
	var NroDias = parseFloat(setNumero($("#NroDias").val()));
	var FechaSalida = $("#FechaSalida").val();
	if (isNaN(NroDias) || NroDias <= 0 || !valFecha(FechaSalida)) $("#lista_detalles").html("");
	else {
		...
		$.ajax({
			type: "POST",
			url: "lib/fphp_funciones_ajax.php",
			data: "accion=vacaciones_periodos_insertar_interrupcion&CodPersona="+CodPersona+"&NroDias="+NroDias+"&FechaSalida="+FechaSalida+"&AnioRelacionada="+AnioRelacionada+"&CodSolicitudRelacionada="+CodSolicitudRelacionada,
			async: true,
			success: function(resp) {
				var datos = resp.split(";char:tr;");
				$("#lista_detalles").html(datos[0]);
				$("#FechaTermino").val(datos[1]);
				$("#FechaIncorporacion").val(datos[2]);
			}
		});
	}
}
...
function bono_periodos_registrar_eventos_insertar(boton, secuencia, FechaInicio, FechaFin) {
	...
	$.ajax({
		type: "POST",
		url: "lib/fphp_funciones_ajax.php",
		data: "accion=bono_periodos_registrar_eventos_insertar&nrodetalle="+nrodetalle+"&candetalle="+candetalle+"&FechaInicio="+FechaInicio+"&FechaFin="+FechaFin+"&secuencia="+secuencia+"&CodPersona="+CodPersona,
		async: true,
		success: function(resp) {
			$("#"+nro).val(nrodetalle);
			$("#"+can).val(candetalle);
...
function getHoraEventos(Secuencia) {
	...
	if (valFecha(Fecha)) {
		$.ajax({
			type: "POST",
			url: "lib/fphp_funciones_ajax.php",
			data: "accion=getHoraEventos&CodPersona="+CodPersona+"&CodHorario="+CodHorario+"&Fecha="+Fecha,
			async: true,
			success: function(resp) {
				var datos = resp.split('|');
				Desde.val(datos[0]);
