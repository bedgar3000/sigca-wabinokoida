...
if ($modulo == "EMPLEADO-IMPUESTO") {
	...
	if ($accion == "INSERTAR") {
		...
		$sql = "INSERT INTO pr_impuestorenta (CodPersona, Anio, Desde, Hasta, Porcentaje, UltimoUsuario, UltimaFecha) VALUES ('".$codpersona."', '".$anio."', '".$desde."', '".$hasta."', '".$porcentaje."', '".$_SESSION['USUARIO_ACTUAL']."', NOW())";
	...
	elseif ($accion == "MODIFICAR") {
		...
		$sql = "UPDATE pr_impuestorenta SET Desde = '".$desde."', Hasta = '".$hasta."', Porcentaje = '".$porcentaje."', UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."', UltimaFecha = NOW() WHERE CodPersona = '".$codpersona."' AND Anio = '".$anio."'";
		$query = mysql_query($sql) or die ($sql.mysql_error());
	}
...
elseif ($modulo == "SUELDOS-MINIMOS") {
	if ($accion == "GUARDAR") {
		...
		$sql = "INSERT INTO mastsueldosmin (Secuencia, Periodo, Monto, UltimoUsuario, UltimaFecha) VALUES ('".$secuencia."', '".$periodo."', '".$monto."', '".$_SESSION['USUARIO_ACTUAL']."', NOW())";
		$query = mysql_query($sql) or die ($sql.mysql_error());
	}
	...
	if ($accion == "GUARDAR") {
		...
		if (trim($detalles_tab6)) {
			...
			foreach ($detalle_tab6 as $linea) {
				...
				$secuencia_det++;
				$sql = "xINSERT INTO rh_empleado_necesidad (
									CodOrganismo,
									Periodo,
									Secuencia,
									CodPersona,
