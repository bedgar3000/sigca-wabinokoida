...
//	consulto si se puede modificar una solicitud de vacaciones
elseif ($accion == "vacaciones_interrumpir") {
	list($Anio, $CodSolicitud) = split("[.]", $codigo);
	$sql = "SELECT Estado
			FROM rh_vacacionsolicitud
			WHERE
	...
	if (mysql_num_rows($query) != 0) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] != "AP") die("Solo se puede interrumpir las Solicitudes <strong>Conformadas</strong>");
	} else die("No se encuentra el registro");
}
...
elseif ($accion == "vacaciones_periodos_insertar") {
	...
	$FechaInicioVacacion = $FechaSalida;
	...
	$FechaFinVacacion = $FechaFin;
	$FechaIncorporacionVacacion = getFechaFinHabiles($FechaFinVacacion, 2);
	echo ";char:tr;$FechaInicioVacacion";
	echo ";char:tr;$FechaFinVacacion";
	echo ";char:tr;$FechaIncorporacionVacacion";
}

//	muestro los periodos disponibles
elseif ($accion == "vacaciones_periodos_insertar_interrupcion") {
	##	solicitud
	$sql = "SELECT *
			FROM rh_vacacionsolicitud
			WHERE
				Anio = '".$AnioRelacionada."' AND
				CodSolicitud = '".$CodSolicitudRelacionada."'";
	$field = getRecord($sql);
	$_Pendiente = $NroDias;
	$_FechaInicio = $FechaSalida;
	##	detalle
	$sql = "SELECT
				vsd.*,
				vp.Anio AS AnioVacacion
			FROM
				rh_vacacionsolicituddetalle vsd
				INNER JOIN mastempleado e ON (e.CodPersona = vsd.CodPersona)
				INNER JOIN rh_vacacionperiodo vp ON (vp.CodPersona = vsd.CodPersona AND
													 vp.NroPeriodo = vsd.NroPeriodo AND
													 vp.CodTipoNom = e.CodTipoNom)
			WHERE
				vsd.Anio = '".$AnioRelacionada."' AND
				vsd.CodSolicitud = '".$CodSolicitudRelacionada."'";
	$field_detalle = getRecords($sql);
	$i=0;
	foreach($field_detalle as $f) {
		++$i;
		if ($f['NroDias'] <= $_Pendiente) $_NroDias = $f['NroDias']; else $_NroDias = $_Pendiente;
		$_Pendiente -= $_NroDias;
		$_FechaFin = getFechaFinHabiles($_FechaInicio, $_NroDias);
		?>
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');">
			<th>
				<input type="text" name="NroPeriodo" class="cell2" style="text-align:center;" value="<?=$f['NroPeriodo']?>" readonly />
			</th>
			<td align="center">
				<input type="checkbox" name="FlagUtlizarPeriodo" checked="checked" disabled="disabled" />
			</td>
			<td align="center">
				<?=$f['AnioVacacion']?> - <?=$f['AnioVacacion']+1?>
			</td>
			<td>
				<input type="text" name="NroDias" class="cell" style="text-align:right;" value="<?=number_format($_NroDias, 2, ',', '.')?>" disabled="disabled" />
			</td>
			<td>
				<input type="text" name="FechaInicio" value="<?=$_FechaInicio?>" style="text-align:center;" class="cell datepicker" disabled="disabled" />
			</td>
			<td>
				<input type="text" name="FechaFin" value="<?=$_FechaFin?>" style="text-align:center;" class="cell datepicker" disabled="disabled" />
			</td>
			<td>
				<input type="text" name="Derecho" value="<?=number_format($f['DiasDerecho'], 2, ',', '.')?>" class="cell2" style="text-align:right;" readonly />
			</td>
			<td>
				<input type="text" name="TotalUtilizados" value="<?=number_format($f['DiasUsados'], 2, ',', '.')?>" class="cell2" style="text-align:right;" readonly />
			</td>
			<td>
				<input type="text" name="Pendientes" value="<?=number_format($f['DiasPendientes'], 2, ',', '.')?>" class="cell2" style="text-align:right;" readonly />
			</td>
			<td>
				<textarea name="Observaciones" class="cell" style="height:20px;" disabled="disabled"><?=htmlentities($f['Observaciones'])?></textarea>
				<input type="hidden" name="Secuencia" />
			</td>
		</tr>
		<?php
		$_FechaInicio = getFechaFinHabiles($_FechaFin, 2);
		if ($_Pendiente <= 0) break;
	}
	$FechaFinVacacion = $_FechaFin;
	$FechaIncorporacionVacacion = getFechaFinHabiles($_FechaFin, 2);
	echo ";char:tr;$FechaFinVacacion";
	echo ";char:tr;$FechaIncorporacionVacacion";
}
...
//	consulto si se puede modificar
elseif ($accion == "reingreso_modificar") {
	list($Tipo, $CodProceso) = split("[_]", $codigo);
	$sql = "SELECT Estado
			FROM rh_procesocesereing
			WHERE
				Tipo = '".$Tipo."' AND
	...
	if (mysql_num_rows($query)) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] != "PE") die("No se puede modificar este registro");
	} else die("No se encuentra el registro");
	
}

//	consulto si se puede modificar
elseif ($accion == "reingreso_conformar") {
	list($Tipo, $CodProceso) = split("[_]", $codigo);
	$sql = "SELECT Estado
			FROM rh_procesocesereing
			WHERE
	...
	if (mysql_num_rows($query)) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] != "PE") die("No se puede conformar este registro");
	} else die("No se encuentra el registro");
	
}

//	consulto si se puede modificar
elseif ($accion == "reingreso_aprobar") {
	list($Tipo, $CodProceso) = split("[_]", $codigo);
	$sql = "SELECT Estado
			FROM rh_procesocesereing
			WHERE
	...
	if (mysql_num_rows($query)) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] != "CN") die("No se puede aprobar este registro");
	} else die("No se encuentra el registro");
	
}

//	consulto si se puede modificar
elseif ($accion == "reingreso_anular") {
	list($Tipo, $CodProceso) = split("[_]", $codigo);
	$sql = "SELECT Estado
			FROM rh_procesocesereing
			WHERE
	...
	if (mysql_num_rows($query)) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] == "AN") die("No se puede anular este registro");
	} else die("No se encuentra el registro");
