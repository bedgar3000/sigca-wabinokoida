...
function loadSelectValores($tabla, $codigo=NULL, $opt=0) {
	switch ($tabla) {	
		...
		case "ESTADO-VACACIONES":
			...
			$c[1] = "RV"; $v[1] = "Autorizado";
			$c[2] = "CO"; $v[2] = "Revisado";
			$c[3] = "AP"; $v[3] = "Conformado";
			$c[4] = "AN"; $v[4] = "Anulado";
			break;
		...
		case "tipo-descuento":
			$c[0] = "P"; $v[0] = "Porcentaje";
			$c[1] = "M"; $v[1] = "Monto";
			break;

		case "tipo-sueldo":
			$c[0] = "SM"; $v[0] = "Sueldo Mínimo";
			$c[1] = "SN"; $v[1] = "Sueldo Normal";
			$c[2] = "SI"; $v[2] = "Sueldo Integral";
			$c[3] = "SP"; $v[3] = "Sueldo Integral Parcial";
			break;
...
function loadSelectEvaluacion($Evaluacion) {
	...
	while ($field = mysql_fetch_array($query)) {	++$i;
		if ($Grupo != $field['TipoEvaluacion']) {
			...
			if ($i>1) { ?></optgroup><?php }
			?><optgroup label="<?=htmlentities($field['TipoEvaluacion']." ".$field['NomTipoEvaluacion'])?>"><?php
		}
		if ($field[0] == $Evaluacion) { ?><option value="<?=$field[0]?>" selected="selected"><?=($field[1])?></option><?php }
...
function loadSelectGrupoCompetencia($Area, $TipoEvaluacion) {
	...
	while ($field = mysql_fetch_array($query)) {	++$i;
		...
		if ($field[0] == $Area) { ?><option value="<?=$field[0]?>" selected="selected"><?=($field[1])?></option><?php }
		else { ?><option value="<?=$field[0]?>"><?=($field[1])?></option><?php }
	}
}
...
function loadSelectValoresGrado($TipoEvaluacion, $Puntaje) {
	...
	for($i=$field['Min']; $i<=$field['Max']; $i++) {
        if ($Puntaje == $i) { ?><option value="<?=$i?>" selected="selected"><?=$i?></option><?php }
		else { ?><option value="<?=$i?>"><?=$i?></option><?php }
	}
}
...
function loadSelectEvaluaciones($CodOrganismo, $Secuencia) {
	...
	while ($field = mysql_fetch_array($query)) {
		if ($field[0] == $Secuencia) { ?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[1])?></option><?php }
