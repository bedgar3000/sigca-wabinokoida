...
if ($_GET['filtro']!="") $sql="SELECT * FROM mastpersonas WHERE (CodPersona LIKE '%".$_GET['filtro']."%' OR Busqueda LIKE '%".$_GET['filtro']."%' OR Ndocumento LIKE '%".$_GET['filtro']."%' OR DocFiscal LIKE '%".$_GET['filtro']."%') AND EsOtros = 'S'";
else $sql="SELECT * FROM mastpersonas WHERE EsOtros = 'S'";
...
<table width="700" class="tblLista">
	...
	if ($registros!=0) {
		//	CONSULTO LA TABLA
		if ($_GET['filtro']!="") $sql="SELECT CodPersona, Busqueda, EsCliente, EsProveedor, EsEmpleado, EsOtros, Ndocumento, DocFiscal FROM mastpersonas WHERE (CodPersona LIKE '%".$_GET['filtro']."%' OR Busqueda LIKE '%".$_GET['filtro']."%' OR Ndocumento LIKE '%".$_GET['filtro']."%' OR DocFiscal LIKE '%".$_GET['filtro']."%') AND EsOtros = 'S' ORDER BY CodPersona LIMIT ".$_GET['limit'].", $MAXLIMIT";
		else $sql="SELECT CodPersona, Busqueda, EsCliente, EsProveedor, EsEmpleado, EsOtros, Ndocumento, DocFiscal FROM mastpersonas WHERE EsOtros = 'S' ORDER BY CodPersona LIMIT ".$_GET['limit'].", $MAXLIMIT";
		$query=mysql_query($sql) or die ($sql.mysql_error());
		...
		for ($i=0; $i<$rows; $i++) {
			...
			echo "
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); selPersonaOtros(\"".$field['Busqueda']."\", ".$campo.");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['CodPersona']."'>
