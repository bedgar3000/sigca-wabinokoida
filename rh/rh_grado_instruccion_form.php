...
if ($opcion == "nuevo") {
	...
	$focus = "CodGradoInstruccion";
}
elseif ($opcion == "modificar" || $opcion == "ver") {
	//	consulto datos generales
	$sql = "SELECT * FROM rh_gradoinstruccion WHERE CodGradoInstruccion = '".$sel_registros."'";
	$field = getRecord($sql);
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=rh_grado_instruccion_lista" method="POST" enctype="multipart/form-data" onsubmit="return form(this, '<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
	...
	<tr>
		...
		<td>
        	<input type="text" id="CodGradoInstruccion" value="<?=$field['CodGradoInstruccion']?>" style="width:50px;" maxlength="3" <?=$disabled_modificar?> />
		</td>
	</tr>
	<tr>
...
function form(form, accion) {
	//	valido
	var error = "";
	if ($("#CodGradoInstruccion").val().trim() == "" || $("#Descripcion").val().trim() == "") error = "Debe llenar los campos obligatorios";
	...
	if (error != "") {
		...
		$.ajax({
			...
			data: "modulo=grado_instruccion&accion="+accion+"&"+post,
			async: false,
