...
if ($fbuscar != "") {
	$cbuscar = "checked";
	$filtro.=" AND (p.CodPais LIKE '%".$fbuscar."%' OR 
					p.Pais LIKE '%".$fbuscar."%')";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=paises_lista" method="post">
	...
	<tr>
		...
		<td>
            ...
            <select name="fordenar" id="fordenar" style="width:100px;" <?=$dordenar?>>
                <?=loadSelectGeneral("ORDENAR-PAISES", $fordenar, 0)?>
            </select>
		</td>
	</tr>
</table>
...
<table width="700" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'gehen.php?anz=paises_form&opcion=nuevo');" />
            
			<input type="button" class="btLista" id="btModificar" value="Modificar" onclick="cargarOpcion2(this.form, 'gehen.php?anz=paises_form&opcion=modificar', 'SELF', '', $('#registro').val());" />
            
			<input type="button" class="btLista" id="btEliminar" value="Eliminar" onclick="opcionRegistro2(this.form, this.form.registro.value, 'paises', 'eliminar');" />
            
			<input type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=paises_form&opcion=ver', 'BLANK', 'height=300, width=700, left=100, top=0, resizable=no', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	while ($field_lista = mysql_fetch_array($query_lista)) {
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$field_lista['CodPais']?>">
