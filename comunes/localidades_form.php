...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	$sql = "SELECT
			...
			WHERE l.CodLocalidad = '".$sel_registros."'";
	$field = getRecord($sql);
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=localidades_lista" method="POST" enctype="multipart/form-data" onsubmit="return localidades(this, '<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
	...
	<tr>
		...
		<td>
        	<input type="text" id="CodLocalidad" value="<?=$field['CodLocalidad']?>" style="width:50px;" disabled />
		</td>
	</tr>
	<tr>
...
function localidades(form, accion) {
	bloqueo(true);
	//	valido
	var error = "";
	...
	if (error != "") {
		...
		$.ajax({
			type: "POST",
			url: "localidades_ajax.php",
			data: "modulo=localidades&accion="+accion+"&"+post,
			async: false,
