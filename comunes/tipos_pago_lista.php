<?php
if ($filtrar == "default") {
	$fordenar = "tp.CodTipoPago";
	$fedoreg = "A";
	$maxlimit = $_SESSION["MAXLIMIT"];
}
if ($fordenar != "") { $cordenar = "checked"; $orderby = "ORDER BY $fordenar"; } else $dordenar = "disabled";
if ($fedoreg != "") { $cedoreg = "checked"; $filtro.=" AND (tp.Estado = '".$fedoreg."')"; } else $dedoreg = "disabled";
if ($fbuscar != "") {
	$cbuscar = "checked";
...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Tipos de Pago</td>
		<td align="right"><a class="cerrar" href="../framemain.php">[cerrar]</a></td>
	</tr>
</table><hr width="100%" color="#333333" /><br />

<form name="frmentrada" id="frmentrada" action="gehen.php?anz=tipos_pago_lista" method="post">
	...
	<tr>
		...
		<td>
            ...
            <select name="fordenar" id="fordenar" style="width:100px;" <?=$dordenar?>>
                <?=loadSelectGeneral("ORDENAR-TIPOPAGO", $fordenar, 0)?>
            </select>
		</td>
	</tr>
</table>
...
<table width="700" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'gehen.php?anz=tipos_pago_form&opcion=nuevo');" />
            
			<input type="button" class="btLista" id="btModificar" value="Modificar" onclick="cargarOpcion2(this.form, 'gehen.php?anz=tipos_pago_form&opcion=modificar', 'SELF', '', $('#registro').val());" />
            
			<input type="button" class="btLista" id="btEliminar" value="Eliminar" onclick="opcionRegistro2(this.form, this.form.registro.value, 'tipos_pago', 'eliminar');" />
            
			<input type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=tipos_pago_form&opcion=ver', 'BLANK', 'height=300, width=700, left=100, top=0, resizable=no', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	while ($field_lista = mysql_fetch_array($query_lista)) {
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$field_lista['CodTipoPago']?>">
			<td align="center"><?=$field_lista['CodTipoPago']?></td>
			<td><?=($field_lista['TipoPago'])?></td>
