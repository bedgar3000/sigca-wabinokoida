...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=plan_cuentas_lista" method="post">
	...
	<tr>
		...
		<td>
            ...
            <select name="ftipocuenta" id="ftipocuenta" style="width:125px;" <?=$dtipocuenta?>>
                <option value="">&nbsp;</option>
                <?=getMiscelaneos($ftipocuenta, "CUENTAS")?>
            </select>
		</td>
...
<table width="900" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'gehen.php?anz=plan_cuentas_form&opcion=nuevo');" />
			<input type="button" class="btLista" id="btModificar" value="Modificar" onclick="cargarOpcion2(this.form, 'gehen.php?anz=plan_cuentas_form&opcion=modificar', 'SELF', '', $('#registro').val());" />
			...
			<input type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=plan_cuentas_form&opcion=ver', 'SELF', '', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				ac_mastplancuenta pc
				LEFT JOIN mastmiscelaneosdet md ON (pc.TipoCuenta = md.CodDetalle AND md.CodMaestro = 'CUENTAS')
			WHERE 1 $filtro";
