...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Listado Plan de Cuentas Contables </td>
		<td align="right"><a class="cerrar" href="../framemain.php">[cerrar]</a></td>
	</tr>
</table><hr width="100%" color="#333333" /><br />

<form name="frmentrada" id="frmentrada" action="gehen.php?anz=listado_cuentas_contables" method="post">
...
<table width="900" class="tblBotones">
	<tr>
		...
		<td align="right">
			...
			<input type="button" class="btLista" id="btEliminar" value="Eliminar" onclick="opcionRegistro2(this.form, this.form.registro.value, 'plan_cuentas_pub20', 'eliminar');" />
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN mastmiscelaneosdet md ON (pc.TipoCuenta = md.CodDetalle AND md.CodMaestro = 'CUENTAS')
			WHERE 1 $filtro";
	...
	while ($field = mysql_fetch_array($query)) {
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro'); listar_cuentas_pub20('<?=htmlentities($field["Descripcion"])?>', '<?=$_GET['campo']?>', '<?=$nom?>','<?=$_GET['nrodetalle'];?>');" id="<?=$field['CodCuenta']?>">
