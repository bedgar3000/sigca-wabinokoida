...
function centro_costos(form, accion) {
	...
	for(var i=0; n=form.elements[i]; i++) {
		...
		//	errores
		if ((n.id == "Codigo" || n.id == "Descripcion" || n.id == "CodDependencia" || n.id == "CodSubGrupoCentroCosto") && n.value.trim() == "") { error = "Debe llenar los campos obligatorios"; break; }
		else if (!valAlfaNumerico(n.value)) { error = "No se permiten caractéres especiales en los campos"; break; }
	}
...
/// ------------------------------------------------------------------------------
//	plan de cuentas pub 20
function plan_cuentas_pub20(form, accion) {
	//	formulario
	var post = "";
	var error = "";
	for(var i=0; n=form.elements[i]; i++) {
		...
		if (n.id == "CodCuenta") {
			var Grupo = CodCuenta.substring(0, 1);	post += "Grupo=" + Grupo + "&";
			var SubGrupo = CodCuenta.substring(1, 2);	post += "SubGrupo=" + SubGrupo + "&";
			var Rubro = CodCuenta.substring(2, 5);	post += "Rubro=" + Rubro + "&";
			var Cuenta = CodCuenta.substring(5, 7);	post += "Cuenta=" + Cuenta + "&";
			var SubCuenta1 = CodCuenta.substring(7, 9);	post += "SubCuenta1=" + SubCuenta1 + "&";
			var SubCuenta2 = CodCuenta.substring(9, 11);	post += "SubCuenta2=" + SubCuenta2 + "&";
			var SubCuenta3 = CodCuenta.substring(11, 13);	post += "SubCuenta3=" + SubCuenta3 + "&";
		}
		...
		if ((n.id == "Nivel" || n.id == "CodCuenta" || n.id == "Descripcion" || n.id == "TipoCuenta") && n.value.trim() == "") { error = "Debe llenar los campos obligatorios"; break; }
			...
			else if (n.value == 3 && CodCuenta.length != 5) error = "El código de la cuenta debe tener 5 caracteres";
			else if (n.value == 4 && CodCuenta.length != 7) error = "El código de la cuenta debe tener 7 caracteres";
			else if (n.value == 5 && CodCuenta.length != 9) error = "El código de la cuenta debe tener 9 caracteres";
			else if (n.value == 6 && CodCuenta.length != 11) error = "El código de la cuenta debe tener 11 caracteres";
			else if (n.value == 7 && CodCuenta.length != 13) error = "El código de la cuenta debe tener 13 caracteres";
		}
		else if (!valAlfaNumerico(n.value)) { error = "No se permiten caractéres especiales en los campos"; break; }
	}
	...
	if (error != "") {
		...
		$.ajax({
			type: "POST",
			url: "lib/form_ajax.php",
			data: "modulo=plan_cuentas_pub20&accion="+accion+"&"+post,
			async: false,
			success: function(msg){
				if (msg.trim() != "") cajaModal(msg, "error", 400);
				else form.submit();
