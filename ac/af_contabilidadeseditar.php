...
<?php echo "<input type='hidden' name='filtro' id='filtro' value='".$_POST['filtro']."' />
            <input type='hidden' name='valor' id='valor' value='AC'/>"; ?>
...
<tr>
 <td>
  ...
  <tr>
    ...
    <td><input type="text" name="descp_contabilidad"  id="descp_contabilidad" size="60" maxlength="100" value="<?=htmlentities($field['Descripcion']);?>"/>*</td>
  </tr>
   <tr>
            ...
            <?
			   while($fa = mysql_fetch_array($qa)){
			      if($fa['CodLibroCont']==$f_con['CodLibroCont']){
				     echo"<option value='".$fa['CodLibroCont']."' selected>".htmlentities($fa['Descripcion'])."</option>";
				  }else{ 
				     echo"<option value='".$fa['CodLibroCont']."'>".htmlentities($fa['Descripcion'])."</option>";
				  }
			   }
			?>
			 </select>
