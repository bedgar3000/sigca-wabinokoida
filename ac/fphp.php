...
function getTipoProcesoNomina($codigo, $periodo, $nomina, $codorganismo, $opt) {
	...
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field['Codigo']==$codigo) echo "<option value='".$field['Codigo']."' selected>".htmlentities($field['Descripcion'])."</option>"; 
				else echo "<option value='".$field['Codigo']."'>".htmlentities($field['Descripcion'])."</option>";
			}
			break;
...
function loadTipoServicioObligacion($tservicio, $rfiscal) {
	...
	$query = mysql_query($sql) or die ($sql.mysql_error());
	while ($field = mysql_fetch_array($query)) {
		if ($field['CodTipoServicio'] == $tservicio) echo "<option value='".$field[0]."' selected='selected'>".htmlentities($field[1])."</option>";
		else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
	}
}
...
function loadSelectImpuesto($impuesto, $rfiscal) {
	...
	for ($i=0; $i<$rows; $i++) {
		$field=mysql_fetch_array($query);
		if ($field[0] == $impuesto) echo "<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>"; 
		else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
	}
}
?>
