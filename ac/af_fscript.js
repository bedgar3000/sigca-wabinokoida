...
function editarContabilidades(formulario){
	...
	var checkOK = "ABCDEFGHIJKLMNOPQRSTUVWXYZÑ" + "abcdefghijklmnopqrstuvwxyzñ" + " ._/" + "0123456789" + "áéíóú";
	var checkStr = formulario.descp_contabilidad.value;
	var allValid = true; 
	for (i = 0; i < checkStr.length; i++) {
	  ch = checkStr.charAt(i); 
	...
	if (!allValid) { 
	 alert("Introduzca sólo letras o números en el campo \"Descripción\"."); 
	 formulario.descp_contabilidad.focus(); 
	 return (false); 
	} 
	...
	if (error_detalles != "") alert(error_detalles);
		...
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "../af/gmactivofijo.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				...
				else cargarPagina(formulario, "valor.php?limit=0");
			}
		}
	}
	return false;
