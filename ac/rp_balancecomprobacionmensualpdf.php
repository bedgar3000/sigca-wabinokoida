...
function Header(){
	...
	list($fano, $fmes) = SPLIT('[-]', $periodobusqueda);
    switch ($fmes) {
	...
	if(($fd!='')and($fh!='')and($fd!='0000-00')and($fh!='9999-99')){					   
		...
		$this->Cell(100, 5, utf8_decode('BALANCE DE COMPROBACIÓN AL MES DE ').$mes, 0, 1, 'C');
		$this->Ln(2);
	}else{
	    $this->SetFont('Arial', 'B', 10);
		$this->Cell(50, 5, '', 0, 0, 'C');
			...
			if($row_a!=0){
			   for($a=0; $a<$row_a; $a++){
				  ...
				  if($field_a['SaldoInicial']>0){ 
				     $saldo_inicial_debe = $field_a['SaldoInicial'];
					 $saldo_inicial_haber = '';
				  }else{  
				  	 $saldo_inicial_haber = $field_a['SaldoInicial'];
					 $saldo_inicial_debe = '';
				  }
  ...
  if($f_con01['CodContabilidad']=='T') $tabla = 'ac_mastplancuenta';
		  ...
		  $s_movmes ="select
			...
			Where
				...
				a1.CodContabilidad='".$f_con01['CodContabilidad']."'
			Group By
	...
	$contador+=1; 
       ...
       $pdf->Cell(16,4,number_format(-1*$monto_saldos_creditos,2,',','.'),'',1,'R');
...
$pdf->Cell(16, 4, number_format($total_debe_mayor, 2, ',', '.'), 1, 0, 'R');
$pdf->Cell(16, 4, number_format((-1*$total_haber_mayor), 2, ',', '.'), 1, 0, 'R');	
...
$pdf->Cell(16, 4, number_format((-1*$total_haber_saldos), 2, ',', '.'), 1, 1, 'R');
