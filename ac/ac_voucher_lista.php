...
if($fContabilidad!=""){ $filtro.= "and vm.CodContabilidad = '".$fContabilidad."' "; $cContabilidad="checked";}else $dContabilidad="disabled";;
//	------------------------------------
?>
...
<table width="1100" class="tblFiltro">
    ...
    <tr>
		...
		<td><input type="checkbox" id="chkcontabilidad" name="chkcontabilidad" value="1" <?=$cContabilidad;?> onclick="chkFiltro(this.checked,'fContabilidad');"/> <select id="fContabilidad" name="fContabilidad" <?=$dContabilidad;?>/>
        <option value=""></option>
 ...
 <thead>
	...
	$sql = "SELECT
			...
			FROM
				...
				inner join ac_voucher acv on (vm.CodVoucher = acv.CodVoucher)
			WHERE 1 $filtro
			ORDER BY vm.Periodo, vm.CodOrganismo, vm.Voucher, vm.CodContabilidad"; 
	$query = mysql_query($sql) or die ($sql.mysql_error());
	$rows = mysql_num_rows($query);
	//	MUESTRO LA TABLA
	while ($field = mysql_fetch_array($query)) {
		$id = "$field[CodOrganismo] $field[Periodo] $field[Voucher] $field[CodContabilidad]";
		?>
