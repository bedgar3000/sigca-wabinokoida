...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	list($CodTipoCargo, $CodNivelClase) = explode("_", $sel_registros);
	$sql = "SELECT *
			FROM rh_nivelclasecargo
			WHERE
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=nivel_tipo_cargo_lista" method="POST" enctype="multipart/form-data" onsubmit="return form(this, '<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
	...
	<tr>
		...
		<td>
            <select id="CodTipoCargo" style="width:200px;" <?=$disabled_modificar?>>
                <option value="">&nbsp;</option>
	...
	<tr>
		...
		<td>
        	<input type="text" id="CodNivelClase" value="<?=$field['CodNivelClase']?>" style="width:50px;" maxlength="3" <?=$disabled_modificar?> />
		</td>
	</tr>
	<tr>
		<td class="tagForm">* Descripci&oacute;n:</td>
		<td>
        	<input type="text" id="NivelClase" value="<?=$field['NivelClase']?>" style="width:315px;" maxlength="30" <?=$disabled_ver?> />
		</td>
	</tr>
	<tr>
...
function form(form, accion) {
	...
	//	valido
	var error = "";
	if ($("#NivelClase").val().trim() == "") error = "Debe llenar los campos obligatorios";
	...
	if (error != "") {
		...
		$.ajax({
			type: "POST",
			url: "nivel_tipo_cargo_ajax.php",
			data: "modulo=nivel_tipo_cargo&accion="+accion+"&"+post,
			async: false,
