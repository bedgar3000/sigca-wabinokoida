...
class PDF extends FPDF {
	...
	function Footer() {
		...
		list($RecibidoNombre, $RecibidoCargo, $RecibidoNivel) = getFirma($field['RecibidoPor']);
		list($IngresadoNombre, $IngresadoCargo, $IngresadoNivel) = getFirma($field['IngresadoPor']);

		$CentroCosto = getVar3("SELECT Descripcion FROM ac_mastcentrocosto WHERE Codigo = '".$_PARAMETRO['CCOSTOCOMPRA']."'");

		if ($field['CodTransaccion'] == "RCJ" || $field['CodTransaccion'] == "DCC" || $field['CodTransaccion'] == "RCC") $firma = "CAJA CHICA";
		else $firma = "ALMACEN";
		$this->SetDrawColor(0, 0, 0);
		$this->SetFillColor(0, 0, 0);
		//if ($TipoMovimiento != 'I') {
			//$this->Rect(20, 253, 65, 0.1, "DF");
			//$this->SetXY(20, 255); $this->Cell(65, 4, utf8_decode(mb_strtoupper($CentroCosto)), 0, 0, 'C');
		//}
		$this->Rect(130, 253, 65, 0.1, "DF");
		//if ($TipoMovimiento == 'I') {
			//$this->SetXY(130, 255); $this->Cell(65, 4, 'RECIBIDO POR', 0, 0, 'C');
		//} else {
			$this->SetXY(130, 255); $this->Cell(65, 4, 'ENTREGADO A:', 0, 0, 'C');
			$this->SetXY(130, 259); $this->Cell(65, 4, utf8_decode(mb_strtoupper($field['CentroCosto'])), 0, 0, 'C');
		//}
	}
}
//---------------------------------------------------
