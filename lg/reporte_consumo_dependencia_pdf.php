...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Image($_PATHLOGO.'logo-alcaldia.jpg', 5, 5, 10, 10);	
	...
	function Footer() {
		...
		$this->SetXY(5, 190);
		$this->Cell(76.5, 5, utf8_decode('T.S.U. Karime Contreras'), 0, 1, 'L', 0);
		$this->SetXY(91.5, 190);
		...
		$this->SetXY(5, 195);
		$this->Cell(76.5, 5, utf8_decode('ASISTENTE ADMINISTRATIVO III'), 0, 1, 'C', 0);
		$this->SetXY(91.5, 195);
		$this->Cell(76.5, 5, utf8_decode('DIRECTORA DE ADMINISTRACION'), 0, 1, 'C', 0);
		$this->SetXY(178, 195);
...
while ($field = mysql_fetch_array($query)) {	$i++;
	...
	$pdf->Row(array($field['CodItem'],
					...
					$field['CodDocumento'].'-'.$field['NroInterno'],
