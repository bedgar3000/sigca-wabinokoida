...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lg_transaccion_commodity_lista" method="post">
...
<table width="1050" class="tblFiltro">
    ...
    <tr>
		...
		<td class="gallery clearfix">
            ...
            <input type="text" name="fCodTransaccion" id="fCodTransaccion" style="width:45px;" class="disabled" value="<?=$fCodTransaccion?>" readonly="readonly" />
			<input type="text" name="fNomTransaccion" id="fNomTransaccion" style="width:240px;" class="disabled" value="<?=$fNomTransaccion?>" readonly="readonly" />
            <a href="../lib/listas/listado_tipo_transacciones_commodity.php?filtrar=default&cod=fCodTransaccion&nom=fNomTransaccion&iframe=true&width=950&height=525" rel="prettyPhoto[iframe1]" id="btTransaccion" style=" <?=$dCodTransaccion?>">
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right" class="gallery clearfix">
			<input type="button" id="btNuevo" value="Nuevo" style="width:75px; <?=$btNuevo?>" onclick="opcionValidar(this.form, 'gehen.php?anz=lg_transaccion_commodity_form&opcion=nuevo', 'CodOrganismo=<?=$fCodOrganismo?>&accion=periodoAbierto');" />
            
			<input type="button" id="btModificar" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=transaccion_commodity_modificar', 'gehen.php?anz=lg_transaccion_commodity_form&opcion=modificar', 'SELF', '');" />
            
			<input type="button" id="btVer" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_transaccion_commodity_form&opcion=ver', 'SELF', '', $('#registro').val());" />
            
            <a href="pagina.php?iframe=true" rel="prettyPhoto[iframe2]" style="display:none;" id="a_imprimir"></a>
            <input type="button" id="btImprimir" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="imprimir_transaccion_commodity();" /> | 
            
			<input type="button" id="btEjecutar" value="Ejecutar" style="width:75px; <?=$btEjecutar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_transaccion_commodity_form&opcion=ejecutar', 'SELF', '', $('#registro').val());" />

			<input type="button" id="btAnular" value="Anular" style="width:75px; <?=$btAnular?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=transaccion_commodity_anular', 'gehen.php?anz=lg_transaccion_commodity_form&opcion=anular', 'SELF', '');" />
...
<table width="1200" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN lg_almacenmast a ON (a.CodAlmacen = t.CodAlmacen AND a.FlagCommodity = 'S')
...
if ($imprimir != "") {
	...
	$(document).ready(function() {
		imprimir_transaccion_commodity('<?=$imprimir?>');
	});
    </script>
    <?php
}
