...
$sql = "SELECT
		...
		FROM
			...
			LEFT JOIN lg_ordencompra oc ON (t.ReferenciaAnio = oc.Anio AND t.CodOrganismo = oc.CodOrganismo AND t.ReferenciaNroDocumento = oc.NroOrden)
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Image($_PARAMETRO["PATHLOGO"].$Logo, 10, 5, 22, 15);		
		$this->SetFont('Arial', '', 8);
		$this->SetXY(35, 8); $this->Cell(100, 5, utf8_decode($NomOrganismo), 0, 1, 'L');
		$this->SetXY(35, 13); $this->Cell(100, 5, utf8_decode($NomDependencia), 0, 0, 'L');
		...
		if ($field['NomProveedor'] != "") {
			$Documento = formatFechaDMA($field['FechaPreparacion']).'   OC-'.$field['DocumentoReferenciaInterno'];
		...
		if ($field['CodTransaccion'] == 'ROC' || $field['CodTransaccion'] == 'RCJ') {
			...
			$this->SetWidths(array(10,15,20,126,10,18));
	...
	function Footer() {
		global $field;
		list($Nombre, $Cargo, $Nivel) = getFirma($field['RecibidoPor']);
		if ($field['CodTransaccion'] == "RCJ") $firma = "CAJA CHICA";
		else $firma = "ALMACEN";
		$this->SetDrawColor(0, 0, 0);
		$this->SetFillColor(0, 0, 0);
		//$this->Rect(20, 253, 65, 0.1, "DF");	
		//$this->SetXY(20, 255); $this->Cell(65, 4, $firma, 0, 0, 'C');
		$this->Rect(130, 253, 65, 0.1, "DF");
		$this->SetXY(130, 255); $this->Cell(65, 4, utf8_decode($firma), 0, 0, 'C');
		//$this->SetXY(130, 255); $this->Cell(65, 4, utf8_decode($Nombre), 0, 0, 'C');
		//$this->SetXY(130, 260); $this->Cell(65, 4, 'RECIBI CONFORME', 0, 0, 'C');
	}
}
//---------------------------------------------------
...
while ($field_detalle = mysql_fetch_array($query_detalle)) {
	if ($field_detalle['ReferenciaCodDocumento'] == "OC") {
		$sql = "SELECT *
				FROM lg_ordencompradetalle
				WHERE
					Anio = '".$field_detalle['ReferenciaAnio']."' AND
					CodOrganismo = '".$field_detalle['CodOrganismo']."' AND
					NroOrden = '".$field_detalle['ReferenciaNroDocumento']."' AND
					Secuencia = '".$field_detalle['ReferenciaSecuencia']."'";
		$field_compra = getRecord($sql);
		$field_detalle['CantidadPendiente'] = $field_compra['CantidadRec'] - $field_compra['CantidadRecibida'];
	} else $field_detalle['CantidadPendiente'] = 0;
