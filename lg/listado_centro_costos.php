...
<table width="800" class="tblLista">
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($ventana == "lista") {
			...
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selListadoFormularioDetalle('<?=($field["Descripcion"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['CodCentroCosto']?>">
				<td align="center"><?=$field['CodCentroCosto']?></td>
				<td><?=($field['Descripcion'])?></td>
				<td><?=($field['NomGrupo'])?></td>
				<td><?=($field['NomSubGrupo'])?></td>
			...
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selTransaccionCCosto('<?=($field["Descripcion"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['CodCentroCosto']?>">
				<td align="center"><?=$field['CodCentroCosto']?></td>
				<td><?=($field['Descripcion'])?></td>
				<td><?=($field['NomGrupo'])?></td>
				<td><?=($field['NomSubGrupo'])?></td>
