...
if ($field['FlagCommodity'] == "S") {
	...
	$sql = "SELECT
			...
			WHERE tt.CodOperacion = '".$_PARAMETRO["TRANSRECEPCC"]."'";
} else {
	//	tipo de transaccion
	$sql = "SELECT
				tt.CodTransaccion,
			...
			FROM
				...
				INNER JOIN lg_tipodocumento td ON (tt.TipoDocGenerado = td.CodDocumento)
			WHERE tt.CodTransaccion = '".$_PARAMETRO["TRANSRECEPCCA"]."'";
}
...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Recepci&oacute;n de Caja Chica</td>
    ...
    <tr>
        <td>
			...
			if ($ActivoFijo) {
				...
				<li id="li2" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 2, 3);">Item/Commodities</a></li>
				<li id="li3" onclick="currentTab('tab', this);"><a href="#" onclick="setActivosFijosAsociados(false);">Activos Asociados</a></li>
				<?php
			} else {
				?>
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lg_transaccion_cajachica_lista" method="POST" onsubmit="return transaccion_cajachica_recepcion(this);">
...
<table width="1000" class="tblForm">
	...
	<tr>
        ...
        <td>&nbsp;</td>
    	<td>
        	<input type="checkbox" name="FlagManual" id="FlagManual" value="S" checked="checked" onchange="setFlagManualAlmacen(this.checked);" /> Valorizaci&oacute;n Manual
        </td>
	</tr>
    <tr>
    ...
    <tr>
		...
		<td class="gallery clearfix">
			<input type="hidden" id="NomUbicacion" value="<?=$field['NomUbicacion']?>" />
			<input type="text" id="CodUbicacion" value="<?=$field['CodUbicacion']?>" style="width:60px;" class="disabled" readonly="readonly" />
	...
	foreach ($detalle as $linea) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
			...
			<td align="center">
            	...
            	<input type="text" name="CantidadRecibida" class="cell" style="text-align:right;" value="<?=number_format($_CantidadPendiente, 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form);" />
            </td>
			<td align="center">
            	<input type="text" name="PrecioUnit" class="cell" style="text-align:right;" value="<?=number_format($_PrecioUnit, 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form);" />
            </td>
			<td align="center">
            	<input type="text" name="Total" id="Total_<?=$nrodetalles?>" class="cell2" style="text-align:right;" value="<?=number_format($_Total, 2, ',', '.')?>" readonly="readonly" />
            </td>
			<td align="center">
            	<input type="hidden" name="CodCentroCosto" value="<?=$_CodCentroCosto?>" />
            	<input type="hidden" name="ReferenciaCodDocumento" value="OC" />
            	<input type="hidden" name="ReferenciaNroDocumento" value="<?=$_CodRequerimiento?>" />
            	<input type="hidden" name="ReferenciaNroInterno" value="<?=$field['CodInterno']?>" />
