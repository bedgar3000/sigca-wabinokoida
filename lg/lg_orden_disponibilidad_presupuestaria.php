<?php
$titulo = "Disponibilidad Presupuestaria de Ordenes";
$_width = 800;
?>
...
<table class="tblLista" style="width:100%; min-width:1450px;">
	...
	foreach ($detalle as $linea) {
		list($_cod_partida, $_CodCuenta, $_CodCuentaPub20, $_Monto, $_MontoAjustado, $_MontoCompromiso, $_PreCompromiso, $_CotizacionesAsignadas, $_MontoDisponible, $_MontoDisponibleReal, $_MontoPendiente, $_CodFuente, $_CategoriaProg) = split(";char:td;", $linea);
		##	descripcion
		...
		##	valido
		if ($Resta <= 0) $style = "style='background-color:#F8637D;'";
		elseif($RestaReal <= 0) $style = "style='background-color:#FFC;'";
		...
		<tr class="trListaBody" <?=$style?>>
			...
			<td align="right"><?=number_format($_PreCompromiso,2,',','.')?></td>
...
<table class="tblLista" style="width:100%; min-width:100%;">
	...
	$detalle = split(";char:tr;", $detalles_partida);
	foreach ($detalle as $linea) {
		list($_cod_partida, $_CodCuenta, $_CodCuentaPub20, $_Monto, $_MontoAjustado, $_MontoCompromiso, $_PreCompromiso, $_CotizacionesAsignadas, $_MontoDisponible, $_MontoDisponibleReal, $_MontoPendiente, $_CodFuente) = split(";char:td;", $linea);
		##	
		if ($_MontoPendiente) {
			$Descripcion = getVar3("SELECT denominacion FROM pv_partida WHERE cod_partida = '".$_cod_partida."'");
