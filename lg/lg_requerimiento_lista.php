...
elseif ($lista == "caja_chica") {
	$titulo = "Lista de Requerimientos (Caja Chica)";
	$btNuevo = "display:none;";
	$btModificar = "display:none;";
	$btModifRest = "display:none;";
	$btRevisar = "display:none;";
	$btConformar = "display:none;";
	$btAprobar = "display:none;";
	...
	$btCuadro = "display:none;";
	$fEstado = "AP";
	$filtro .= " AND (r.FlagCajaChica = 'S') ";
}
//	------------------------------------
if ($filtrar == "default") {
	...
	<tr>
		...
		<td>
			...
			elseif ($lista == "caja_chica") {
				...
				<input type="checkbox" onclick="this.checked=!this.checked;" checked="checked" />
                <select name="fEstado" id="fEstado" style="width:140px;">
                    <?=loadSelectValores("ESTADO-REQUERIMIENTO-CC", $fEstado)?>
                </select>
                <?php
			} else {
				?>
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			...
			<input type="button" value="Modif. Restringida" class="ver" style="width:100px; <?=$btModifRest?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_requerimiento_form&opcion=modificacion_restringida&origen=lg_requerimiento_lista', 'SELF', '', 'registro');" />
			...
			<input type="button" id="btImprimir" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="cargarOpcion2(this.form, 'lg_requerimiento_pdf.php?', 'BLANK', 'height=10000, width=10000, left=100, top=0, resizable=no', $('#registro').val());" />
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($grupo != $field['CodDependencia']) {
            ...
            <tr class="trListaBody2">
                <td colspan="6"><?=($field['Dependencia'])?></td>
            </tr>
            <?php
		}
		##
		...
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			...
			<td><?=htmlentities($field['NomClasificacion'])?></td>
			<td title="<?=htmlentities($field['Comentarios'])?>"><?=htmlentities($Comentarios)?></td>
			...
			<td><?=htmlentities($field['NomAlmacen'])?></td>
