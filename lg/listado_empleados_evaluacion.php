...
<form name="frmentrada" action="listado_empleados_evaluacion.php" method="POST" onsubmit="return false">
...
<center><input type="button" name="btBuscar" value="Buscar" onclick="filtroListadoEmpleadosEvaluacion(this.form, <?=$limit?>)"></center>
...
$sql="SELECT mp.Foto, mp.Estado AS EsReg, mp.CodPersona, mp.NomCompleto, mp.Ndocumento, me.CodEmpleado, me.Fingreso, me.Estado AS SitTra, md.Dependencia, rp.DescripCargo FROM mastpersonas mp INNER JOIN mastempleado me ON (mp.CodPersona=me.CodPersona) INNER JOIN rh_puestos rp ON (me.CodCargo=rp.CodCargo) LEFT OUTER JOIN mastdependencias md ON (me.CodDependencia=md.CodDependencia) WHERE (mp.EsEmpleado='S') $filtrado";
...
<table width="910" class="tblLista">
    ...
    if ($registros!=0) {
        ...
        $sql="SELECT mp.Foto, mp.Estado AS EsReg, mp.CodPersona, me.CodEmpleado, mp.NomCompleto, mp.Ndocumento, me.Fingreso, me.Estado AS SitTra, md.Dependencia, rp.DescripCargo FROM mastpersonas mp INNER JOIN mastempleado me ON (mp.CodPersona=me.CodPersona) INNER JOIN rh_puestos rp ON (me.CodCargo=rp.CodCargo) LEFT OUTER JOIN mastdependencias md ON (me.CodDependencia=md.CodDependencia) WHERE (mp.EsEmpleado='S') $filtrado ".$ordenar." LIMIT ".$limit.", $MAXLIMIT";
        $query=mysql_query($sql) or die ($sql.mysql_error());
        ...
        for ($i=0; $i<$rows; $i++) {
			...
			?>
            <tr class="trListaBody" onclick="mClk(this, 'registro'); selListadoEvaluacion('<?=$field["NomCompleto"]?>', '<?=$field["DescripCargo"]?>', '<?=$cod?>', '<?=$nom?>', '<?=$id?>', '<?=$field["CodPersona"]?>', '<?=$field["Foto"]?>');" id="<?=$field["CodEmpleado"]?>">
