...
elseif ($accion=="SET-TIPO-CONTRATOS") {
	...
	if (mysql_num_rows($query)!=0) {
		$field=mysql_fetch_array($query);
		echo "0|:|".$field["FlagVencimiento"]."|:|".$field["CodFormato"]."|:|".($field["Documento"]);
	}
}
...
elseif ($accion == "insertarEvaluacionNecesidad") {
	...
	echo "||";
	?>
    <td><input type="text" name="txtnecesidad_tab6" maxlength="100" value="<?=($field_necesidad['Necesidad'])?>" style="width:99%" /></td>
    <td>
    ...
    <td><input type="text" name="txtobjetivo_tab6" maxlength="100" value="<?=($field_necesidad['Objetivo'])?>" style="width:99%" /></td>
	<?php
}
...
elseif ($accion == "mostrarIncidentesCriticos") {
	...
	*/
	...
	while ($field = mysql_fetch_array($query)) { $nrodetalles++;
		...
		<tr class="trListaBody">
            <td>
            	<input type="hidden" name="txtfuncion_tab3" value="<?=($field['Descripcion'])?>" />
                <input type="hidden" id="txtcodcalificativo_tab3_<?=$nrodetalles?>" />
		...
		while ($field_desempeno = mysql_fetch_array($query_desempeno)) { $nrodetalles++;
			...
			if ($agrupa != $idagrupa) {			
				...
				<tr class="trListaBody2">
					<td align="center">&nbsp;</td>
					<td colspan="5"><strong><?=($field_desempeno["NomTipoCompetencia"]." - ".$field_desempeno['NomArea'])?></strong></td>
				</tr>
				<?php
				$agrupa = $idagrupa;
			}
			...
			<tr class="trListaBody" onclick="mClk(this, 'seldetalle_tab2');" id="tab2_<?=$field_desempeno['Competencia']?>">
				...
				<td>
					<input type="hidden" id="txtcodcalificativo_tab2_<?=$field_desempeno['Competencia']?>" value="<?=($field_desempeno["NomCalificacion"])?>" />
