...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Cell(75, 5, utf8_decode($field['CodAlmacen']), 0, 0, 'L');
		$this->Ln(6);
	...
	function Footer() {
		...
		$this->SetXY(130, 255); $this->Cell(65, 4, 'ALMACEN', 0, 0, 'C');
	}
}
//---------------------------------------------------
...
if ($field['CodDocumentoReferencia'] == "RQ") {
	$sql = "SELECT
				td.CodItem,
				td.Descripcion,
				td.CodUnidad,
			...
			FROM
				...
				INNER JOIN lg_requerimientosdet rd ON (rd.CodRequerimiento = td.ReferenciaNroDocumento AND
													   rd.Secuencia = td.ReferenciaSecuencia)
			WHERE
