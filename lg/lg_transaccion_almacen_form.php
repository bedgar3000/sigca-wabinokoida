...
if ($opcion == "nuevo") {
	...
	$field_transaccion['FechaDocumento'] = substr($Ahora, 0, 10);
	//	default
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lg_transaccion_almacen_lista" method="POST" onsubmit="return transaccion_almacen(this, '<?=$accion?>');">
...
<input type="hidden" id="CodDocumentoReversa" value="<?=$CodDocumento?>" />
<input type="hidden" id="NroDocumentoReversa" value="<?=$NroDocumento?>" />
...
<table width="1000" class="tblForm">
    ...
    <tr>
		...
		<td>
			<select id="CodDependencia" style="width:275px;" <?=$disabled_ver?> onChange="loadSelect($('#CodCentroCosto'), 'tabla=centro_costo&opcion='+this.value, 1);">
				<option value="">&nbsp;</option>
		...
		<td class="gallery clearfix">
			...
			<a href="../lib/listas/listado_tipo_transacciones.php?filtrar=default&cod=CodTransaccion&ventana=transaccion_almacen_sel&nom=NomTransaccion&iframe=true&width=950&height=525" rel="prettyPhoto[iframe1]" style=" <?=$display_modificar?>">
    ...
    <tr>
		...
		<td>
			<select id="CodAlmacen" style="width:200px;" <?=$disabled_ver?>>
				<?=loadSelectAlmacen($field_transaccion['CodAlmacen'], "N", 0)?>
			</select>
		</td>
	...
	<tr>
		...
		<td><input type="text" id="FechaDocumento" value="<?=formatFechaDMA($field_transaccion['FechaDocumento'])?>" maxlength="10" style="width:60px;" class="datepicker" onkeyup="setFechaDMA(this);" <?=$disabled_ver?> /></td>
	...
	<tr>
		...
		<td>
			<input type="text" id="DocumentoReferencia" maxlength="20" style="width:125px;" value="<?=$field_transaccion['DocumentoReferencia']?>" <?=$disabled_referencia?> />
		</td>
...
<table width="1000" class="tblBotones">
	<tr>
		<td align="right" class="gallery clearfix">
        	<a id="aItem" href="../lib/listas/listado_items.php?filtrar=default&ventana=almacen_detalles_insertar&iframe=true&width=950&height=525" rel="prettyPhoto[iframe3]" style="display:none;"></a>
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN lg_itemalmaceninv iai ON (iai.CodItem = td.CodItem AND
	...
	while($field_detalle = mysql_fetch_array($query_detalle)) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
			...
			<td align="center">
            	<input type="text" name="CantidadRecibida" class="cell" style="text-align:right;" value="<?=number_format($field_detalle['CantidadRecibida'], 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form);" <?=$disabled_item?> />
            </td>
			<td align="center">
            	<input type="text" name="PrecioUnit" class="cell PrecioUnit" style="text-align:right;" value="<?=number_format($field_detalle['PrecioUnit'], 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form);" <?=$dPrecioUnit?> />
            </td>
			<td align="center">
            	<input type="text" name="Total" id="Total_<?=$nrodetalles?>" class="cell2" style="text-align:right;" value="<?=number_format($field_detalle['Total'], 2, ',', '.')?>" readonly="readonly" />
            </td>
			<td align="center">
            	<input type="hidden" name="CodCentroCosto" value="<?=$field_detalle['CodCentroCosto']?>" />
            	<input type="hidden" name="ReferenciaCodDocumento" value="<?=$field_detalle['ReferenciaCodDocumento']?>" />
            	<input type="hidden" name="ReferenciaNroDocumento" value="<?=$field_detalle['ReferenciaNroDocumento']?>" />
            	<input type="hidden" name="ReferenciaSecuencia" value="<?=$field_detalle['ReferenciaSecuencia']?>" />
                <?=$field_detalle['ReferenciaCodDocumento']?>-<?=$field_detalle['ReferenciaNroDocumento']?>-<?=$field_detalle['ReferenciaSecuencia']?>
			</td>
		</tr>
		<?php
	}
