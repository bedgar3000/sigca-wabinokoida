...
foreach ($detalle as $linea) {
	list($_Linea, $CommoditySub, $Descripcion, $CodUnidad, $CantidadPedida, $CantidadRecibida, $CantidadPendiente, $PrecioUnit, $FlagExonerado, $CodCentroCosto, $CodClasificacion, $CantidadCompra, $CodUnidadCompra, $PrecioUnitCompra, $Clasificacion) = split(";char:td;", $linea);
	if ($Clasificacion == "ACT" || $Clasificacion == "BME") {
		$ActivoFijo = true;
...
$sql = "SELECT CodDependencia FROM ac_mastcentrocosto WHERE CodCentroCosto = '".$CodCentroCosto."'";
$query_dependencia = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
...
<table width="1000" class="tblForm">
    ...
    <tr>
		...
		<td>
			<select id="CodDependencia" style="width:275px;">
				<?=loadSelect("mastdependencias", "CodDependencia", "Dependencia", $field_dependencia['CodDependencia'], 1)?>
			</select>
		</td>
    ...
    <tr>
		...
		<td>
			<select id="CodCentroCosto" style="width:275px;">
				<?=loadSelect("ac_mastcentrocosto", "CodCentroCosto", "Descripcion", $CodCentroCosto, 1)?>
			</select>
		</td>
    ...
    <tr>
		...
		<td>
			<input type="text" id="NotaEntrega" maxlength="10" style="width:125px;" />
		</td>
        <td>&nbsp;</td>
        <td>
	...
	foreach ($detalle as $linea) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
			...
			<td align="center">
            	...
            	<input type="text" name="CantidadRecibida" class="cell" style="text-align:right;" value="<?=number_format($CantidadPendiente, 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form); setActivosFijosAsociados(false);" />
            </td>
			<td align="center">
			...
			<td align="center">
            	...
            	<input type="hidden" name="ReferenciaNroDocumento" value="<?=$_NroOrden?>" />
