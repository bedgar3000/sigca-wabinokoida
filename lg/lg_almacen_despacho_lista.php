...
<input type="hidden" name="CodRequerimiento" id="CodRequerimiento" />
...
<table width="1050" class="tblBotones">
	<tr>
		<td align="right" class="gallery clearfix">
			...
			<input type="button" id="btDirigirCompras" value="Dirigir a Compras para Reposición" onclick="opcionRegistro2(this.form, $('#CodRequerimiento').val(), 'almacen', 'dirigir-compras');" />
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			ORDER BY Dependencia, CodDependencia, Anio, CodInterno, CodItem, CommoditySub";
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($grupo != $field['CodDependencia']) {
            ...
            <tr class="trListaBody2">
                <td colspan="4"><?=htmlentities($field['Dependencia'])?></td>
            </tr>
            <?php
		}
		##
		?>
		<tr class="trListaBody" onclick="mClk(this, 'CodRequerimiento'); appendAjax('accion=almacen_despacho_detalles&CodRequerimiento='+$('#CodRequerimiento').val(), $('#lista_detalles'));" id="<?=$field['CodRequerimiento']?>">
            ...
            <td title="<?=$TitleComentarios?>"><?=htmlentities($Comentarios)?></td>
