...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
            ...
            <input type="button" value="Modif. Restringida" style="width:100px; <?=$btModifRest?>" class="ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_orden_compra_form&opcion=modificacion_restringida&origen=lg_orden_compra_lista', 'SELF', '', 'registro');" />
            ...
            <input type="button" id="btImprimir" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="cargarOpcion2(this.form, 'lg_orden_compra_pdf.php?', 'BLANK', 'height=900, width=1000, left=100, top=0, resizable=no', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="2300" class="tblLista">
	...
	while ($field = mysql_fetch_array($query)) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			...
			<td align="right"><?=number_format($field['MontoPagado'], 2, ',', '.')?></td>
			<td align="right"><?=number_format($field['MontoPendiente'], 2, ',', '.')?></td>
			<td><?=htmlentities($field['NomAlmacen'])?></td>
			<td><?=htmlentities($field['NomAlmacenIngreso'])?></td>
