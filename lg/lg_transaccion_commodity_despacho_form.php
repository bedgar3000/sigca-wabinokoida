...
foreach ($detalle as $linea) {
	...
	//	obtengo los comentarios
	$sql = "SELECT Comentarios FROM lg_requerimientos WHERE CodRequerimiento = '".$CodRequerimiento."'";	
	$query_comentarios = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
...
$field_requerimiento['CodDependencia'] = $field_dependencia['CodDependencia'];
$field_requerimiento['CodCentroCosto'] = $field_dependencia['CodCentroCosto'];
$field_requerimiento['CodInterno'] = $field_dependencia['CodInterno'];
?>
	...
	foreach ($detalle as $linea) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
			...
			<td align="center">
            	...
            	<input type="text" name="CantidadRecibida" class="cell" style="text-align:right;" value="<?=number_format($CantidadPendiente, 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" />
            </td>
			<td align="center">
			...
			<td align="center">
            	...
            	<input type="hidden" name="ReferenciaNroInterno" value="<?=$CodInterno?>" />
