...
<table width="800" class="tblLista">
	...
	$sql = "SELECT 
				s.*,
				f.Descripcion AS NomFamilia,
				l.Descripcion AS NomLinea
			FROM 
				lg_clasesubfamilia s
				INNER JOIN lg_clasefamilia f ON (s.CodFamilia = f.CodFamilia AND s.CodLinea = f.CodLinea)
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($agrupa2 != $field['CodFamilia']) {
			$agrupa2 = $field['CodFamilia'];
			?>
			<tr class="trListaBody3">
				<td align="center"><?=$field['CodFamilia']?></td>
				<td colspan="2"><?=($field['NomFamilia'])?></td>
			</tr>
			<?php
		}
			
		?>
		<tr class="trListaBody" id="<?=$field['CodLinea']?>|<?=$field['CodFamilia']?>|<?=$field['CodSubFamilia']?>" onclick="mClk(this, 'registro'); selSubFamilia('<?=$campo1?>', '<?=$campo2?>', '<?=$campo3?>', '<?=$field['NomLinea']?>', '<?=$field['NomFamilia']?>', '<?=$field['Descripcion']?>');">
