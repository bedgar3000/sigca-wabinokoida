...
elseif ($opcion == "modificar" || $opcion == "ejecutar" || $opcion == "reversa" || $opcion == "ver" || $opcion == "anular") {
	...
	elseif ($opcion == "anular") {
		$titulo = "Anular Transacci&oacute;n";
		$accion = "anular";
		$label_submit = "Anular";
		$disabled_ver = "disabled";
		$display_ver = "display:none;";
	...
	if ($field_transaccion['FlagManual'] != "S" || $opcion == "ejecutar" || $opcion == "anular" || $opcion == "ver") $dPrecioUnit = "disabled";
}
//	------------------------------------
?>
	...
	<table width="100%" class="tblLista">
		...
		while($field_detalle = mysql_fetch_array($query_detalle)) {
			...
			<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
				...
				<td align="center">
	            	<input type="text" name="CantidadRecibida" class="cell" style="text-align:right;" value="<?=number_format($field_detalle['Cantidad'], 2, ',', '.')?>" onBlur="numeroBlur(this);" onFocus="numeroFocus(this);" onchange="setMontosAlmacen(this.form);" <?=$disabled_item?> />
	            </td>
				<td align="center">
