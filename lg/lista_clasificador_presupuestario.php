...
<script>
function selPartidaCuentaItem(busqueda, cod, nom, codcuenta, nomcuenta) {
	var registro = document.getElementById("registro").value;
	opener.document.getElementById(cod).value = registro;
	opener.document.getElementById(nom).value = busqueda;
	opener.document.getElementById("ctagasto").value = codcuenta;
	opener.document.getElementById("nomctagasto").value = nomcuenta;
	window.close();
}
</script>
</head>
...
<table width="100%" class="tblLista">
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($destino == "selPartidaCuenta") {
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selPartidaCuenta('<?=($field["denominacion"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$field['CodCuenta']?>', '<?=($field["NomCuenta"])?>');" id="<?=$field['cod_partida']?>">
		...
		elseif ($destino == "selPartidaConceptoPerfil") {
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selPartidaConceptoPerfil('<?=($field["denominacion"])?>', '<?=$field['CodCuenta']?>', '<?=$field['TipoSaldo']?>');" id="<?=$field['cod_partida']?>">
				<td align="center"><span <?=$style?>><?=$field['cod_partida']?></span></td>
				<td><span <?=$style?>><?=($field['denominacion'])?></span></td>
				<td align="center"><span <?=$style?>><?=$status?></span></td>
			</tr>
		...
		else if ($destino == "selPartidaCuentaItem") {
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selPartidaCuentaItem('<?=($field["denominacion"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$field['CodCuenta']?>', '<?=($field["NomCuenta"])?>');" id="<?=$field['cod_partida']?>">
				<td align="center"><span <?=$style?>><?=$field['cod_partida']?></span></td>
				<td><span <?=$style?>><?=($field['denominacion'])?></span></td>
				<td align="center"><span <?=$style?>><?=$status?></span></td>
			</tr>
			...
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selListado('<?=($field["denominacion"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['cod_partida']?>">
				<td align="center"><span <?=$style?>><?=$field['cod_partida']?></span></td>
				<td><span <?=$style?>><?=($field['denominacion'])?></span></td>
				<td align="center"><span <?=$style?>><?=$status?></span></td>
			</tr>
