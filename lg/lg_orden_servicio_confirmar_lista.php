...
<table width="1500" class="tblLista">
	...
	while ($field_confirmar = mysql_fetch_array($query_confirmar)) {
		...
		if ($agrupa != $field_confirmar['NroInterno']) {
			$agrupa = $field_confirmar['NroInterno'];
			?>
            <tr class="trListaBody2">
                <td align="center">O/S</td>
                <td align="center"><?=$field_confirmar['NroInterno']?></td>
                <td colspan="9"><?=htmlentities($field_confirmar['NomProveedor'])?></td>
            </tr>
            <?php
		}
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			...
			<td><?=htmlentities($field_confirmar['Descripcion'])?></td>
...
<table width="1050" class="tblBotones">
	<tr>
		<td align="right">
			<input type="button" id="btDesConfirmar" value="Desconfirmar Servicio" onclick="opcionRegistro3(this.form, $('#registro').val(), 'orden_servicio', 'desconfirmar', 'lg_orden_servicio_ajax.php');" /> |
			<input type="button" id="btImprimir" value="Imprimir Acta" onclick="imprimirConfirmacion($('#registro').val());" />
		</td>
	</tr>
</table>
<div style="overflow:scroll; width:1050px; height:300px;">
<table width="1800" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN ap_documentosdetalle dd ON (d.CodProveedor = dd.CodProveedor AND
													   ...
													   d.DocumentoReferencia = dd.DocumentoReferencia AND
													   d.Anio = dd.Anio)
				INNER JOIN lg_ordenserviciodetalle osd ON (d.CodOrganismo = osd.CodOrganismo AND
														   d.ReferenciaNroDocumento = osd.NroOrden AND
														   dd.Secuencia = osd.Secuencia AND
														   dd.Anio = osd.Anio)
				INNER JOIN lg_ordenservicio os ON (os.Anio = osd.Anio AND 
												   os.CodOrganismo = osd.CodOrganismo AND 
												   os.NroOrden = osd.NroOrden)
	...
	while ($field_desconfirmar = mysql_fetch_array($query_desconfirmar)) {
		...
		if ($agrupa != $field_desconfirmar['NroInterno']) {
			$agrupa = $field_desconfirmar['NroInterno'];
			$i=1;
			?>
            <tr class="trListaBody2">
                <td align="center">O/S</td>
                <td align="center"><?=$field_desconfirmar['NroInterno']?></td>
                <td colspan="2"><?=htmlentities($field_desconfirmar['NomProveedor'])?></td>
            </tr>
            <?php
            //htmlentities($field_desconfirmar['Descripcion'])
		}
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			...
			<td><?=htmlentities($field_desconfirmar['Descripcion'])?></td>
			...
			<td><?=htmlentities($field_desconfirmar['NomConfirmadoPor'])?></td>
		</tr>
		<?php
	}
	?>
...
<script type="text/javascript" charset="utf-8">
	function imprimirConfirmacion(registro) {
		if (registro == "") cajaModal('Debe seleccionar un registro','error',350);
		else window.open('lg_orden_servicio_confirmar_pdf.php?registro='+registro, 'lg_orden_servicio_confirmar_pdf', 'toolbar=no, menubar=no, location=no, scrollbars=yes, width=1000, height=600');
	}
</script>