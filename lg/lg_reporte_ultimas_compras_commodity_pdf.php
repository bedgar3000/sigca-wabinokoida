...
##
if ($fCommoditySub != "") $filtro.=" AND (ocd.CommoditySub = '".$fCommoditySub."')";
//---------------------------------------------------
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Ln(5);
		$this->Cell(210, 5, utf8_decode('Últimas Compras Realizadas (Commodity)'), 0, 1, 'C', 0);
		$this->Ln(5);
		##
		...
		$this->Ln(1);
		$this->SetFont('Arial', '', 8);
		$this->SetWidths(array(20,12,90,38,10,15,25));
		$this->SetAligns(array('C','C','L','R','C','R','L'));
	}
...
$sql = "SELECT
		...
		FROM
			...
			INNER JOIN lg_commoditysub i ON (i.Codigo = ocd.CommoditySub)
		WHERE 1 $filtro
