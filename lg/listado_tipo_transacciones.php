...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			WHERE 1 $where 
			ORDER BY tt.TipoMovimiento, tt.CodTransaccion";
	...
	for ($i=0; $i<$rows; $i++) {
		...
		if ($ventana == "selListadoAgregarTransaccion") {
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selListadoAgregarTransaccion('<?=($field["Descripcion"])?>', '<?=$cod?>', '<?=$nom?>', '<?=$field["TipoMovimiento"]?>', '<?=$field["TipoDocGenerado"]?>', '<?=($field["DocGenerado"])?>', '<?=$field["TipoDocTransaccion"]?>');" id="<?=$field['CodTransaccion']?>">
			...
			?>
			<tr class="trListaBody" onclick="mClk(this, 'registro'); selListado('<?=($field["Descripcion"])?>', '<?=$cod?>', '<?=$nom?>');" id="<?=$field['CodTransaccion']?>">
                <td align="center"><?=$field['CodTransaccion']?></td>
                <td><?=($field['Descripcion'])?></td>
                <td align='center'><?=$flagconsumo?></td>
                <td align='center'><?=$flagajuste?></td>
