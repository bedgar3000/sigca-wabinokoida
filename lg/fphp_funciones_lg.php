...
elseif ($accion == "guardarInvitacionProveedores") {
	...
	foreach ($invitacion as $proveedor) {
		...
		$cotizacion_numero = getCodigo_2("lg_cotizacion", "Numerocotizacion", 8);
...
elseif ($accion == "insertarItemOrden") {
    ...
    <td><input type="text" name="txtdescripcion" value="<?=($field['Descripcion'])?>" style="width:98%" /></td>
		...
		$sql = "(SELECT 
				 ...
				 FROM
					...
					LEFT JOIN pv_presupuesto pvp ON (pvp.Organismo = pvpd.Organismo AND
													 ...
													 pvp.Ejercicio = SUBSTRING(do.Periodo, 1, 4))
				 WHERE
					...
					p.cod_partida NOT IN (SELECT 
										  ...
										  FROM
												...
												LEFT JOIN pv_presupuesto pvp1 ON (pvp1.Organismo = pvpd1.Organismo AND
																				  ...
																				  pvp1.Ejercicio = SUBSTRING(do1.Periodo, 1, 4))
										  WHERE
...
elseif ($accion == "insertarItemOrdenServicio") {
    ...
    <td><input type="text" name="txtobs" value="<?=($descripcion)?>" style="width:98%" /></td>	
	<?php
	echo "||$idrow||";
...
elseif ($accion == "insertarItemRequerimiento") {
    ...
    <td align="center"><?=$codigo_mostrar?></td>
    <td><input type="text" name="txtdescripcion" value="<?=($field['Descripcion'])?>" style="width:98%" <?=$ddesc?> /></td>
    <td align="center"><input type="hidden" name="txtcodunidad" value="<?=$field['CodUnidad']?>" /><?=$field['CodUnidad']?></td>
    <td>
    ...
    <td align="center"><?=("En Preparación")?></td>
...
elseif ($accion == "setLineasActivos") {
	...
	foreach ($detalle as $registro) {
        ...
        <tr class="trListaBody2" id="<?=$idactivogroup?>">
            ...
            <td colspan="13"><?=($field['Descripcion'])?></td>
		</tr>
		<?php
...
//	--------------------------
elseif ($accion == "cargarOpcion_ReversaTransaccion") {
	connect();
	...
	if (mysql_num_rows($query) != 0) {
		$field = mysql_fetch_array($query);
		if ($field['Estado'] == "AN") echo "¡ERROR: Transacción ya se encuentra anulada!";
		elseif ($field['CodTransaccion'] != "ROC" && $field['CodTransaccion'] != "REQ") echo "¡ERROR: Por los momentos solo puede anular Recepción de O/C y Despacho de Requisiciones!";
