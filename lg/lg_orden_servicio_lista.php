...
if ($fBuscar != "") { 
	...
	$filtro.=" AND (os.NroInterno LIKE '%".$fBuscar."%' OR 
					...
					os.Observaciones LIKE '%".utf8_decode($fBuscar)."%' OR
					os.Anio LIKE '%".utf8_decode($fBuscar)."%')";
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			...
			<input type="button" value="Modif. Restringida" style="width:100px; <?=$btModifRest?>" class="ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_orden_servicio_form&opcion=modificacion_restringida&origen=lg_orden_servicio_lista', 'SELF', '', 'registro');" />
			...
			<input type="button" id="btImprimir" value="Imprimir" style="width:75px; <?=$btImprimir?>" onclick="cargarOpcion2(this.form, 'lg_orden_servicio_pdf.php?', 'BLANK', 'height=900, width=1000, left=100, top=0, resizable=no', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="3100" class="tblLista">
    ...
    <tr>
		...
		<th width="600" align="left">Descripci&oacute;n</th>
		<th width="75">Fecha Anulaci&oacute;n</th>
		<th width="600" align="left">Descripci&oacute;n Adicional</th>
		<th width="400" align="left">Raz&oacute;n Rechazo</th>
	...
	while ($field = mysql_fetch_array($query)) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$id?>">
			...
			<td align="center"><?=printValoresGeneral("ESTADO-SERVICIO", $field['Estado'])?></td>
			<td><?=($field['NomProveedor'])?></td>
			<td align="right"><strong><?=number_format($field['TotalMontoIva'], 2, ',', '.')?></strong></td>
			<td title="<?=$field['Descripcion']?>"><?=htmlentities($Descripcion)?></td>
			<td align="center"><?=formatFechaDMA($field['FechaAnulacion'])?></td>
			<td title="<?=$field['DescAdicional']?>"><?=htmlentities($DescAdicional)?></td>
			<td title="<?=$field['MotRechazo']?>"><?=htmlentities($MotRechazo)?></td>
			<td title="<?=$field['Observaciones']?>"><?=htmlentities($Observaciones)?></td>
		</tr>
		<?php
	}
	?>
