...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Baja de Activos | Generar Acta</td>
...
echo"<form name='frmentrada' id='frmentrada' action='af_bajactivosgeneraracta.php?limit=0' method='POST'>
...
//$v = substr($valor,0,4); echo '-///////////'.$v;
  ...
  $sa= "select 
		  ...
		  from 
		      af_transaccionbaja 
         where 
              FlagActaGenerada<>'S' and Organismo<>'' $filtro
         order by 
              Activo"; //echo $sa;
...
<table width="1000" class="tblLista">
 <tr> <input type="hidden" id="registro" name="registro"/>
  ...
  <td align="right"></td>
  <td align="right">
    <input type="button"  id="btMasivo" name="btMasivo" class="btLista01" value="Generar Acta Masiva" onclick="cargarMasivoBajaActivo(this.form,'GenerarActaBajaMasiva');"/>
    <input type="button" name="btActa" id="btActa" class="btLista01" value="Generar Acta" onclick="cargarBajaActivoGenerarActa(this.form, 'GenerarActaBaja')"/>
  </tr>
</table>
		...
		<table width="200%" class="tblLista" border="0">
		  ...
		  if($ra!=0){
		   ...
		   for($i=0;$i<$ra;$i++){
		     ...
		     $id= $fa['Activo']."|".$fa['Organismo']."|".$fa['CodTransaccionBaja']."|".$fa['Dependencia'];
