...
if($origen=='lista'){
      list($Organismo, $MovimientoNumero, $Estado) = split('[,]',$registro);
      global $MovimientoNumero, $Organismo, $origen, $Estado;  //echo $Activo.'//';
}
...
function Header(){
	  ...
	  global $origen; //echo $origen.'**';
	  if($origen=='lista'){
         list($Organismo, $MovimientoNumero, $Estado) = split('[,]',$registro);
        global $MovimientoNumero, $Organismo, $Estado;
		//echo $Estado, $Organismo,$MovimientoNumero;
      }
	  global $MovimientoNumero, $Organismo; //echo $Activo.'/*/';
	  //echo $Organismo;
	...
	$this->SetXY(20, 30); $this->Cell(20, 4, utf8_decode('Movimiento#:'), 0, 0, 'L');/// NRO DE DOCUMENTO
   ...
   $this->Ln(10);
    ...
    $this->Ln();
    $this->SetFont('Arial','B','8');
	$this->SetXY(20,34);$this->Cell(20, 5,utf8_decode('Comentario: '), 0, 0, 'L');
	$this->SetFont('Arial','','8');
	$this->Cell(20, 5,utf8_decode($field['Comentario']), 0, 1, 'L');
	
	
    $this->SetFont('Arial','B','8');
	$this->SetXY(20,40); $this->Cell(60, 5, utf8_decode('Items a ser transferidos: '), 0, 0, 'L');
	...
	$this->SetXY(20, 44); $this->Cell(20, 3, utf8_decode('#'), 0, 0, 'C');
...
$scon = "select 
		    ...
		    where 
			    a.MovimientoNumero='".$MovimientoNumero."' and
				a.Organismo = '".$Organismo."'";  //echo $scon;
...
$pdf->SetFont('Arial', 'B', 8);
	 $pdf->SetXY(21, 48); $pdf->Cell(20, 5,$fcon['MovimientoNumero'].'  '.$fcon['Activo'], 0, 0, 'L');
	 $pdf->SetXY(70, 48); $pdf->Cell(20, 5,$fcon['DescpActivo'], 0, 1, 'L');
     
	 $pdf->SetXY(20, 54); $pdf->Cell(80, 5, 'Anterior', 0, 0 , 'C');
	 					  $pdf->Cell(80, 5, 'Actual', 0, 1, 'C');
$pdf->SetFont('Arial', '', 6);
     $pdf->SetXY(20, 57); $pdf->Cell(15, 5, 'C.Costos:', 0, 0, 'L');
	 					  $pdf->Cell(15, 5, $fcon['CentroCostoAnterior'], 0, 0, 'L');
						  $pdf->Cell(65, 5, strtoupper(utf8_decode($cc_anterior)), 0, 0, 'L');
						  $pdf->Cell(12, 5, $fcon['CentroCosto'], 0, 0, 'L');
						  $pdf->Cell(10, 5, strtoupper(utf8_decode($cc_actual)), 0, 1, 'L'); 
	 
	 $pdf->SetXY(20, 61); $pdf->Cell(15, 5, utf8_decode('Ubicación:'), 0, 0, 'L'); 
	 					  $pdf->Cell(15, 5, $fcon['UbicacionAnterior'], 0, 0, 'L');
						  $pdf->Cell(65, 5, strtoupper(utf8_decode($ubicacion_anterior)), 0, 0, 'L');
						  $pdf->Cell(12, 5, $fcon['Ubicacion'], 0, 0, 'L');
						  $pdf->Cell(10, 5, strtoupper(utf8_decode($ubicacion_actual)), 0, 1, 'L'); 
	 	 
	 $pdf->SetXY(20, 65); $pdf->Cell(15, 5, utf8_decode('Dependencia:'), 0, 0, 'L'); 
	 ...
	 $pdf->SetXY(20, 69); $pdf->Cell(19, 5, utf8_decode('Emp. Usuario:'), 0, 0, 'L');
	 ...
	 $pdf->SetXY(20, 74); $pdf->Cell(19, 5, utf8_decode('Emp. Responsable:'), 0, 0, 'L'); 
	 ...
	 $pdf->SetFont('Arial', 'B', 8);
	 $pdf->SetXY(20, 79); $pdf->Cell(10, 5, utf8_decode('ACTIVOS RELACIONADOS : '), 0, 1, 'L'); 
