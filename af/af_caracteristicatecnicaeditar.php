...
<? 
 ...
 $sql = "select * from af_caracteristicatecnica where CodCaractTecnica = '".$_POST['registro']."'";
...
<tr>
 ...
 <td><input type="text" name="descp_local"  id="descp_local" size="33" maxlength="30" value="<?=$field['DescripcionLocal']?>"/>*</td>
</tr>
<tr>
 <td></td>
</tr>
<tr>
 ...
 <td><input type="hidden" id="radioEstado" name="radioEstado" value="<?=$field['Estado'];?>"/> 
     <? if($field['Estado']=='A'){?>
           <input type="radio" name="radio1" checked="checked" onclick="estadosPosee(this.form);"/>Activo
     ...
     <? }else{?>
          <input type="radio" name="radio1"  onclick="estadosPosee(this.form);"/>Activo
           <input type="radio" name="radio2" checked="checked" onclick="estadosPosee(this.form);"/>Inactivo
      <? }?></td>
</tr>
