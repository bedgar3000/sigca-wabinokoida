...
if($_POST['filtro']!="") $sql="select * from mastpersonas where (CodPersona LIKE '%".$_POST['filtro']."%' OR Busqueda LIKE '%".$_POST['filtro']."%' OR NomCompleto LIKE '%".$_POST['filtro']."%' OR Direccion LIKE '%".$_POST['filtro']."%') and EsProveedor = 'S'"; else $sql="SELECT * FROM mastpersonas where EsProveedor = 'S'";
...
<thead>
	...
	if ($registros!=0) {
		...
		for ($i=0; $i<$rows; $i++) {
			...
			}elseif($ventana==insertProveedor){
