...
<form name="frmentrada" id="frmentrada" action="af_tipomovimientoactivo.php" method="POST">
<table width="825px" class="tblBotones">
	<tr>
		...
		<td align="right">
			...
			<input name="btVer" type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcionClasfActivos(this.form, document.getElementById('seleccion').value, 'af_tipomovimientoactivo_ver.php', 'BLANK', 'height=250, width=750, left=250, top=200, resizable=no');" />
...
<thead>
	...
	for ($i=0; $i<$rows; $i++) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'seleccion');" id="<?=$field['CodTipoMovimiento']?>">
			...
			<td><span <?=$style?>><?=utf8_encode($field['DescpMovimiento']);?></span></td>
