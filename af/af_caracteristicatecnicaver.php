...
<? 
 ...
 $sql = "select * from af_caracteristicatecnica where CodCaractTecnica = '".$_GET['registro']."'";
 $qry = mysql_query($sql) or die ($sql.mysql_error());
 $field = mysql_fetch_array($qry);
 echo"<input type='hidden' name='regresar' id='regresar' value='".$regresar."' />";
?>

<form name="frmentrada" id="frmentrada" action="af_caracteristicatecnica.php" method="POST" onsubmit="return  editarCaracteristicaTecnica(this);">
<? 
...
<tr>
 ...
 <td width="393"><input type="text" name="cod_caractTecnica"  id="cod_caractTecnica" size="8" maxlength="4" value="<?=$field['CodCaractTecnica'];?>" readonly/>*</td>
</tr>
<tr>
 <td class="tagForm" colspan="2">Descripci&oacute;n Local:</td>
 <td><input type="text" name="descp_local"  id="descp_local" size="33" maxlength="30" value="<?=$field['DescripcionLocal']?>" readonly/>*</td>
</tr>
<tr>
 <td></td>
</tr>
<tr>
 ...
 <td><input type="hidden" id="radioEstado" name="radioEstado" value="<?=$field['Estado'];?>"/> 
     <? if($field['Estado']=='A'){?>
           <input type="radio" name="radio1" checked="checked" onclick="estadosPosee(this.form);" disabled/>Activo
           <input type="radio" name="radio2" onclick="estadosPosee(this.form);"/>Inactivo
     <? }else{?>
          <input type="radio" name="radio1"  onclick="estadosPosee(this.form);"/>Activo
           <input type="radio" name="radio2" checked="checked" onclick="estadosPosee(this.form);" disabled/>Inactivo
      <? }?></td>
</tr>
...
<tr>
  ...
  <center>
    ...
    <input name="cancelar" type="button" id="cancelar" value="Cancelar" onClick="cargarPagina(this.form, 'af_caracteristicatecnica.php');" disabled/>
  </center><br />
</form>
