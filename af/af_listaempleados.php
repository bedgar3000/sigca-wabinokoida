...
?>
<form name="frmlista" id="frmlista" method="post" action="af_listaempleados.php?limit=0&amp;campo=<?=$campo?>">
      ...
      (mastempleado.CodCargo=rh_puestos.CodCargo) ORDER BY mastempleado.CodPersona LIMIT ".$_GET['limit'].", $MAXLIMIT";
		...
		for ($i=0; $i<$rows; $i++) {
			...
			if($ventana==insertarDestinatarioEmp){
			 echo "
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); insertarDestinatarioEmp(this.id,\"".$ventana."\");' id='".$field['CodPersona']."'>
			...
			elseif($ventana=="SelEmpleadoUsuario"){
			  echo "
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); selCentralActivoFijo(\"".$field['Busqueda']."\", ".$_GET['campo'].", \"".$field['NomCompleto']."\", \"".$field['CodCargo']."\");' id='".$field['CodPersona']."'>
				<td align='center'>".$field['CodEmpleado']."</td>
