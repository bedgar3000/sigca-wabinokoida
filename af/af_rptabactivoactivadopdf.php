...
function Header(){
	...
	$this->Row(array('Voucher', '#Documento', utf8_decode('Fecha Factura'), 'Activo', utf8_decode('Cód. Interno'), utf8_decode('Descripción'), 
		             'Orden Compra', 'Serie', utf8_decode('Ubicación Física'), 'Monto Local', utf8_decode('Período Ingreso'), 
		              utf8_decode('Período Deprec.'), utf8_decode('Período Inflación')));
	//$this->SetFillColor(255, 255, 255);
...
if($r_con_01!=0)
   for($i=0; $i<$r_con_01; $i++){
	  ...
	  if($f_categoria['CodCategoria']!=$categoria_capturada){
	    ...
	    $pdf->SetFont('Arial', '', 7);  //$pdf->Cell(20,4,$f_con_01['VoucherIngreso'], 0,0,'C'); se omite para mostrar VoucherIngPub20
	    								$pdf->Cell(20,4,$f_con_01['VoucherIngPub20'], 0,0,'C');
		 								$pdf->Cell(20,4,$f_con_01['FacturaTipoDocumento'].$obligacion.$f_con_01['FacturaNumeroDocumento'], 0,0,'C');
										$pdf->Cell(20,4,$fecha_factura, 0,0,'C');
										$pdf->Cell(20,4,$f_con_01['Activo'], 0,0,'C');
										$pdf->Cell(20,4,$f_con_01['CodigoInterno'], 0,0,'C');
