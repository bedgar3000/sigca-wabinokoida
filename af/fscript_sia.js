		...
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				...
				if (datos[0] != 0) alert (datos[0]);
					...
					if (confirm("¿Desea ingresar las partidas presupuestarias para esta cuenta?"))
						cargarPagina(form, "plan_cuentas_editar.php?registro="+datos[1]);
...
function selPartidaConceptoPerfil(busqueda, codcuenta, tipo) {
	var cod = document.getElementById("cod").value; alert(tipo);
	
	var registro=document.getElementById("registro").value;
	opener.document.getElementById("partida_"+cod).value=registro;
	if (tipo == "A") opener.document.getElementById("debe_"+cod).value=codcuenta;
	else if (tipo == "D")opener.document.getElementById("haber_"+cod).value=codcuenta;
	window.close();
}