...
{ $fecha2 = " ";}
		...
		{$fecha2 = "fecha_desincorporacion >='$fecha_ini' AND fecha_desincorporacion<='$fecha_fin'AND ";} 
		 $sql2 = "SELECT * FROM bienes WHERE clasificacion='$clasificacion' AND $fecha2 ubicacion='$ubicacion' $seccion AND estatus!='FALTANTE' AND estatus!='INCORPORADO' ORDER BY codigo_bien ASC ";
			 
	 
	  $sql = "SELECT * FROM bienes WHERE clasificacion='$clasificacion' AND $fecha ubicacion='$ubicacion' $seccion AND estatus!='FALTANTE'  AND estatus!='DESINCORPORADO' ORDER BY codigo_bien ASC ";
