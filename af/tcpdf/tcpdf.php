...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		 ...
		 * @since 3.0.015 (2008-06-06)
		 */
		public function setPageFormat($format, $orientation='L') {
			//Page format
			if (is_string($format)) {
		 ...
		 */
		public function SetMargins($left, $top, $right=6, $keepmargins=false) {
		...
		public function Header() {
			...
			$this->SetX($header_x);
			$this->Cell(0, $cell_height, $headerdata['title'], 0, 0, '', 0, '', 0);
			// header string
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
			$this->SetX($header_x);
			$this->MultiCell(0, $cell_height, $headerdata['string'], 0, '', 0, 0, '0', '0', true, 0, false);
				...
				if ($c == 10) { // 10 = "\n" = new line
					...
					if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) > $wmax)) ) {
						...
						if ($sep == -1) {
							...
							$this->Cell($w, $h, $shy_char_left.$tmpstr.$shy_char_right, 0, 0, $align, $fill, $link, $stretch);
							unset($tmpstr);
							if ($firstline) {
