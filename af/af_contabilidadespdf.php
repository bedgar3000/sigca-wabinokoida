...
function Header(){
	...
	$this->Ln(6); 
	$this->SetFont('Arial', 'B', 9);
	$this->Cell(180, 3, utf8_decode('Maestro de Contabilidad'), 0, 1, 'C');$this->Ln(4);
	
	$this->SetDrawColor(0, 0, 0); $this->SetFillColor(200, 200, 200); $this->SetTextColor(0, 0, 0);
	$this->SetFont('Arial', 'B', 9);
	$this->Cell(15, 5);
	$this->Cell(25, 5, 'Contabilidad', 1, 0, 'C', 1);
...
$pdf->SetFont('Times','',12);
 ...
 $sc = "select * from ac_contabilidades "; //echo $sc;
...
if($rc!=0){
  for($i=0; $i<$rc; $i++){
	...
	$pdf->Cell(15, 5); $pdf->Row(array($fc['CodContabilidad'], utf8_decode($fc['Descripcion']), $estado));
  }
}
