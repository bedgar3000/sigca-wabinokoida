...
if(!$_POST){ $fOrganismo = $_SESSION["FILTRO_ORGANISMO_ACTUAL"]; $fDependencia=$_SESSION['DEPENDENCIA_ACTUAL']; $fSituacionActivo="IF";}
if(!$_POST) $fEstado="AP";
...
if($fbuscar!="")$filtro.=" AND (Activo='".$fbuscar."' OR CodigoInterno='".$fbuscar."' OR CodigoBarras='".$fbuscar."' OR Descripcion='".$fbuscar."')";
if($fSituacionActivo!=""){$filtro.=" AND (SituacionActivo='".$fSituacionActivo."')"; $cSituacion = "checked";}else $dSituacion = "disabled";
if($fEstado !=""){$filtro .="AND (Estado = '".$fEstado."')"; $cEstado = "checked"; }else $dEstado ="disabled"; 

echo"<form name='frmentrada' id='frmentrada' action='af_selectoractivos.php?limit=0&campo=$campo&cierre=$cierre&ventana=$ventana' method='POST'>
...
<input type='hidden' name='can_detalle' id='can_detalle' value='".$can_detalle."'/> 
<input type='hidden' name='sel_detalle' id='sel_detalle' value='".$sel_detalle."'/>
...
<tr>
   ...
   <td align='left'>
	 ...
	 getDependencias($fDependencia,$_SESSION['ORGANISMO_ACTUAL'],3 );
	 echo"
	 </select>
   </td>
...
<tr>   
   ...
   <td><input type='text' id='fbuscar' name='fbuscar' value='$fbuscar'/></td>
...
<tr>   
   ...
   <td><input type='checkbox' id='checkSituacionActivo' name='checkSituacionActivo' value='1' $cSituacion onclick='enabledSituacionActivo(this.form);'/>
       <select id='fSituacionActivo' name='fSituacionActivo' class='selectMed' $dSituacion> 
	   <option value=''></option>";
...
?>
<form id="tabs" name="tabs" method="post" action="af_selectoractivos.php?limit=0&amp;campo=<?=$campo;?>&ventana=<?=$ventana;?>">
...
}elseif($ventana=="SelActivoMovimiento"){
   echo"<tr class='trListaBody' onclick='mClk(this, \"registro\"); selCentralActivoFijo(\"".$field['Busqueda']."\", ".$_GET['campo'].", \"".$fa['Descripcion']."\", \"".$fa['CodigoInterno']."\" , \"".$fa['CentroCosto']."\", \"".$f_consultas['DescripCentroCosto']."\", \"".$f_consultas['CodUbicacion']."\", \"".$f_consultas['DescripUbicacion']."\", \"".$f_consultas['4']."\", \"".$f_consultas['5']."\", \"".$f_consultas['6']."\", \"".$f_consultas['7']."\", \"".$f_consultas['10']."\", \"".$f_consultas['11']."\", \"".$f_consultas['8']."\", \"".$f_consultas['9']."\" );' id='".$fa['Activo']."'>";
}
echo"
