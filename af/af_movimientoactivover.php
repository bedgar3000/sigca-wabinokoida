...
<body>
<?
list($organismo, $num_movimiento, $estado, $NroActa, $anio) = split('[|]', $_GET['registro']);
//// CONSULTA PRINCIPAL
///echo $num_orden,$num_movimiento;
$sql = "select * from af_movimientos where Organismo='$organismo' and MovimientoNumero='$num_movimiento' and Anio='$anio' and Estado='$estado'";
...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Movimientos de Activos | Ver</td>
		<td align="right"><a class="cerrar" href="" onclick="window.close();">[cerrar]</a></td>
	</tr>
</table>
<hr width="100%" color="#333333" />

<form id="frmentrada" name="frmentrada"  method="POST"  onsubmit="return guardarRevisarMovimientoActivo(this)">
<table class='tblForm' width='1000' height='50'>
<?
 $sql_b = "select * from usuarios where usuario='".$_SESSION['USUARIO_ACTUAL']."'";
 $qry_b = mysql_query($sql_b) or die ($sql_b.mysql_error());
 $field_b = mysql_fetch_array($qry_b);
?>
<input type="hidden" id="estado" name="estado" value="<?=$estado;?>" size="20" disabled/>
<input type="hidden" id="anio" name="anio" value="<?=$anio;?>"/>
<input type="hidden" id="revisadopor" name="revisadopor" value="<?=$field_b['CodPersona'];?>" size="20" disabled/>
<tr>
   <td>
   <table>
   <tr>
       ...
       <td><textarea id='comentario' name='comentario'  style=" text-justify:auto; width:250px; height:35px" disabled><?=$field['Comentario'];?></textarea></td>	   
   </tr>
   <tr>
     <td width='5'></td>
	...
	?>
	 ...
	 <td><? if($field['InternoExternoFlag']=I){ ?> 
         <input type='radio' id='radio1' name='radio1' checked disabled/>Interno
         <input type='radio' id='radio2' name='radio2'  disabled/>Externo
         <? }else{?>
         <input type='radio' id='radio1' name='radio1' disabled/>Interno
	     <input type='radio' id='radio2' name='radio2' checked disabled/>Externo
         <? } ?></td>
    <td></td>
   </tr>
   ...
   <tr>
	...
	?>
     ...
     <? if($field['InternoExternoFlag']='I'){?>
        <select id="motivoTrasladoInterno" name="motivoTrasladoInterno" class="selectMed" style="display:block;" disabled>
          <option value=""></option>
          <?
     ...
     <? }else{?>
        <select id="motivoTrasladoExterno" name="motivoTrasladoExterno" class="selectMed" style="display:block;" disabled>
          <option value=""></option>
          <?
...
<tr>
 ...
 <td>Seleccionar <input type="button" id="btactivo" name="btactivo" value="Activo" onclick="cargarVentanaLista(this.form, 'af_selectoractivos.php?limit=0&campo=1','height=500, width=870, left=200, top=100, resizable=yes');" disabled/><input type="button" id="centro_costos" name="centro_costos" value="Centro de Costos" onclick="cargarVentanaLista(this.form, 'af_listacentroscostos.php?limit=0&campo=10','height=500, width=870, left=200, top=100, resizable=yes');" disabled/><input type="button" id="btubicacion" name="btubicacion" value="Ubicaci&oacute;n" onclick="cargarVentanaLista(this.form, 'af_listaubicacionesactivo?limit=0&campo=11','height=500, width=870, left=200, top=100, resizable=yes');" disabled/><input type="button" id="btDependencia" name="btDependencia" value="Dependencia" onclick="cargarVentanaLista(this.form, 'af_listadependencias.php?limit=0&campo=14','height=500, width=870, left=200, top=100, resizable=yes');" disabled/> <input type="button" id="btEmpleadoUsuario" name="btEmpleadoUsuario" value="Empl. Usuario" onclick="cargarVentanaLista(this.form, 'af_listaempleados.php?limit=0&campo=12', 'height=500, width=870, left=200, top=100, resizable=yes');" disabled/><input type="button" id="btEmpleadoResponsable" name="btEmpleadoResponsable" value="Empl. Respons." onclick="cargarVentanaLista(this.form, 'af_listaempleados.php?limit=0&campo=13', 'width=870, height=500, top=100, left=200, resizable=yes');" disabled/></td>
 </tr>
</table>
</td>
</tr>
<tr>
  <td align='center'>Ultima Modif.:<input type='text' id='ultimo_usuario' name='ultimo_usuario' value="<?=$field['UltimoUsuario'];?>" size='25' disabled/><input type='text' id='ultima_fecha' value="<?=$field['UltimaFechaModif'];?>" disabled/></td>
</tr>
</table>
...
<tr>
  ...
  <tr>
<td colspan="4"><input type="text" name="nro_movimientos" id="nro_movimientos" style="border-color:#666; text-align:right" size="18" value="<?=$cont;?>" disabled/><input type="text" id="activo" name="activo" style="border-color:#666; text-align:right" size="60" value='<?=$f_consulta['Activo'];?>' disabled/><input type="text" id="descripcion" name="descripcion" style="border-color:#666" size="88" value="<?=$f_consulta['Descripcion'];?>" disabled/> Cod.Interno <input type="text" id="cod_bar" name="cod_bar" size="30" style="border-color:#666; text-align:right" disabled value="<?=$f_consulta['CodigoInterno'];?>"/></td>
</tr>
  <tr>
 ...
 <td>Empl. Usuario</td><td colspan="3"><input type="text" id="e_usuario" name="e_usuario" size="6" value="<?=$f_consulta['emp_Anterior'];?>" disabled/><input type="text" id="e_usuario2" name="e_usuario2" size="89" value="<?=htmlentities($f_consulta['nom_empAnterior']);?>" disabled/><input type="text" id="e_usuarioActual" name="e_usuarioActual" size="6" value="<?=$f_consulta['emp_Actual'];?>" disabled/><input type="text" id="e_usuarioActual2" name="e_usuarioActual2" size="89" value="<?=htmlentities($f_consulta['nom_empActual']);?>" disabled/></td>
  ...
  <tr>
 <td>Empl. Respons.</td><td colspan="3"><input type="text" id="e_responsable" name="e_responsable" size="6" value="<?=$f_consulta['empresp_Anterior'];?>" disabled/><input type="text" id="e_responsable2" name="e_responsable2" size="89" value="<?=htmlentities($f_consulta['nom_resAnterior']);?>" disabled/><input type="text" id="e_responsableActual" name="e_responsableActual" value="<?=$f_consulta['empresp_Actual'];?>" size="6" disabled/><input type="text" id="e_responsableActual2" name="e_responsableActual2" size="89" value="<?=htmlentities($f_consulta['nom_resActual']);?>" disabled/></td>
</tr>
  <tr>
 <td>Organismo</td><td colspan="3"><input type="text" id="organismo" name="organismo" size="6" value="<?=$f_consulta['orgAnterior'];?>" disabled/><input type="text" id="organismo2" name="organismo2" size="89" value="<?=$f_consulta['nomorgAnterior'];?>" disabled/><select id="organismoActual" name="organismoActual"  disabled>
                                 <option value=""></option>
                                 <?
  ...
  <select id="mot_trasl_des" name="mot_trasl_des" onchange="cargarMotTraslDes(this.form)" style="width:47px;" disabled>
    <?
  ...
  <select id="mot_trasl_incorp" name="mot_trasl_incorp" onchange="cargarMotTraslIncorp(this.form)" style="width:47px;" disabled>
   <?
      $sql_a = "select * from af_tipomovimientos ";
	  $qry_a = mysql_query($sql_a) or die ($sql_a.mysql_error());
	  $row_a = mysql_num_rows($qry_a);
...
<center>
<? if($revisar=='revisar'){ ?>
<input type="submit" id="guardar" name="guardar" value="Revisado"/>
<?
}else{
?>
 <input type="submit" id="guardar" name="guardar" value="Guardar Registro" disabled/>
 <input type="button" id="cancelar" name="cancelar" value="Cancelar" onClick="cargarPagina(this.form, 'af_movimientoactivos.php');" disabled/>
<? }?>
</center>
</form>
