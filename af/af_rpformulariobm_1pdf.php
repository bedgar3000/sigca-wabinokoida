...
class PDF extends FPDF{
	...
	function Header(){
		...
		//$cadena=strtoupper($f_cons['DescpCentroCosto']);
		$cadena= strtoupper($f_cons['DescpCentroCosto']);
		...
		$this->SetXY(10,22);$this->SetFont('Arial', 'B', 8);
							...
							$this->SetFont('Arial', '', 8);	$this->Cell(20, 3, utf8_decode($cadena), 0, 1, 'L');
							
		$this->SetXY(10,25);$this->SetFont('Arial', 'B', 8);
							$this->Cell(45, 3, 'UNIDAD DE TRABAJO O AREA:', 0, 0, 'L');
							$this->SetFont('Arial', '', 8);$this->Cell(100, 3, utf8_decode($f_cons['Dependencia']), 0, 1, 'L');
		...
		$this->Cell(25, 14, utf8_decode('N° Identificación'), 1, 0, 'C');
		$this->Cell(110, 14, utf8_decode('Nombre y Descripción de los Elementos'), 1, 0, 'C');
...
if($ractivo!=0)
   for($i=0; $i<$ractivo; $i++){
	  ...
	  $pdf->SetFont('Arial', '', 9);
	  $pdf->SetWidths(array(16,16,16,15,25,110,30,30));
