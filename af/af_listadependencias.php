...
?>
<form name="frmlista" id="frmlista" method="post" action="af_listadependencias.php?limit=0&ventana=<?=$ventana;?>&campo=<?=$campo;?>&tabla=item">
...
<center>
<div style="overflow:scroll; width:750px; height:350px;">
	...
	if ($registros!=0) {
		...
		if ($_POST['filtro']!="") $sql="SELECT 
										 ...
										 FROM 
										    mastdependencias 
									    WHERE 
										   CodDependencia LIKE '%".$_POST['filtro']."%' OR
										   Dependencia LIKE '%".$_POST['filtro']."%'
									ORDER BY 
									       CodDependencia LIMIT ".$_GET['limit'].", $MAXLIMIT";
		else $sql="SELECT
		...
		for ($i=0; $i<$rows; $i++) {
			...
			}elseif($ventana=="SelDependenciaActivo"){
			  echo "
				<tr class='trListaBody' onclick='mClk(this, \"registro\"); selCentralActivoFijo(\"".$field['Busqueda']."\", ".$_GET['campo'].", \"".$field['Dependencia']."\");' id='".$field['CodDependencia']."'>
			    	<td align='center'>".$field['CodDependencia']."</td>
					<td align='left'>".$field['Dependencia']."</td>
				</tr>";
