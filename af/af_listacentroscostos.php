...
if($_POST['filtro']!="") $sql="select 
								 ...
								 from 
									  ...
									  INNER JOIN ac_subgrupocentrocosto sgcc ON (mcc.CodSubGrupoCentroCosto = sgcc.CodSubGrupoCentroCosto 
									                                         AND mcc.CodGrupoCentroCosto = sgcc.CodGrupoCentroCosto)
				                      INNER JOIN ac_grupocentrocosto gcc ON (mcc.CodGrupoCentroCosto = gcc.CodGrupoCentroCosto)
							    WHERE 
								     (mcc.CodCentroCosto LIKE '%".$filtro."%' OR mcc.Descripcion LIKE '%".$filtro."%' OR sgcc.Descripcion LIKE '%".$filtro."%' OR gcc.Descripcion LIKE '%".$filtro."%')"; else $sql="SELECT * FROM ac_mastcentrocosto";
...
<?
  echo"<input type='hidden' name='sel_detalle' id='sel_detalle' value='".$sel_detalle."'/>";
?>
 ...
 <thead>
	...
	if ($filtro != ""){ $sql = "select 
								 ...
								 from 
				                      ...
				                      INNER JOIN ac_grupocentrocosto gcc ON (mcc.CodGrupoCentroCosto = gcc.CodGrupoCentroCosto)
							    WHERE 
								     (mcc.CodCentroCosto LIKE '%".$filtro."%' OR mcc.Descripcion LIKE '%".$filtro."%' OR sgcc.Descripcion LIKE '%".$filtro."%' OR gcc.Descripcion LIKE '%".$filtro."%')";
	}else $sql = "SELECT
				mcc.*,
				sgcc.Descripcion AS NomSubGrupo,
				gcc.Descripcion AS NomGrupo
	...
	$rows = mysql_num_rows($query);
		...
		for ($i=0; $i<$rows; $i++) {
			...
			}elseif($ventana=="selCentroCosto"){
			  if($field['Estado']=='A') $estado = Activo; else $estado = Inactivo;
			    echo "
			    <tr class='trListaBody' onclick='mClk(this, \"registro\"); selCentralActivoFijo(\"".$field['Busqueda']."\", ".$_GET['campo'].", \"".$field['Descripcion']."\");' id='".$field['CodCentroCosto']."'>";
				?>
