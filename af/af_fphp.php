...
function loadSelect($tabla, $campo1, $campo2, $codigo, $opt) {
	switch ($opt) {
		...
		case 10:
			...
			$query = mysql_query($sql) or die ($sql.mysql_error());
			while ($field = mysql_fetch_array($query)) {
				if ($field[0] == $codigo) { ?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[0].' - '.$field[1])?></option><? }
				else { ?><option value="<?=$field[0]?>"><?=htmlentities($field[0].' - '.$field[1])?></option><? }
			}
			break;
		...
		case 11:
			...
			$query = mysql_query($sql) or die ($sql.mysql_error());
			while ($field = mysql_fetch_array($query)) {
				?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[0].' - '.$field[1])?></option><?
			}
			break;
	}
}
...
/// ------------------------------------------------------------
/// CONDICIONES
/*if($accion=="cargarSeelctCodClasificacion20"){
 connect(); 
  $sql = "select * from af_clasificacionactivo20 where Nivel = '".$_POST['valor']."'"; //echo $sql;
  ...
  if($row!=0){
     for($i=0;$i<$row;$i++){
		...
		echo"<option value='".$field['CodClasificacion']."'>".$field['CodClasificacion']."-".$field['Descripcion']."</option>";
	 }
  }
}*/
function getValorNivel($valorNivel, $opt){
connect();	
 switch ($opt) {
	case 0:
		$sql = "select * from af_clasificacionactivo20 where Nivel = '$valorNivel'"; echo "<option selected>$sql</option>";
		$qry = mysql_query($sql) or die ($sql.mysql_error());	
		$row = mysql_num_rows($qry);
		for($i=0;$i<$row;$i++){
			 echo "<option value='".$field['CodClasificacion']."' selected>".$field['CodClasificacion'].'-'.$field['Descripcion']."</option>";
		}
		break;
	}
}
/// ------------------------------------------------------------
/// CARGAR SELECTOR PARA CLASIFICACION DE ACTIVO PUBLICACION 20
if ($_POST['accion']=="activarVisible") {
	if ($_POST['tabla']=="codigo2") {
		echo "
		<select name='codigo2' id='codigo2' class='selectMed'>
			<option value=''>";
				getCodigo2("", $_POST['nivel'], 0);
		echo "</select>*";
	}
}
/// ------------------------------------------------------------
function getCodigo2($codigo2, $nivel, $opt){
connect();
switch ($opt) {
	case 0:
	    $nivel = $_POST['nivel'] - 1; //echo "<option selected>$nivel</option>";
		$sql="SELECT 
		           CodClasificacion, 
				   Descripcion 
			   FROM 
			  ...
			  WHERE 
			       Nivel='$nivel' ORDER BY Nivel, CodClasificacion"; //echo "<option selected>$sql</option>";
		$query=mysql_query($sql) or die ($sql.mysql_error());
		$rows=mysql_num_rows($query);
		for ($i=0; $i<$rows; $i++) {
			$field=mysql_fetch_array($query);
			if ($field[0]==$codigo2) echo "<option value='".$field[0].'-'.$field[1]."' selected>".htmlentities($field[1])."</option>"; 
			else echo "<option value='".$field[0].'-'.$field[1]."'>".$field[0]."-".htmlentities($field[1])."</option>";
		}
		break;
}
}
