...
<thead>
	...
	if ($registros!=0) {
		...
		for ($i=0; $i<$rows; $i++) {
			...
			}elseif($ventana=="SelUbicacionesActivo"){
			  if($field['Estado']=='A') $estado = Activo; else $estado = Inactivo;
				echo "
				<tr class='trListaBody' onclick='mClk(this, \"registro\"); selCentralActivoFijo(\"".$field['Busqueda']."\", ".$campo.", \"".$field['Descripcion']."\");' id='".$field['CodUbicacion']."'>
