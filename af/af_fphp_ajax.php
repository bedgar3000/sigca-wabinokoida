...
if($accion=='ELIMINARREGISTROS'){
  ...
  elseif($modulo=='eliminarBajaActivo'){
	 
	 list($Activo, $Organismo, $CodTransaccionBaja) = split('[|]', $codigo);
	 
	 $s_a = "select * from af_transaccionbaja where Activo='".$Activo."' and Organismo='".$Organismo."' and CodTransaccionBaja='".$CodTransaccionBaja."'";
	 $q_a = mysql_query($s_a) or die ($s_a.mysql_error());
	 $r_a = mysql_num_rows($s_a); if($r_a != 0) $f_a = mysql_fetch_array($q_a);
	 
	 if($f_a['Estado']=='PR'){
	   
	   $sql = "delete from af_transaccionbaja where Activo='".$Activo."' and Organismo='".$Organismo."' and CodTransaccionBaja='".$CodTransaccionBaja."'";
	   $qry = mysql_query($sql) or die ($sql.mysql_error()); 
	 }else{
	   echo " El Registro no puede ser modificado ";
	 }
  }
}
//// ----------------------------------------------------------------------------
...
//// ----------------------------------------------------------------------------
if($accion=='cargarMotTraslDes'){
 $sql = "select * from af_tipomovimientos  where CodTipoMovimiento = '".$mot_trasl_des."'";
 $qry = mysql_query($sql) or die ($sql.mysql_error());
 $row = mysql_num_rows($qry);
 
 if($row!=0){
   $field = mysql_fetch_array($qry);
   $DescpTipoMovimiento = $field['DescpMovimiento'];
   echo $DescpTipoMovimiento;  
 }
}
//// ----------------------------------------------------------------------------
////       CARGAR NOMBRE DEL TIPO DE MOVIMIENDO DE ACTIVOS INCORPORACION
//// ----------------------------------------------------------------------------
if($accion=='cargarMotTraslIncorp'){
 $sql = "select * from af_tipomovimientos  where CodTipoMovimiento = '".$mot_trasl_incorp."'";
 $qry = mysql_query($sql) or die ($sql.mysql_error());
 $row = mysql_num_rows($qry);
...
//// ----------------------------------------------------------------------------
if($Modulo=='TipoMovimientoActivo'){
if($accion=='NUEVO'){
  ...
  if($r_con!=0) echo"¡EL CODIGO INTRODUCIDO YA EXISTE!";
    ...
    $s_insert = "insert into af_tipomovimientos(CodTipoMovimiento, TipoMovimiento,
												DescpMovimiento, Estado,
												UltimoUsuario, UltimaFechaModif, FlagMovimiento, 
												FlagIncorporacion, FlagDesincorporacion) 
										  values('".$_POST['codigo']."', '".$_POST['t_movimiento']."',
												 '".$_POST['descripcion']."', '".$_POST['status']."',
												 '".$_SESSION['USUARIO_ACTUAL']."', '".date("Y-m-d H:i:s")."', '".$FlagMovimiento."', 
												 '".$FlagIncorporacion."', '".$FlagDesincorporacion."')";
	$q_insert = mysql_query($s_insert) or die ($s_insert.mysql_error());
  }
}
elseif($accion=='EDITAR'){
 $sql_a = "update af_tipomovimientos set TipoMovimiento = '".$t_movimiento."', 
 										 DescpMovimiento = '".$descripcion."',
										 Estado = '".$status."',
										 FlagMovimiento = '".$FlagMovimiento."',
										 FlagIncorporacion = '".$FlagIncorporacion."',
										 FlagDesincorporacion = '".$FlagDesincorporacion."'  
									where 
									     CodTipoMovimiento='".$codigo."'";
