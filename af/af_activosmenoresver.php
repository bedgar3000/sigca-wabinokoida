...
<tr>
  ...
  <td colspan="3"><textarea id="descripcionLocal" name="descripcionLocal" cols="145" rows="4" disabled><?=$field['Descripcion']; ?></textarea><!--<input type="button" id="btCargar" name="btCargar" value="..." onclick="cargarVentanaLista(this.form, 'af_listactivoslogistica.php?limit=0&campo=2','height=500, width=820, left=200, top=100, resizable=yes');"/>--></td>
  <td></td>
</tr>
<tr>
  <td class="tagForm">Descrip. Corta:</td>
  <td><input type="text" name="descripcionCorta" id="descripcionCorta" size="148" value="<?=$field['DescpCorta'];?>" disabled/></td>
</tr>
</table>

<table class="tblForm" width="900">
<tr>
   <td width="170" class="tagForm">Situaci&oacute;n Activo:</td>
   <td>
...
<tr>
 ...
 <td>
    <select id="select_categoria" style="width:75px;" disabled>
   <?
   ...
   ?>
     ...
     <input type="text" id="categoria" name="categoria" size="51" value="<?=$Descp;?>" disabled/></td>
</tr>
<tr>
<td class="tagForm"></td>
   <td></td>
   ...
   if($r_c20!=0) $f_c20 = mysql_fetch_array($q_c20);
	...
	?>
   <td><input type="hidden" id="clasificacion20" name="clasificacion20" disabled value="<?=$f_c20['CodClasificacion'];?>"/>
       <input type="text" id="clasificacion20Descp" name="clasificacion20Descp" size="67" value="<?=htmlentities($f_c20['Descripcion']);?>" disabled/></td>
...
<tr>
   ...
   ?>
   <td><input type="text" id="clasificacion" name="clasificacion" style="width:70px;" value="<?=$f_clactivo['CodClasificacion'];?>" disabled="true"/>
   ...
   <?
    $s_resp = "select 
	                  mtper1.CodPersona as CodPersonaResp, 
        					  ...
        					  mtper3.NomCompleto as NomCompletoUsuario,
        					  mtper3.CodPersona as CodPersonaUsuario
        				 from 
        				    af_activo afact 
        					  ...
        					  inner join mastpersonas mtper3 on (mtper3.CodPersona = afact.EmpleadoUsuario)
        				where 
        				      Activo ='".$field['Activo']."'";
...
<tr>
 ...
 <td class="tagForm" width="156">Empleado Usuario:</td>
   <td><input type="hidden" name="cod_usuario" id="cod_usuario" value="<?=$f_resp['CodPersonaUsuario'];?>" /><input type="text" id="nomb_usuario" name="nomb_usuario" size="67" value="<?=$f_resp['NomCompletoUsuario'];?>" disabled/></td>
   <td class="gallery clearfix"><input type="hidden" name="btEmpleado" id="btEmpleado" value="..." onclick="cargarVentanaLista(this.form,'af_listaempleados.php?limit=0&campo=7','height=500, width=800,left=200,top=100,resizable=yes');" disabled/></td>
</tr>
...
<input type="text" name="ultima_fecha" value="<?=$field['UltimaFechaModif'];?>" style="text-align:center;" size="20" disabled/></td></tr>
</table>
</div>
...
<tr>
  ...
  <td width="183">
    ...
    <select id="fabricante" name="fabricante" class="selectMed" disabled/>
       <option></option>
       <?
        $scon2 = "select * from lg_marcas where CodMarca='".$field['Marca']."'";
        $qcon2= mysql_query($scon2) or die ($scon2.mysql_error()); 
        $rcon2 = mysql_num_rows($qcon2); 
        for($i=0;$i<$rcon2;$i++){
           $fcon2= mysql_fetch_array($qcon2);
           echo"<option value='".$fcon2['CodMarca']."' selected>".htmlentities($fcon2['Descripcion'])."</option>";
        }
       ?>
    </select>
  </td>
  ...
  <td width="361" colspan="2">
      <input type="text" id="proveedor" name="proveedor" size="4" value="<?=$f_resp['CodPersonaProveedor'];?>" disabled/> 
      <input type="text" id="nomb_proveedor" name="nomb_proveedor" size="50" value="<?=$f_resp['NomCompletoProveedor'];?>" disabled/>
...
<tr>
  ...
  <td><input type="text" name="modelo" id="modelo" size="40" value="<?=$field['Modelo'];?>" disabled/></td>
  <td></td>

  <td class="tagForm">Oligaci&oacute;n:</td>
  <td colspan="2">
     <select id="obligacionTipoDocumento" name="obligacionTipoDocumento" style="width:130px;" disabled>
        <option value=""></option>
        <?
        ...
        while($f_facnumerodoc = mysql_fetch_array($q_facnumerodoc)){
            if($f_facnumerodoc['CodTipoDocumento']==$field['ObligacionTipoDocumento']){  
     ...
     <input type="text" id="numObligacion" name="numObligacion" size="20" value="<?=$field['ObligacionNroDocumento'];?>" style="text-align:right" disabled/>
        <? 
           if($field['ObligacionFechaDocumento']!=""){ 
             list($a,$b,$c)= split('[-]',$field['ObligacionFechaDocumento']); 
             $obligacionfdoc= $c.'-'.$b.'-'.$a; 
           }
        ?>
     <input type="text" id="fechaObligacion" name="fechaObligacion" size="8" value="<?=$obligacionfdoc;?>" disabled/></td>
</tr>

<tr>
  <td class="tagForm">N&uacute;mero de Serie:</td>
  <td><input type="text" name="nro_serie" id="nro_serie" size="40" value="<?=$field['NumeroSerie'];?>" disabled/></td>
  <td></td>
  ...
  <td class="tagForm">Nro. Factura:</td>
   ...
   ?>
   <td><input type="text" id="nrofactura" name="nrofactura" size="25" value="<?=$field['NroFactura'];?>" style="text-align:right" disabled/>
       <input type="text" id="factura_fecha" name="factura_fecha" size="8" value="<?=$FacturaFecha;?>" disabled/></td>
</tr>
...
<tr>
  ...
  <td></td>
   ...
   ?>
  <td><input type="text" id="orden_compra" name="orden_compra" size="25" value="<?=$field['NumeroOrden']?>" maxlength="15" disabled/> 
      <input type="text" id="fecha_ordencompra" name="fecha_ordencompra" size="8" maxlength="10" value="<?=$fechaOrdenCompra;?>" disabled/></td>
</tr>

<tr>
  <td class="tagForm">C&oacute;digo de Barras:</td>
  <td><input type="text" name="cod_barras" id="cod_barras" size="40" value="<?=$field['CodigoBarras'];?>" style="text-align:right;" disabled/></td>
  <td></td>
  ...
  ?>
  <td><input type="text" id="nro_guiaremision" name="nro_guiaremision" size="25" value="<?=$field['NumeroGuia'];?>" maxlength="15" disabled/> 
      <input type="text" id="fecha_guiaremision" name="fecha_guiaremision" size="8" maxlength="10" value="<?=$fechaguiaremision;?>" disabled/></td>
</tr>
...
<tr>
 ...
 <td><input type="text" name="periodo_registro" id="periodo_registro" size="8" value="<?=$field['PeriodoIngreso'];?>" style="text-align:center;" disabled/></td>
</tr>
</table>
</div>
<!-- ****************************************************** COMIENZO TAB3 ************************************************ -->
