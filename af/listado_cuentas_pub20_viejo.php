...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Listado de Plan de Cuentas Pub20</td>
		<td align="right"><a class="cerrar" href="#" onclick="javascript:window.close();">[cerrar]</a></td>
	</tr>
</table><hr width="100%" color="#333333" />

<form name="frmentrada" id="frmentrada" action="listado_cuentas_pub20.php?limit=<?=$limit;?>&campo=<?=$campo;?>&destino=<?=$destino;?>" method="POST">
...
<thead>
	...
	if ($filtro != "") "select 
						  ...
						  from 
						       ac_masplancuenta 
						  WHERE 
						        CodCuenta LIKE '%".$filtro."%' OR 
								Descripcion LIKE '%".$filtro."%' OR 
								Tipocuenta LIKE '%".$filtro."%' OR 
								Naturaleza LIKE '%".$filtro."%')"; 
	else $sql = "SELECT
				ampc.*,
				mmd.Descripcion AS NomTipoCuenta
			FROM
				ac_mastplancuenta20 ampc
				INNER JOIN mastmiscelaneosdet mmd ON (ampc.TipoCuenta = mmd.CodDetalle AND mmd.CodMaestro = 'CUENTPUB20')
			$filtro 
			WHERE FlagTipo='A'
			ORDER BY 
			     ampc.CodCuenta";
	...
	for ($i=0; $i<$rows; $i++) {
		...
		$sql = "SELECT * FROM ac_mastplancuenta20 WHERE CodCuenta LIKE '".$field['CodCuenta']."%' AND CodCuenta <> '".$field['CodCuenta']."'";
		...
		if (mysql_num_rows($query_sub) == 0 && strlen($field['CodCuenta']) >= 3) {
			...
			elseif ($destino == "listadoCuentaContable") {
				...
				?>
				<tr class="trListaBody" onclick="mClk(this, 'registro'); selOperacion('<?=htmlentities($field["Descripcion"])?>', '<?=$_GET['campo']?>', '<?=$nom?>','<?=$_GET['nrodetalle'];?>');" id="<?=$field['CodCuenta']?>">
					<td><?=$field['CodCuenta']?></td>
					<td><?=utf8_encode($field['Descripcion'])?></td>
