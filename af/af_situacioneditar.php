...
<table width="100%" cellspacing="0" cellpadding="0">
 <tr>
  <td class="titulo">Maestro Situaci&oacute;n de Activo | Editar Registro</td>
  <td align="right">
...
<?
 $sql="SELECT * FROM af_situacionactivo WHERE CodSituActivo='".$_POST['registro']."'";
...
<table width="700" class="tblForm">
  ...
  <tr>
    ...
    <td><input type="text" name="descp_situactivo"  id="descp_situactivo" size="60" maxlength="100" value="<?=$field['Descripcion']?>" />*</td>
  </tr>
  <tr>
   <td>
    ...
    <td><?
        if($field['DepreciacionFlag']=='S'){?>
          <input type="checkbox" name="proceso_situactivo"  id="proceso_situactivo" value="S" checked/>
	    <? }else{?>
           <input type="checkbox" name="proceso_situactivo"  id="proceso_situactivo" value="S"/>
        <? }?>
     </td>
    ...
    <td><?
        if($field['DepreciacionFlag']=='S'){?>
           <input type="checkbox" name="proceso_ajuste"  id="proceso_ajuste" value="S" checked />
         <? }else{?>
           <input type="checkbox" name="proceso_ajuste"  id="proceso_ajuste" value="S"/>
        <? }?>
    </td>
   ...
   <tr>
	    ...
	    <td>
		 <?
		  ...
		  <? }else{?>
            <input name='status_situactivo' type='radio' value='A' /> Activo
