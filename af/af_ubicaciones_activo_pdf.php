...
function Header(){
	...
	$this->Ln(6); 
	$this->SetFont('Arial', 'B', 9);
	$this->Cell(200, 3, utf8_decode('Maestro Ubicaciones'), 0, 1, 'C');$this->Ln(4);
	
	$this->SetDrawColor(0, 0, 0); $this->SetFillColor(200, 200, 200); $this->SetTextColor(0, 0, 0);
	$this->SetFont('Arial', 'B', 8);
	$this->Cell(25, 5, utf8_decode('Cod. Ubicación'), 1, 0, 'C', 1);
	$this->Cell(100, 5, utf8_decode('Descripción'), 1, 0, 'C', 1);
	$this->Cell(20, 5, utf8_decode('Nivel'), 1, 0, 'C', 1);
	$this->Cell(25, 5, utf8_decode('Estado'), 1, 1, 'C', 1);
...
$pdf->SetFont('Times','',12);
 ...
 $sc = "select * from af_ubicaciones"; //echo $sc;
...
if($rc!=0){
  for($i=0; $i<$rc; $i++){
	...
	$pdf->SetFont('Arial', '', 8);
	$pdf->SetWidths(array(25, 100, 20, 25));
	$pdf->SetAligns(array('C', 'L', 'C', 'C'));
	$pdf->Row(array($fc['CodUbicacion'], utf8_decode($fc['Descripcion']), $fc['Nivel'], $estado));
  }
