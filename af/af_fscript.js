...
function cargarCampoCategoria(valor){ 
 var valorEnviado = document.getElementById(valor).value; 
 if(valorEnviado!=""){
    var ajax=nuevoAjax();
	ajax.open("POST", "af_fphp_ajax.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); 
	...
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			...
			if (resp != ""){ var respuesta = resp.split('-');
			                 document.getElementById("categoria").value = respuesta[0]; 
							}
		}
	}
 }else{
...
function selEmpleado(busqueda, campo, variable, otro1, otro2, otro3) { 
	...
	if(campo==22){
	 ...
	 if((campo!='18')&&(campo!='26')&&
		...
		(campo!='19')&&(campo!='17')&&
		(campo!='27')&&(campo!='16')&&
		(campo!='29')&&(campo!='30')&&(campo!='31')&&(campo!='32'))window.close();
...
function guardarCatastro(form) {
	...
	var descp_catastro = document.getElementById("descp_catastro").value.trim(); 
...
function quitarLineaCatastroEditar(seldetalle) {
	...
	if(eliminar){
		...
		var id_catanual= document.getElementById(seldetalle).id; 
		    listaDetalles.removeChild(tr);
	    document.getElementById("seldetalle").value = "";
		// ---------------------------------------------- //
		var cod_catastro = document.getElementById("cod_catastro").value.trim(); 
...
function guardarEditarTipoSeguro(formulario){
 var cod_tseguro = document.getElementById("cod_tseguro").value; 
 var descp_tseguro = document.getElementById("descp_tseguro").value.trim(); 
...
function estadosPosee02(form, valor){
 ...
 if((estado == 'I')&&(form.radio1.checked)&&(valor!="a")) {
	   form.radio1.checked = false; 
	   form.radio2.checked = true; 
	   document.getElementById('radioEstado').value = 'E' ; 
 }else{
...
function quitarLineaEditarContabilidades(seldetalle,form) {
	...
	if(eliminar){
		var listaDetalles = document.getElementById("listaDetalles");
	    var tr = document.getElementById(seldetalle);
		var id_contabilidades= document.getElementById(seldetalle).id; 
		    listaDetalles.removeChild(tr);
	    document.getElementById("seldetalle").value = "";
		// ---------------------------------------------- //
...
function selActivo(busqueda, campo, variable, otro1, otro2, otro3, otro4, otro5, otro6, otro7, otro8, otro9, otro10, otro11, otro12, otro13, otro14, otro15,otro16, otro17) { 
	...
	if(campo==2){
		...
		parent.document.getElementById("monto_local").value=otro16;
...
function selOperacion(busqueda, campo, variable, otro1, otro2, otro3, otro4, otro5, otro6, otro7) { 
	var registro=document.getElementById("registro").value; //alert('registro='+registro); alert('campo='+campo);
	if (campo==1){
	...
	if(campo==7){ 
	  opener.document.frmentrada.proveedor.value = registro ;
	  opener.document.frmentrada.nomb_proveedor.value = variable; 
	}
...
function crearPeriodo(form, id){
  ...
  if(mes=='12'){
	 ...
	 document.getElementById("periodo_registro").value = ano+"-0"+mes;  
     mes = mes + 1;
	 if((mes>="1") && (mes<="9")){
...
function guardarActivosTransferidos(form){ 
  ...
  /// Validación
  if(document.getElementById("codigo_interno").value==''){ alert('¡Debe Introducir el Codigo Interno¡'); return false;}
  else if(document.getElementById("clasificacion20").value==''){alert('¡Debe Seleccionar la Clasificacion20¡'); return false;}
  else if(document.getElementById("ubicacion").value==''){alert('¡Debe Seleccionar la Ubicación¡'); return false;}
  else if(document.getElementById("cod_usuario").value==''){alert('¡Debe Seleccionar el Empleado Usuario¡'); return false;}
  else if(document.getElementById("cod_empresponsable").value==''){alert('¡Debe Seleccionar el Empleado Responsable¡'); return false;}
  else if(document.getElementById("select_categoria").value==''){alert('¡Debe Seleccionar la Categoría¡'); return false;};
  
  if((document.getElementById("gen_voucher").checked==true)&&(document.getElementById("tipobaja").value=="")){ 
     alert('¡ Debe seleccionar el Tipo de Transacción ¡');
	 return false;
  }
  
  var anio = document.getElementById("anio").value;
  var nrointerorden = document.getElementById("nrointerno").value;
  ...
  var Descripcion = document.getElementById("descripcion").value;
  var PeriodoVoucher = document.getElementById("voucher_periodo").value;
  ...
  var LocalInicio = document.getElementById("h_inicioAnoLocal").value; LocalInicio = setNumero(LocalInicio); 
  ...
  var ObligacionNroDocumento = document.getElementById("numObligacion").value;
  var FechaObligacion = document.getElementById("fechaObligacion").value;
  
  var NumeroFactura = document.getElementById("nrofactura").value;
  var FacturaFecha = document.getElementById("factura_fecha").value; 
  // -------------------------------------
  var ajax=nuevoAjax();
	ajax.open("POST", "gmactivofijo.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=GuardarActivosTransferidos&CodOrganismo="+CodOrganismo+"&CodDependencia="+CodDependencia+"&Descripcion="+Descripcion+
		      "&TipoActivo="+TipoActivo+"&EstadoConserv="+EstadoConserv+"&CodigoBarras="+CodigoBarras+"&CodigoInterno="+CodigoInterno+
		      "&TipoSeguro="+TipoSeguro+"&TipoVehiculo="+TipoVehiculo+"&Categoria="+Categoria+"&Clasificacion="+Clasificacion+
		      "&ClasificacionPublic20="+ClasificacionPublic20+"&Ubicacion="+Ubicacion+"&TipoMejora="+TipoMejora+"&ActivoConsolidado="+ActivoConsolidado+
		      "&EmpleadoUsuario="+EmpleadoUsuario+"&EmpleadoResponsable="+EmpleadoResponsable+"&CentroCosto="+CentroCosto+"&Marca="+Marca+
		      "&Modelo="+Modelo+"&NumeroSerie="+NumeroSerie+"&NumeroSerieMotor="+NumeroSerieMotor+"&NumeroPlaca="+NumeroPlaca+"&MarcaMotor="+MarcaMotor+
		      "&NumeroAsiento="+NumeroAsiento+"&Material="+Material+"&Dimensiones="+Dimensiones+"&NumerodeParte="+NumerodeParte+"&Color="+Color+
		      "&FabricacionPais="+FabricacionPais+"&FabricacionAno="+FabricacionAno+"&PolizaSeguro="+PolizaSeguro+"&NumeroUnidades="+NumeroUnidades+
		      "&CodigoCatastro="+CodigoCatastro+"&AreaFisicaCatastro="+AreaFisicaCatastro+"&MontoCatastro="+MontoCatastro+
		      "&GenerarVoucherIngresoFlag="+GenerarVoucherIngresoFlag+"&CodProveedor="+CodProveedor+"&NumeroOrden="+NumeroOrden+
		      "&NumeroOrdenFecha="+NumeroOrdenFecha+"&NumeroGuia="+NumeroGuia+"&NumeroGuiaFecha="+NumeroGuiaFecha+"&NumeroDocAlmacen="+NumeroDocAlmacen+
		      ...
		      "&PeriodoInicioRevaluacion="+PeriodoInicioRevaluacion+"&PeriodoBaja="+PeriodoBaja+"&VoucherBaja="+VoucherBaja+"&MontoLocal="+MontoLocal+
		      "&MontoReferencia="+MontoReferencia+"&MontoMercado="+MontoMercado+"&VoucherIngreso="+VoucherIngreso+
		      "&FlagParaMantenimiento="+FlagParaMantenimiento+"&FlagParaOperaciones="+FlagParaOperaciones+"&SituacionActivo="+SituacionActivo+
		      "&PreparadoPor="+PreparadoPor+"&OrigenActivo="+OrigenActivo+"&UnidadMedida="+UnidadMedida+"&DepreEspecificaFlag="+DepreEspecificaFlag+
		      "&Naturaleza="+Naturaleza+"&Contabilidad="+Contabilidad+"&NroOrden="+NroOrden+"&Secuencia="+Secuencia+"&NroSecuencia="+NroSecuencia+
		      "&CodTipoMovimiento="+CodTipoMovimiento+"&Periodo="+Periodo+"&LocalInicio="+LocalInicio+"&LocalNeto="+LocalNeto+"&ALocalInicio="+ALocalInicio+
		      "&ALocalNeto="+ALocalNeto+"&LocalMesFinal="+LocalMesFinal+"&ALocalMesFinal="+ALocalMesFinal+"&TipoTransaccion="+TipoTransaccion+
		      "&Descripcion="+Descripcion+"&PeriodoVoucher="+PeriodoVoucher+"&anio="+anio+"&nrointerorden="+nrointerorden+
		      "&ObligacionTipoDocumento="+ObligacionTipoDocumento+"&ObligacionNroDocumento="+ObligacionNroDocumento+"&FechaObligacion="+FechaObligacion+
		      "&NumeroFactura="+NumeroFactura+"&FacturaFecha="+FacturaFecha);
...
function guardarActivosListaActivos(form){ 
  ...
  var ClasificacionPublic20 = document.getElementById("clasificacion20").value;
  var CodTipoMovimiento = document.getElementById("conceptoMovimiento").value; 
  var TipoTransaccion = document.getElementById("tipobaja").value; //Voucher Ingreso
 ...
 var ajax=nuevoAjax();
	...
	ajax.send("accion=GuardarActivosTransferidos&CodOrganismo="+CodOrganismo+"&CodDependencia="+CodDependencia+"&Descripcion="+Descripcion+
		      ...
		      "&FlagParaMantenimiento="+FlagParaMantenimiento+"&FlagParaOperaciones="+FlagParaOperaciones+"&SituacionActivo="+SituacionActivo+
		      "&PreparadoPor="+PreparadoPor+"&OrigenActivo="+OrigenActivo+"&UnidadMedida="+UnidadMedida+"&DepreEspecificaFlag="+DepreEspecificaFlag+
		      "&CodTipoMovimiento="+CodTipoMovimiento+"&LocalInicio="+LocalInicio+"&Naturaleza="+Naturaleza+'&TipoTransaccion='+TipoTransaccion+
		      "&Contabilidad="+Contabilidad+"&Periodo="+Periodo+"&LocalMesFinal="+LocalMesFinal+"&LocalNeto="+LocalNeto+"&ALocalInicio="+ALocalInicio+
		      "&ALocalMesFinal="+ALocalMesFinal+"&ALocalNeto="+ALocalNeto+"&ObligacionTipoDocumento="+ObligacionTipoDocumento+
		      "&ObligacionNroDocumento="+ObligacionNroDocumento+"&FechaObligacion="+FechaObligacion+"&NumeroFactura="+NumeroFactura+
...
function guardarActivosMenores(form){ 
 ...
 var ajax=nuevoAjax();
	ajax.open("POST", "af_php_acciones.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=GuardarActivosMenores&CodOrganismo="+CodOrganismo+"&CodDependencia="+CodDependencia+"&Descripcion="+Descripcion+
		      "&CodigoBarras="+CodigoBarras+"&CodigoInterno="+CodigoInterno+"&Clasificacion="+Clasificacion+"&Ubicacion="+Ubicacion+
		      "&ActivoConsolidado="+ActivoConsolidado+"&EmpleadoResponsable="+EmpleadoResponsable+"&CentroCosto="+CentroCosto+"&Marca="+Marca+
		      "&Modelo="+Modelo+"&NumeroSerie="+NumeroSerie+"&Dimensiones="+Dimensiones+"&Color="+Color+"&FabricacionPais="+FabricacionPais+
		      "&FabricacionAno="+FabricacionAno+"&CodProveedor="+CodProveedor+"&NumeroOrden="+NumeroOrden+"&NumeroOrdenFecha="+NumeroOrdenFecha+
		      "&NumeroGuia="+NumeroGuia+"&NumeroGuiaFecha="+NumeroGuiaFecha+"&FechaIngreso="+FechaIngreso+"&PeriodoIngreso="+PeriodoIngreso+
		      "&MontoLocal="+MontoLocal+"&FlagParaOperaciones="+FlagParaOperaciones+"&SituacionActivo="+SituacionActivo+"&PreparadoPor="+PreparadoPor+
		      "&Naturaleza="+Naturaleza+"&DescpCorta="+DescpCorta+"&Estado="+Estado+"&CodTipoMovimiento="+CodTipoMovimiento+
		      "&ClasificacionPublic20="+ClasificacionPublic20+"&Categoria="+Categoria+"&EmpleadoUsuario="+EmpleadoUsuario+
		      "&NumeroFactura="+NumeroFactura+"&FacturaFecha="+FacturaFecha+"&ObligacionTipoDocumento="+ObligacionTipoDocumento+
		      "&ObligacionNroDocumento="+ObligacionNroDocumento+"&FechaObligacion="+FechaObligacion);
...
//// ------------------------------------------------------------------------------------- ///
function modificarActivosMenores(form, valor){
 ...
 var ajax=nuevoAjax();
	ajax.open("POST", "af_php_acciones.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=GuardarModificacionesActivosMenores&CodOrganismo="+CodOrganismo+"&CodDependencia="+CodDependencia+"&Descripcion="+Descripcion+
		      "&CodigoBarras="+CodigoBarras+"&CodigoInterno="+CodigoInterno+"&Clasificacion="+Clasificacion+"&Ubicacion="+Ubicacion+
		      "&ActivoConsolidado="+ActivoConsolidado+"&EmpleadoResponsable="+EmpleadoResponsable+"&CentroCosto="+CentroCosto+"&Marca="+Marca+
		      "&Modelo="+Modelo+"&NumeroSerie="+NumeroSerie+"&Dimensiones="+Dimensiones+"&Color="+Color+"&FabricacionPais="+FabricacionPais+
		      "&FabricacionAno="+FabricacionAno+"&CodProveedor="+CodProveedor+"&NumeroOrden="+NumeroOrden+
		      "&NumeroOrdenFecha="+NumeroOrdenFecha+"&NumeroGuia="+NumeroGuia+"&NumeroGuiaFecha="+NumeroGuiaFecha+"&FechaIngreso="+FechaIngreso+
		      "&PeriodoIngreso="+PeriodoIngreso+"&MontoLocal="+MontoLocal+"&FlagParaOperaciones="+FlagParaOperaciones+"&SituacionActivo="+SituacionActivo+
		...
		if (ajax.readyState==4)	{
			var resp = ajax.responseText.trim();
			if (resp != "")alert(resp); 
            else if(valor=='AprobarAltaActivo') cargarPagina(form, "af_procaprobacionactalta.php?limit=0");
            else if(valor=='ModifcarActivoMenor') cargarPagina(form, "af_activosmenores.php?limit=0");
		}
	}
	return false;
}
...
function valorTerreno(form, id){
 var CodCatastro = document.getElementById(id).value; 
 ...
 var ajax=nuevoAjax();
	...
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText.trim();
			if (resp != "")document.getElementById("area_terreno2").value = resp; 
		}
	}
...
function cargarOpcionListActEditar(form, pagina, target, param) { 
	var codigo=form.registro.value;
	var estadoMovimiento = document.getElementById("estadoMov").value;
	
	if (codigo=="") msjError(1000);
	else 
	  if((estadoMovimiento=='AP')||(estadoMovimiento=='RV')){ 
	     if(estadoMovimiento=='AP') var ST = 'Aprobado'; else if(estadoMovimiento=='AP') var ST = 'Revisado';
...
//// -------------------------------------------------------------------------------------
function c_opcion_estado(form, estadoMovimientoActivo){
 document.getElementById("estadoMov").value = estadoMovimientoActivo; 
}
//// -------------------------------------------------------------------------------------
//	 -------------------		FUNCION PARA CARGAR UNA NUEVA PAGINA 
function cargarImprimir(form, pagina, target, param) { 
	var codigo = form.registro.value;
	    codigo = codigo.split('|');
	
	var Organismo = codigo[0];
	var MovimientoNumero = codigo[1];
	var Estado = codigo[2];
	var NroActa = codigo[3];
	var Anio = codigo[4];
	var TipoActa = codigo[5];
	var Pase = 'P';
	
	
	if (codigo == "") alert("¡Debe seleccionar un registro!");
	else if(Estado!='AP') alert("¡El registro debe estar en estado Aprobado¡");
	else {
		if (target == "SELF") cargarPagina(form, pagina);
			...
			pagina2 = pagina + "&registro=" + codigo;
			window.open('af_actaentregabmmovimiento.php?CodOrganismo='+Organismo+'&Pase='+Pase+"&nroActaEntrega="+NroActa+"&Anio="+Anio+"&MovimientoNumero="+MovimientoNumero+"&TipoActa="+TipoActa,'', 'height=500, width=870, left=200, top=100, resizable=yes');
		}
	}
}
//// -------------------------------------------------------------------------------------
///  -----------------      GUARDAR NUEVO MOVIMIENTO DE ACTIVO
function guardarRevisarMovimientoActivo(form){
  var Organismo = document.getElementById("fOrganismo").value;
  var NumeroMovimiento = document.getElementById("fmovimiento").value;
  ...
  var ajax=nuevoAjax();
	ajax.open("POST", "gmactivofijo.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=guardarRevisarMovimientoActivo&Activo="+Activo+"&Organismo="+Organismo+"&NumeroMovimiento="+NumeroMovimiento+"&Estado="+Estado+"&RevisadoPor="+RevisadoPor+"&Anio="+Anio);
...
//// ------------------------------------------------------------------------------------- ####
//// -----------------		APROBAR ACTIVO "ESTADO = AP" PROCESO DE ALTA ACTIVO
function AprobarActivo(form, otro){
   var estado = 'AP'; 
   var paso = '1';
   var Activo = document.getElementById("nro_activo").value; 
   var CodOrganismo = document.getElementById("organismo").value;
   var FechaIngreso = document.getElementById("fecha_ingreso").value;  //alert(Activo+'#'+CodOrganismo+'#'+FechaIngreso);
   var valor = FechaIngreso.split('-');
   var AnioActaIncorpActivo = valor[2]; 
   //var FechaInventario = document.getElementById("fechainventario").value;
       
   var ajax=nuevoAjax();
	ajax.open("POST", "gmactivofijo.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=AprobarActivo&estado="+estado+"&Activo="+Activo+"&CodOrganismo="+CodOrganismo+"&paso="+paso+"&FechaIngreso="+FechaIngreso);
    ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText;
			if(resp!=""){ //alert(resp);
			  resp = resp.split('|');
			  opener.document.getElementById("frmentrada").submit();
			  window.open('af_actaincorporacion.php?Anio='+AnioActaIncorpActivo+'&CodOrganismo='+CodOrganismo+"&nroActaIncorp="+resp[0],'', 'height=500, width=870, left=200, top=100, resizable=yes');
	           window.open('af_actaentregabm.php?Anio='+resp[1]+'&CodOrganismo='+CodOrganismo+"&nroActaEntrega="+resp[2],'','height=500, width=870, left=200, top=100, resizable=yes');
			   
			}
			//else alert("No esta Autorizado para dar de Alta a un Activo");
            //else opener.document.getElementById("frmentrada").submit();
		}
    }
...
function selectMotMovimiento(form){
    var radio = document.getElementById("radioEstado").value; 
	if(radio == 'I'){ 
...
//// -------------------------------------------------------------------------------------
//// CARGAR NOMBRE DEL TIPO DE MOVIMIENDO DE ACTIVOS DESINCORPORACION
function cargarMotTraslDes(form){
   var mot_trasl_des = document.getElementById("mot_trasl_des").value; 
   
   var ajax=nuevoAjax();
	ajax.open("POST", "af_fphp_ajax.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=cargarMotTraslDes&mot_trasl_des="+mot_trasl_des);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText;
			document.getElementById("motivo_trasladodes").value = resp;
		}
	}
   
}
//// -------------------------------------------------------------------------------------
//// CARGAR NOMBRE DEL TIPO DE MOVIMIENDO DE ACTIVOS INCORPORACION
function cargarMotTraslIncorp(form){
   var mot_trasl_incorp = document.getElementById("mot_trasl_incorp").value; 
   
   var ajax=nuevoAjax();
	ajax.open("POST", "af_fphp_ajax.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=cargarMotTraslIncorp&mot_trasl_incorp="+mot_trasl_incorp);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText;
			document.getElementById("motivo_trasladoinc").value = resp;
		}
	}
...
function cambioFormatoCantidad(id,valor){
    valorF = setNumeroFormato(valor,2,'.',','); 
...
//// --------------------------------------------------------------------------------------
function clickAC(){
   document.getElementById("clickAF").value = 1; 
}
//// --------------------------------------------------------------------------------------
...
/// -------------------------------------------------------------------------------- ///
/// -----------------		ANULAR REGISTRO MOVIMIENTO DE ACTIVO
/// -------------------------------------------------------------------------------- ///
function anularRegistro(form){
	var codigo=form.registro.value; 
	if (codigo=="") msjError(1000);
	else {
		...
		if (anular) {
			...
			ajax.open("POST", "gmactivofijo.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("accion=AnularMovimientoActivo&codigo="+codigo);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4){
					var resp = ajax.responseText;
					if(resp!="") alert(resp);
					else opener.document.getElementById("frmentrada").submit(); //cargarPagina(form,"af_movimientoactivos.php?limit=0");
				}
			}
		}
	}
