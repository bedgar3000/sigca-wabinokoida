...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Acta Responsabilidad de Uso</td>
...
if($fusuario!=""){
  $filtro.="AND (EmpleadoUsuario='".$fusuario."')"; $cUsuario = "checked"; $vis_usuario='style="visibility:visible"';
}else{ 
  $dUsuario="disabled"; $vis_usuario='style="visibility:hidden"';
}
if($fprimario!=""){
  $filtro.="AND (EmpleadoResponsable='".$fprimario."')"; $cPrimario = "checked"; $vis_primario='style="visibility:visible"';
}else{ 
  $dPrimario="disabled"; $vis_primario='style="visibility:hidden"';
}

echo"<form name='frmentrada' id='frmentrada' action='af_actaresponuso.php?limit=0' method='POST'>
<table class='tblForm' width='100%' height='50'>
<tr>
	<td width='15'></td>
...
<tr>
   ...
   echo"</select></td>
   <td align='right'>R. Primario:</td>
   <td class='gallery clearfix'>
	   <input type='checkbox' name='chkprimario' id='chkprimario' value='1' $cPrimario onclick='enabledPrimarioActivoUso(this.form);'/>
	   <input type='hidden' name='fprimario' id='fprimario' value='$fprimario'/>
	   <input type='text' name='fnombprimario' id='fnombprimario' value='$fnombprimario' size='40' $dPrimario readonly/>";?> 
	   <input type="hidden" name="btprimario" id="btprimario" value="..." onclick="cargarVentanaLista(this.form, 'af_listaempleados.php?limit=0&campo=8&ventana=SelEmpleadoUsuario','height=500, width=850, left=200, top=100, resizable=yes');" <?=$dPrimario;?>/> <a id="primario" href="af_listaempleados.php?filtrar=default&limit=0&campo=31&cierre=1&iframe=true&width=75%&height=100%" rel="prettyPhoto[iframe3]" <?=$vis_primario;?>>
			<img src="../imagenes/f_boton.png" width="20" title="Seleccionar" align="absbottom" style="cursor:pointer;"/>
		</a>
   <? echo" </select></td>
...
<tr>
	...
	<td align='left'>
	   ...
	   <option value=''></option>";
		 getSituacionActivo($fSituacionActivo, 1);
	   echo"
	   </select>
	</td>
	<td width='15'></td>
	<td align='right'>Cat/Clasf.:</td>
	   ...
	   echo"
	   </select><input type='hidden' id='fClasificacion' name='fClasificacion' value='$fClasificacion'/><input type='text' id='DescpClasificacion' name='DescpClasificacion' value='$DescpClasificacion' $dCatClasf size='16' readonly/>";?><input type='hidden' id='btClasif' name='btClasif' value='...' onclick="cargarVentanaLista(this.form, 'af_listaclasificacionactivo.php?limit=0&campo=17&ventana=insertarClasificacionActivo','height=500, width=800, left=200, top=100, resizable=yes');" <?=$dCatClasf?>/> <a id="clasificacion" href="af_listaclasificacionactivo.php?filtrar=default&limit=0&campo=17&ventana=insertarClasificacionActivo&iframe=true&width=70%&height=100%" rel="prettyPhoto[iframe4]" <?=$visib_CatClasf;?>>
				<img src="../imagenes/f_boton.png" width="20" title="Seleccionar" align="absbottom" style="cursor:pointer;"/>
			</a>
	 <? echo"</td>	
	 
	 <td align='right'>R. Usuario:</td>
	 <td class='gallery clearfix'>
		 <input type='checkbox' name='chkusuario' id='chkusuario' value='1' $cUsuario onclick='enabledUsuarioActivoUso(this.form);'/>
		 <input type='hidden' name='fusuario' id='fusuario' value='$fusuario'/>
		 <input type='text' name='fnombusuario' id='fnombusuario' value='$fnombusuario' size='40' $dUsuario readonly/>";?> 
		 <input type="hidden" name="btusuario" id="btusuario" value="..." onclick="cargarVentanaLista(this.form, 'af_listaempleados.php?limit=0&campo=7&ventana=SelEmpleadoUsuario','height=500, width=850, left=200, top=100, resizable=yes');" <?=$dUsuario;?>/> <a id="usuario" href="af_listaempleados.php?filtrar=default&limit=0&campo=32&iframe=true&width=75%&height=100%" rel="prettyPhoto[iframe5]" <?=$vis_usuario;?>>
				<img src="../imagenes/f_boton.png" width="20" title="Seleccionar" align="absbottom" style="cursor:pointer;"/>
			</a>
...
<table width="100%" class="tblLista">
 <tr> <input type="hidden" id="registro" name="registro"/>
  ...
  <input type="button" class="btLista" id="btGenerar" name="btGenerar" value="Generar" onclick="cargarMasivoActaRespUsoActivo(this.form,'GenerarActaRespUso');"/>
  <!-- <input type="button" class="btLista" id="btImprimir" name="btImprimir" value="Generar" />-->
...
<thead>
  ...
  if($ra!=0){
   ...
   for($i=0;$i<$ra;$i++){
	 ...
	 $id = $fa['CodOrganismo']."|".$fa['Activo']."|".$fa['CodDependencia']."|".$fa['EmpleadoUsuario'];
	 $valor = $fa['Naturaleza'];
