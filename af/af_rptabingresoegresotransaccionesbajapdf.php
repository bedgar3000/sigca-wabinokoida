...
function Header(){
	...
	$this->SetFont('Arial', '', 8);
	  $this->Cell(130,4,'___________________________________________________________________________________________________________________________________________________________________________',0,1,'L');
		
	$this->SetDrawColor(255, 255, 255);
	//$this->SetDrawColor(0, 0, 0);
	$this->SetFillColor(255, 255, 255);
	$this->SetWidths(array(19,20,65,16,16,45,20,45,20));
	$this->SetAligns(array('C','C','C','C','C','C','C','C','C'));
	$this->Row(array('Activo', utf8_decode('Código Barras'), utf8_decode('Descripcion'), utf8_decode('Fecha Ingreso'), utf8_decode('Fecha Baja'), utf8_decode('Ubicación'), 'Monto Activo Local', 'Cuenta', 'Local' ));
	//$this->SetFillColor(255, 255, 255);
	
	$this->SetFont('Arial', '', 8);
	  $this->Cell(130,2,'___________________________________________________________________________________________________________________________________________________________________________',0,1,'L');
	///// ******************	
}
//Page footer
function Footer(){
...
$s_con_01 = "select 
			   ...
			   from 
			    	af_activo a
					inner join af_transaccionbaja b on (b.Organismo=a.CodOrganismo)and(b.Activo=a.Activo)and(b.Estado='AP')
...
if($r_con_01!=0)
   for($i=0; $i<$r_con_01; $i++){
	...
	list($fb_ano, $fb_mes, $fb_dia) = split('[-]', $f_con_01['FechaBaja']); //// --- Fecha Baja de af_transaccionbaja
	...
	if($periodoCapturado!=$f_con_01['PeriodoIngreso']){	
	  ...
	  if($Pase==1){
		...
		$pdf->SetFont('Arial','B', 8); $pdf->Cell(150,4,utf8_decode('Total Categoría:'),0,0,'R');
									   $pdf->SetFont('Arial','', 8); $pdf->Cell(95,4,$Descp_cat,0,0,'L');
									   $pdf->Cell(20,4,'______________',0,1,'L');
									   
									   $pdf->Cell(235,4,'',0,0,'R');
									   $pdf->Cell(10,4,'Total: ',0,0,'L');
									   $pdf->SetFont('Arial','B', 8);$pdf->Cell(20,4,number_format($monto,2,',','.'),0,1,'C');
	  ...
	  $Descp_cat = substr(utf8_decode($f_categoria['DescripcionLocal']),0,50);
		/// Imprime titulo
	  ...
	  $pdf->SetFont('Arial', 'B', 8);$pdf->Cell(15,4,'C. Costos:', 0,0,'L');
	  $pdf->SetFont('Arial', '', 8);$pdf->Cell(30,4,$f_con_01['CentroCosto'], 0,1,'L');
	    /// Imprime datos de Archivo 19,20,65,16,16,40,20,20,20
	  $pdf->SetFont('Arial', '', 8);$pdf->Cell(19,4,$f_con_01['Activo'],0,0,'L');
	  								$pdf->Cell(20,4,$f_con_01['CodigoBarras'],0,0,'C');
									$pdf->Cell(65,4,substr(utf8_decode($f_con_01['Descripcion']),0,55), 0,0,'L');
									$pdf->Cell(16,4,$factura_ingreso,0,0,'C');
									$pdf->Cell(16,4,$fecha_baja, 0,0,'C');
									$pdf->Cell(45,4,substr(utf8_decode($f_ubicacion['Descripcion']),0,40), 0,0,'L');
									$pdf->Cell(20,4,number_format($f_con_01['MontoLocal'],2,',','.'),0,0,'R');
									$pdf->Cell(45,4,$f_con_01['CuentaHistorica'].' - '.substr(utf8_decode($f_categoria['DescripcionLocal']),0,20), 0,0,'L');
									$pdf->Cell(20,4,number_format($f_con_01['MontoLocal'],2,',','.'),0,1,'C'); $pdf->Ln(1);
	  $Pase=1; 
	...
	if($cont_veces==$r_con_01){
	   /// Imprime totales Categoria
		...
		$pdf->SetFont('Arial','B', 8); $pdf->Cell(225,4,'',0,0,'R');
									   $pdf->Cell(20,4,'____________________',0,1,'L');
		$pdf->SetFont('Arial','B', 8); $pdf->Cell(225,4,'Total General: ',0,0,'R');
