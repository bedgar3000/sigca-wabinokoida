...
function empleados_nivelaciones(form) {
	...
	else if ($("#CodOrganismo").val() == $("#CodOrganismoAnt").val() && $("#CodDependencia").val() == $("#CodDependenciaAnt").val() && $("#CodTipoNom").val() == $("#CodTipoNomAnt").val() && $("#CodCargo").val() == $("#CodCargoAnt").val() && $("#CodCargo").val() == $("#CodCargoReal").val() && $("#Paso").val() == $("#PasoAnt").val() && $("#CodCentroCosto").val() == $("#CodCentroCostoAnt").val()) error = "No ha realizado ning&uacute;n movimiento para la nivelaci&oacute;n";
	//	formulario
	var opcion = $("#opcion").val();
	var post = "";
	for(var i=0; n=form.elements[i]; i++) {
...
function empleado_vacaciones() {
	...
	for(var i=0; n=frm.elements[i]; i++) {
		...
		else if (n.name == "Anio") detalles_utilizacion += n.value + ";char:td;";
		else if (n.name == "CodSolicitud") detalles_utilizacion += n.value + ";char:td;";
...
//	empleados (conceptos)
function empleados_conceptos(form, accion) {
	bloqueo(true);
	
	//	valido
	var error = "";
	if ($("#CodConcepto").val() == "") error = "Debe llenar los campos obligatorios";
	else if ($("#PeriodoDesde").val() > $("#PeriodoHasta").val() && $("#PeriodoHasta").val().trim() != "") error = "Periodo de Inicio no puede ser mayor al Periodo Fin";
	else if (isNaN(setNumero($("#Monto").val()))) error = "Formato de Monto incorrecto";
	else if (isNaN(setNumero($("#Cantidad").val()))) error = "Formato de Cantidad incorrecto";
	...
	if (error != "") { cajaModal(error, "error", 400); }
		...
		$.ajax({
			type: "POST",
			url: "lib/form_ajax.php",
			data: "modulo=empleados_conceptos&accion="+accion+"&"+post,
			async: false,
			success: function(resp) {
				if (resp.trim() != "") cajaModal(resp, "error", 400);
				else form.submit();
