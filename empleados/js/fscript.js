...
function getPasoSueldo(CodCargo, iCategoriaCargo, iPaso, iSueldoActual) {
	$.ajax({
		type: "POST",
		url: "lib/fphp_funciones_ajax.php",
		data: "accion=getPasoSueldo&CodCargo="+CodCargo,
		async: false,
		dataType: "json",
		success: function(data) {
			$(iCategoriaCargo).val(data['CategoriaCargo']);
			$(iPaso).html(data['Pasos']);
			$(iPaso).val(data['Paso']);
			$(iSueldoActual).val(data['NivelSalarial']).formatCurrency();
		}
	});
}

function getSueldoxPaso(CodCargo, Paso, iSueldoActual) {
	if (CodCargo != '' && Paso != '') {
		$.ajax({
			type: "POST",
			url: "lib/fphp_funciones_ajax.php",
			data: "accion=getSueldoxPaso&CodCargo="+CodCargo+"&Paso="+Paso,
			async: false,
			dataType: "json",
			success: function(data) {
				$(iSueldoActual).val(data['SueldoPromedio']).formatCurrency();
			}
		});
	} else {
		$(iSueldoActual).val('0').formatCurrency();
	}
}