	...
	if ($opcion == "contratar") {
		...
		$sql = "SELECT
				...
				FROM
					...
					LEFT JOIN pv_categoriaprog cp ON (cp.CategoriaProg = e.CategoriaProg)
...
<input type="hidden" id="HCodCargo" value="<?=$field['CodCargo']?>" />
<input type="hidden" id="HPaso" value="<?=$field['Paso']?>" />
...
<div id="tab1" style="display:block;">
	<table width="750" class="tblForm">
		...
		<tr>
			...
			<td>
	           	<input type="text" id="Ndocumento" style="width:100px;;" maxlength="20" value="<?=$field['Ndocumento']?>" class="documento" onchange="validarCedula(this.value);" <?=$disabled_ver?> />
			</td>
		</tr>
		<tr>
			...
			<td>
	           	<input type="text" id="DocFiscal" style="width:100px;;" maxlength="20" value="<?=$field['DocFiscal']?>" class="documento" onchange="validarRif(this.value);" <?=$disabled_ver?> />
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
	            <select id="CodPaisNac" style="width:175px;;" onchange="getOptionsSelect(this.value, 'estado', 'CodEstadoNac', true, 'CodMunicipioNac', 'CiudadNacimiento');" <?=$disabled_ver?>>
	                <?=loadSelect("mastpaises", "CodPais", "Pais", $field['CodPaisNac'], 0);?>
	            </select>
			</td>
			<td class="tagForm">* Estado:</td>
			<td>
	            <select id="CodEstadoNac" style="width:175px;;" onchange="getOptionsSelect(this.value, 'municipio', 'CodMunicipioNac', true, 'CiudadNacimiento');" <?=$disabled_ver?>>
		...
		<tr>
			...
			<td>
	            <select id="CodMunicipioNac" style="width:175px;;" onchange="getOptionsSelect(this.value, 'ciudad', 'CiudadNacimiento', true); setLnacimiento();" <?=$disabled_ver?>>
			...
			<td>
	            <select id="CiudadNacimiento" style="width:175px;" onchange="setLnacimiento();"> <?=$disabled_ver?>>
	    ...
	    <tr>
			...
			<td>
	        	<input type="text" id="Fnacimiento" style="width:60px;" class="datepicker" maxlength="10" value="<?=formatFechaDMA($field['Fnacimiento'])?>" onchange="getEdad($(this).val(), '<?=formatFechaDMA($FechaActual)?>', $('#EdadAnios'), $('#EdadMeses'), $('#EdadDias'))" <?=$disabled_ver?> />
			</td>
		...
		<tr>
			...
			<td colspan="3">
	        	<input type="text" id="Lnacimiento" style="width:99%;" class="disabled" value="<?=$field['Lnacimiento']?>" disabled />
			</td>
		</tr>
		<tr>
...
<div id="tab3" style="display:none;">
	<table width="750" class="tblForm">
		...
		<tr>
			...
			<td colspan="3">
	            <select id="CodOrganismo" style="width:400px;" onchange="getOptionsSelect(this.value, 'dependencia_filtro', 'CodDependencia', true, 'CodCentroCosto');" <?=$disabled_ver?>>
		...
		<tr>
			...
			<td colspan="3">
	            <select id="CodDependencia" style="width:400px;" onchange="getOptionsSelect(this.value, 'centro_costo', 'CodCentroCosto', true);" <?=$disabled_ver?>>
		...
		<tr>
			<td class="tagForm">Categoria Prog.:</td>
			<td colspan="3" class="gallery clearfix">
				...
				<input type="text" name="UnidadEjecutora" id="UnidadEjecutora" value="<?=$field['UnidadEjecutora']?>" style="width:296px;" disabled />
				<a href="../lib/listas/gehen.php?anz=lista_pv_categoriaprog&filtrar=default&ventana=unidad_ejecutora&FlagOrganismo=S&fCodOrganismo=<?=$field['CodOrganismo']?>&campo1=CategoriaProg&campo2=UnidadEjecutora&iframe=true&width=100%&height=100%" rel="prettyPhoto[iframe1]" id="aCategoriaProg" style=" <?=$display_modificar?>">
		...
		<tr>
			...
			<td>
	            <select id="CodTipoNom" style="width:200px;" <?=$disabled_ver?>>
	            	<option value="">&nbsp;</option>
...
<div id="tab4" style="display:none;">
	<table width="750" class="tblForm">
		...
		<tr>
			...
			<td>
	            <input type="text" id="NroResolucionIngreso" style="width:95%;" class="cese" value="<?=$field['NroResolucionIngreso']?>" <?=$disabled_ver?> />
			</td>
		</tr>
		<tr>
	    ...
	    <tr>
			...
			<td>
	            <input type="radio" name="SitTra" id="SitTraA" value="A" <?=chkOpt($field['SitTra'], "A");?> onclick="setCese($(this).val());" disabled="disabled" /> Activo
	            <input type="radio" name="SitTra" id="SitTraI" value="I" <?=chkOpt($field['SitTra'], "I");?> onclick="setCese($(this).val());" disabled="disabled" /> Inactivo
			</td>
			<td class="tagForm" width="125">Motivo del Cese:</td>
			<td>
	            <select id="CodMotivoCes" style="width:200px;" class="cese" disabled="disabled">
	            	<option value="">&nbsp;</option>
	    ...
	    <tr>
			...
			<td>
	            <input type="text" id="Fegreso" style="width:60px;" class="datepicker cese" maxlength="10" value="<?=formatFechaDMA($field['Fegreso'])?>" disabled="disabled" />
			</td>
			<td class="tagForm">Explicaci&oacute;n:</td>
			<td>
	            <input type="text" id="ObsCese" style="width:95%;" class="cese" value="<?=$field['ObsCese']?>" disabled="disabled" />
			</td>
		</tr>
	    <tr>
	    	<td>&nbsp;</td>
			...
			<td class="tagForm">Nro. Resoluci&oacute;n:</td>
			<td>
	            <input type="text" id="NroResolucionEgreso" style="width:95%;" class="cese" value="<?=$field['NroResolucionEgreso']?>" disabled />
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3">
	            <select id="CodGrupOcup" style="width:400px;" onchange="loadSelect($('#CodSerieOcup'), 'tabla=serie-ocupacional&opcion='+$(this).val(), 1, ['CodCargo','CategoriaCargo','SueldoActual','Paso']);" <?=$disabled_modificar?>>
	            	<option value="">&nbsp;</option>
		...
		<tr>
			...
			<td colspan="3">
	            <select id="CodSerieOcup" style="width:400px;" onchange="loadSelect($('#CodCargo'), 'tabla=cargo&opcion='+$(this).val(), 1, ['CategoriaCargo','SueldoActual','Paso']);" <?=$disabled_modificar?>>
		...
		<tr>
			...
			<td colspan="3">
	            <select id="CodCargo" style="width:400px;" onchange="getPasoSueldo($(this).val(), $('#CategoriaCargo'), $('#Paso'), $('#SueldoActual'));" <?=$disabled_modificar?>>
		...
		<tr>
			...
			<td colspan="3">
	            ...
	            <select id="Paso" style="width:55px;" <?=$disabled_modificar?> onchange="getSueldoxPaso($('#CodCargo').val(),$('#Paso').val(),$('#SueldoActual'));">
...
<script type="text/javascript" language="javascript">
	...
	function setLnacimiento() {
		var CiudadNacimiento = $("#CiudadNacimiento option:selected").text();
		var CodEstadoNac = $("#CodEstadoNac option:selected").text();
		var Lnacimiento = CiudadNacimiento + ' ESTADO ' + CodEstadoNac;
		$('#Lnacimiento').val(Lnacimiento);
	}
    function validarCedula(Ndocumento) {
    	if (Ndocumento) {
	        $.post('empleados_ajax.php', {Ndocumento:Ndocumento, modulo:'ajax', accion:'validarCedula'}, function(data) {
	            if (data['status'] == 'error') {
	            	cajaModal(data['message']);
	            	$('#Ndocumento').val('').focus();
	            }
	            else if (data['status'] == 'update') {
	            	cajaModal(data['message'],'info');
	            	$('#CodPersona').val(data['CodPersona']);
	            	$('#Apellido1').val(data['Apellido1']);
	            	$('#Apellido2').val(data['Apellido2']);
	            	$('#Nombres').val(data['Nombres']);
	            	$('#NomCompleto').val(data['NomCompleto']);
	            	$('#Busqueda').val(data['Busqueda']);
	            	$('#Sexo').val(data['Sexo']);
	            	$('#Nacionalidad').val(data['Nacionalidad']);
	            	$('#Fnacimiento').val(data['Fnacimiento']);
	            	$('#CiudadNacimiento').val(data['CiudadNacimiento']);
	            	$('#Lnacimiento').val(data['Lnacimiento']);
	            	$('#EstadoCivil').val(data['EstadoCivil']);
	            	$('#FedoCivil').val(data['FedoCivil']);
	            	$('#Direccion').val(data['Direccion']);
	            	$('#Telefono1').val(data['Telefono1']);
	            	$('#Telefono2').val(data['Telefono2']);
	            	$('#Fax').val(data['Fax']);
	            	$('#CiudadDomicilio').val(data['CiudadDomicilio']);
	            	$('#TipoDocumento').val(data['TipoDocumento']);
	            	$('#Ndocumento').val(data['Ndocumento']);
	            	$('#DocFiscal').val(data['DocFiscal']);
	            	$('#TipoPersona').val(data['TipoPersona']);
	            	$('#NomEmerg1').val(data['NomEmerg1']);
	            	$('#DirecEmerg1').val(data['DirecEmerg1']);
	            	$('#TelefEmerg1').val(data['TelefEmerg1']);
	            	$('#CelEmerg1').val(data['CelEmerg1']);
	            	$('#NomEmerg2').val(data['NomEmerg2']);
	            	$('#DirecEmerg2').val(data['DirecEmerg2']);
	            	$('#TelefEmerg2').val(data['TelefEmerg2']);
	            	$('#CelEmerg2').val(data['CelEmerg2']);
	            	$('#ParentEmerg1').val(data['ParentEmerg1']);
	            	$('#ParentEmerg2').val(data['ParentEmerg2']);
	            	$('#SituacionDomicilio').val(data['SituacionDomicilio']);
	            	...
	            	$('#ExpiraLicencia').val(data['ExpiraLicencia']);
	            	$('#SiAuto').val(data['SiAuto']);
	            	$('#EdoReg').val(data['EdoReg']);
	            	$('#CodPaisNac').val(data['CodPaisNac']);
	            	$('#CodEstadoNac').val(data['CodEstadoNac']);
	            	$('#CodMunicipioNac').val(data['CodMunicipioNac']);
	            	$('#CodPaisDom').val(data['CodPaisDom']);
	            	$('#CodEstadoDom').val(data['CodEstadoDom']);
	            	$('#CodMunicipioDom').val(data['CodMunicipioDom']);
	            	getEdad($('#Fnacimiento').val(), '<?=formatFechaDMA($FechaActual)?>', $('#EdadAnios'), $('#EdadMeses'), $('#EdadDias'));
	            }
	        }, 'json');
    	}
    }
    function validarRif(DocFiscal) {
    	if (DocFiscal) {
	        $.post('empleados_ajax.php', {DocFiscal:DocFiscal, modulo:'ajax', accion:'validarRif'}, function(data) {
	            if (data['status'] == 'error') {
	            	cajaModal(data['message']);
	            	$('#DocFiscal').val('').focus();
	            }
