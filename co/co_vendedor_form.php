...
if ($opcion == "nuevo") {
	$field['Estado'] = 'A';
	##
	$_titulo = "Vendedores / Nuevo Registro";
	$accion = "nuevo";
	$disabled_modificar = "";
	$disabled_ver = "";
	$read_modificar = "";
...
elseif ($opcion == "modificar" || $opcion == "ver") {
	...
	$sql = "SELECT
			...
			WHERE v.CodVendedor = '$sel_registros'";
	$field = getRecord($sql);
	##
	if ($opcion == "modificar") {
		$_titulo = "Vendedores / Modificar Registro";
		$accion = "modificar";
	...
	##
	elseif ($opcion == "ver") {
		$_titulo = "Vendedores / Ver Registro";
		$accion = "";
		$disabled_modificar = "disabled";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=co_vendedor_lista" method="POST" enctype="multipart/form-data" onsubmit="return formSubmit('co_vendedor_ajax', 'modulo=formulario&accion=<?=$accion?>');" autocomplete="off">
	...
	<table style="width:100%; max-width:<?=$_width?>px;" class="tblForm">
	    ...
	    <tr>
			...
			<td>
	        	<input type="text" name="CodVendedor" id="CodVendedor" value="<?=$field['CodVendedor']?>" style="width:75px; font-weight:bold;" readonly />
			</td>
		</tr>
		<tr>
			...
			<td class="gallery clearfix">
				...
				<a href="../lib/listas/gehen.php?anz=lista_personas&campo1=CodPersona&campo2=NomPersona&ventana=&filtrar=default&FlagClasePersona=S&fEsEmpleado=S&fEsOtros=S&iframe=true&width=100%&height=100%" rel="prettyPhoto[iframe1]" style=" <?=$display_modificar?>">
	            	<img src="../imagenes/f_boton.png" width="20" title="Seleccionar" align="absbottom" style="cursor:pointer;" />
	            </a>
			</td>
		</tr>
	    <tr>
			<td class="tagForm">* Equipo:</td>
			<td>
				<select name="EquipoVenta" id="EquipoVenta" style="width:304px;" <?=$disabled_ver?>>
					<option value="">&nbsp;</option>
					<?=getMiscelaneos($field['EquipoVenta'], "COEQUIVENT", 0)?>
				</select>
			</td>
		</tr>
		<tr>
