...
if ($opcion == "nuevo") {
	$field['Estado'] = 'A';
	##
	$_titulo = "Tipos de Tarjeta / Nuevo Registro";
	$accion = "nuevo";
	$disabled_modificar = "";
	$disabled_ver = "";
	$read_modificar = "";
	...
	$focus = "CodTipoTarjeta";
}
elseif ($opcion == "modificar" || $opcion == "ver") {
	##	consulto datos generales
	$sql = "SELECT *
			FROM co_tipotarjeta
			WHERE CodTipoTarjeta = '$sel_registros'";
	$field = getRecord($sql);
	##
	if ($opcion == "modificar") {
		$_titulo = "Tipos de Tarjeta / Modificar Registro";
		$accion = "modificar";
	...
	##
	elseif ($opcion == "ver") {
		$_titulo = "Tipos de Tarjeta / Ver Registro";
		$accion = "";
		$disabled_modificar = "disabled";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=co_tipotarjeta_lista" method="POST" enctype="multipart/form-data" onsubmit="return formSubmit('co_tipotarjeta_ajax', 'modulo=formulario&accion=<?=$accion?>');" autocomplete="off">
	...
	<table style="width:100%; max-width:<?=$_width?>px;" class="tblForm">
	    ...
	    <tr>
			...
			<td>
	        	<input type="text" name="CodTipoTarjeta" id="CodTipoTarjeta" value="<?=$field['CodTipoTarjeta']?>" style="width:75px; font-weight:bold;" maxlength="3" <?=$read_modificar?> />
			</td>
		</tr>
	    <tr>
		...
		<tr>
			...
			<td>
				<select name="CodTipoPago" id="CodTipoPago" style="width:145px;" <?=$disabled_ver?>>
					<option value="">&nbsp;</option>
					<?=tipos_pago_tarjeta($field['CodTipoPago'])?>
				</select>
			</td>
		</tr>
		<tr>
