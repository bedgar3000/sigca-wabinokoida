...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=co_documento_lista" method="post" autocomplete="off">
				...
				<td>
					<?php if ($lista == 'listar') { ?>
			            ...
			            <select name="fEstado" id="fEstado" style="width:100px;" <?=$dEstado?>>
			                <option value="">&nbsp;</option>
			                <?=loadSelectValores("documento2-estado", $fEstado, 0)?>
			            </select>
					<?php } else { ?>
			            <input type="checkbox" <?=$cEstado?> onclick="this.checked=!this.checked;" />
			            <select name="fEstado" id="fEstado" style="width:100px;" <?=$dEstado?>>
			                <?=loadSelectValores("documento2-estado", $fEstado, 1)?>
			            </select>
					<?php } ?>
				</td>
	...
	<table class="tblBotones" style="width:100%; min-width:<?=$_width?>px;">
	    <tr>
	        ...
	        <td align="right" class="gallery clearfix">
	            ...
	            <input type="button" value="Nuevo" style="width:75px; <?=$_btNuevo?>" class="insert" onclick="cargarPagina(this.form, 'gehen.php?anz=co_documento_form&opcion=nuevo&origen=co_documento_lista');" />
            	<input type="button" value="Modificar" style="width:75px; <?=$_btModificar?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'co_documento_ajax.php', 'modulo=validar&accion=modificar', 'gehen.php?anz=co_documento_form&opcion=modificar', 'SELF', '');" />
	            <input type="button" value="Ver" style="width:75px;" class="ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=co_documento_form&opcion=ver', 'SELF', '', $('#sel_registros').val());" />
            	<input type="button" value="Modificar (Restringido)" style="width:125px; <?=$_btModificar?>" class="update" onclick="cargarOpcion2(this.form, 'gehen.php?anz=co_documento_form_restringido', 'SELF', '', $('#sel_registros').val());" />
	            <input type="button" value="Renumerar" style="width:75px; display: <?=opcionesPermisos('','01-0011')[0]?'':'none'?>;" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'co_documento_ajax.php', 'modulo=validar&accion=renumerar', 'gehen.php?anz=co_documento_form_renumerar', 'SELF', '');" />
	            <input type="button" value="Transferir CxP" style="width:75px; display: <?=opcionesPermisos('','01-0013')[0]?'':'none'?>;" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'co_documento_ajax.php', 'modulo=validar&accion=transferir', 'gehen.php?anz=co_documento_form_transferir', 'SELF', '');" />

	            <input type="button" value="Imprimir" style="width:75px;" class="ver" onclick="abrirReporteVal('a_reporte', 'co_documento_pdf', '', '', $('#sel_registros'), 0, this.form);" />
	        </td>
	    </tr>
	</table>
