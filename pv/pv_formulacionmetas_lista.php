...
//	------------------------------------
if ($lista == "listar") {
	$_titulo = "Formulaci&oacute;n por Metas";
	$_btNuevo = "";
	$_btModificar = "";
	$_btAprobar = "display:none;";
	$_btGenerarReformulacion = "display:none;";
}
...
elseif ($lista == "generar-reformulacion") {
	$fEstado = "GE";
	##	
	$_titulo = "Formulaci&oacute;n por Metas / Generar Reformulaci&oacute;n";
	$_btNuevo = "display:none;";
	$_btModificar = "display:none;";
	$_btAprobar = "display:none;";
	$_btGenerarReformulacion = "";
}
//	------------------------------------
if ($fBuscar != "") {
	$cBuscar = "checked";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pv_formulacionmetas_lista" method="post" autocomplete="off">
			...
			<td>
				...
				if ($lista == "listar") {
		            ...
		            <select name="fEstado" id="fEstado" style="width:107px;" <?=$dEstado?>>
		                <option value="">&nbsp;</option>
		                <?=loadSelectValores("metas-estado", $fEstado, 0)?>
		            </select>
					<?php
				} else {
					?>
		            ...
		            <select name="fEstado" id="fEstado" style="width:107px;" <?=$dEstado?>>
		                <?=loadSelectValores("metas-estado", $fEstado, 1)?>
		            </select>
					<?php
				}
				?>
...
<table class="tblBotones" style="width:100%; min-width:<?=$_width?>px;">
    <tr>
        ...
        <td align="right">
            <input type="button" value="Nuevo" style="width:75px; <?=$_btNuevo?>" class="insert" onclick="cargarPagina(this.form, 'gehen.php?anz=pv_formulacionmetas_form&opcion=nuevo&action=pv_formulacionmetas_lista');" />
            <input type="button" value="Modificar" style="width:75px; <?=$_btModificar?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_formulacionmetas_ajax.php', 'modulo=validar&accion=modificar', 'gehen.php?anz=pv_formulacionmetas_form&opcion=modificar&action=pv_formulacionmetas_lista', 'SELF', '');" />
            <input type="button" value="Aprobar" style="width:75px; <?=$_btAprobar?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_formulacionmetas_ajax.php', 'modulo=validar&accion=aprobar', 'gehen.php?anz=pv_formulacionmetas_form&opcion=aprobar&action=pv_formulacionmetas_lista', 'SELF', '');" />
            <input type="button" value="Generar" style="width:75px; <?=$_btGenerarReformulacion?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_formulacionmetas_ajax.php', 'modulo=validar&accion=generar-reformulacion', 'gehen.php?anz=pv_formulacionmetas_form&opcion=generar-reformulacion&action=pv_formulacionmetas_lista', 'SELF', '');" />
            <input type="button" value="Ver" style="width:75px;" class="ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pv_formulacionmetas_form&opcion=ver&action=pv_formulacionmetas_lista', 'SELF', '', $('#sel_registros').val());" />
        </td>
    </tr>
</table>
