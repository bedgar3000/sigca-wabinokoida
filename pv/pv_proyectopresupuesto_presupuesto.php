...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pv_proyectopresupuesto_presupuesto" method="post" autocomplete="off">
...
<table class="tblBotones" style="width:100%; min-width:<?=$_width?>px; margin:auto;">
    <tr>
        ...
        <td align="right">
            <input type="button" value="Generar" style="width:75px;" class="insert" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pv_proyectopresupuesto_form&opcion=generar-presupuesto&action=pv_proyectopresupuesto_presupuesto', 'SELF', '', $('#sel_registros').val());" />
        </td>
    </tr>
</table>
	...
	<table class="tblLista" style="width:100%; min-width:100%;">
		...
		$sql = "SELECT
				...
				FROM
				    ...
				    LEFT JOIN pv_unidadejecutoradep ued ON (ued.CodUnidadEjec = ue.CodUnidadEjec)
				WHERE (fmdt.Estado = 'AP') $filtro
				GROUP BY CodOrganismo, Ejercicio, CategoriaProg
				ORDER BY $fOrderBy
