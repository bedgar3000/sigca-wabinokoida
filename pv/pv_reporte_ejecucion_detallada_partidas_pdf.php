...
if (trim($fPeriodoD)) {$filtro .= " AND co.Fecha >= '$fPeriodoD'"; $filtro2 .= " AND co.Fecha >= '$fPeriodoD'";}
if (trim($fPeriodoH)) {$filtro .= " AND co.Fecha <= '$fPeriodoH'"; $filtro2 .= " AND co.Fecha <= '$fPeriodoH'";}
...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(196, 5, utf8_decode('EJECUCIÓN DETALLADA POR PARTIDAS'), 0, 1, 'C');
        $this->Ln(5);
        ##  
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(196, 5, utf8_decode(mb_strtoupper($f['CatProg'].' '.$f['Actividad'])), 0, 1, 'L');
        $this->Ln(1);
        ##  
        ...
        {
            $this->SetWidths(array(20,126,25,25));
...
$sql = "SELECT
            ...
            pptod.MontoAprobado,
            pptod.MontoAjustado,
            pptod.MontoCompromiso,
            (pptod.MontoAjustado - pptod.MontoCompromiso) AS MontoDisponible
        FROM
            pv_compromisos co
...
foreach ($field_detalle as $f) {
    ...
    if ($fFlagDisponible == 'S')
    {
        $sql = "SELECT SUM(MontoAjuste)
                ...
                WHERE
                    ...
                    AND ad.CodPresupuesto = '$f[CodPresupuesto]'
                    AND ad.CodFuente = '$f[CodFuente]'
        ...
        if ($MontoDisponible80 > $f['MontoDisponible']) $MontoDisponible80 = $f['MontoDisponible'];
        elseif ($MontoDisponible80 < 0) $MontoDisponible80 = 0;
        ##  
        $pdf->SetWidths(array(20,126,25,25));
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                LEFT JOIN ap_obligaciones ob On (ob.CodProveedor = co.CodProveedor
                                                AND ob.CodTipoDocumento = co.CodTipoDocumento
                                                AND ob.NroDocumento = co.NroDocumento)
                LEFT JOIN lg_ordencompra oc ON (oc.Anio = co.Anio
                                                AND oc.CodOrganismo = co.CodOrganismo
                                                AND oc.NroOrden = co.NroOrden)
                LEFT JOIN lg_ordenservicio os ON (os.Anio = co.Anio
                                                  AND os.CodOrganismo = co.CodOrganismo
                                                  AND os.NroOrden = co.NroOrden)
                LEFT JOIN ap_bancotransaccion tb ON (tb.NroTransaccion = co.NroTransaccion
            ...
            AND co.cod_partida = '$f[cod_partida]' $filtro2
            ORDER BY
                Fecha,
                Monto";
    ...
    foreach ($field_compromisos as $fc) {
        ...
        $pdf->SetDrawColor(255,255,255);
        $pdf->SetFillColor(255,255,255);
        if ($fFlagDisponible == 'S') $pdf->SetWidths(array(15,33,78,25));
        else $pdf->SetWidths(array(15,33,103,25));
