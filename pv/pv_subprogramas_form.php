...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pv_subprogramas_lista" method="POST" enctype="multipart/form-data" onsubmit="return formSubmit('pv_subprogramas_ajax', 'modulo=formulario&accion=<?=$accion?>');" autocomplete="off">
...
<table width="<?=$_width?>" class="tblForm">
    ...
    <tr>
		...
		<td>
        	<input type="text" name="IdSubPrograma" id="IdSubPrograma" value="<?=$field['IdSubPrograma']?>" style="width:65px; font-weight:bold;" readonly="readonly" />
		</td>
	</tr>
    <tr>
		<td class="tagForm">* C&oacute;digo:</td>
		<td>
        	<input type="text" name="CodSubPrograma" id="CodSubPrograma" value="<?=$field['CodSubPrograma']?>" style="width:65px;" maxlength="2" <?=$disabled_modificar?> />
		</td>
	</tr>
    <tr>
		<td class="tagForm">* Sub-Sector:</td>
		<td>
			<select name="IdSubSector" id="IdSubSector" style="width:404px;" <?=$disabled_modificar?> onChange="loadSelect($('#IdPrograma'), 'tabla=pv_programas&IdSubSector='+$('#IdSubSector').val(), 1);">
				<option value="">&nbsp;</option>
    ...
    <tr>
		...
		<td>
			<select name="IdPrograma" id="IdPrograma" style="width:404px;" <?=$disabled_modificar?>>
				<option value="">&nbsp;</option>
