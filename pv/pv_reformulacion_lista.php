...
if ($filtrar == "default") {
	...
	$fOrderBy = "CodReformulacion";
}
//	------------------------------------
if ($lista == "listar") {
	$_titulo = "Reformulaci&oacute;n de Presupuesto";
	$_btNuevo = "";
	$_btModificar = "";
...
elseif ($lista == "aprobar") {
	$fEstado = "PR";
	##	
	$_titulo = "Reformulaci&oacute;n de Presupuesto (Aprobar)";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pv_reformulacion_lista" method="post" autocomplete="off">
		...
		<td>
			...
			if ($lista == 'listar') {
	            ...
	            <select name="fEstado" id="fEstado" style="width:100px;" <?=$dEstado?>>
	                <option value="">&nbsp;</option>
	                <?=loadSelectValores("reformulacion-estado", $fEstado, 0)?>
	            </select>
				<?php
			} else {
				?>
	            ...
	            <select name="fEstado" id="fEstado" style="width:100px;" <?=$dEstado?>>
	                <?=loadSelectValores("reformulacion-estado", $fEstado, 1)?>
	            </select>
				<?php
			}
			?>
...
<table class="tblBotones" style="width:100%; min-width:<?=$_width?>px;">
    <tr>
        ...
        <td align="right">
            <input type="button" value="Nuevo" style="width:75px; <?=$_btNuevo?>" class="insert" onclick="cargarPagina(this.form, 'gehen.php?anz=pv_reformulacion_form&opcion=nuevo&action=pv_reformulacion_lista');" />
            <input type="button" value="Modificar" style="width:75px; <?=$_btModificar?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_reformulacion_ajax.php', 'modulo=validar&accion=modificar', 'gehen.php?anz=pv_reformulacion_form&opcion=modificar&action=pv_reformulacion_lista', 'SELF', '');" />
            <input type="button" value="Aprobar" style="width:75px; <?=$_btAprobar?>" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_reformulacion_ajax.php', 'modulo=validar&accion=aprobar', 'gehen.php?anz=pv_reformulacion_form&opcion=aprobar&action=pv_reformulacion_lista', 'SELF', '');" />
            <input type="button" value="Anular" style="width:75px;" class="update" onclick="cargarOpcionValidar3(this.form, $('#sel_registros').val(), 'pv_reformulacion_ajax.php', 'modulo=validar&accion=anular', 'gehen.php?anz=pv_reformulacion_form&opcion=anular&action=pv_reformulacion_lista', 'SELF', '');" /> |
            <input type="button" value="Ver" style="width:75px;" class="ver" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pv_reformulacion_form&opcion=ver&action=pv_reformulacion_lista', 'SELF', '', $('#sel_registros').val());" />
        </td>
    </tr>
</table>
...
<table class="tblLista" style="width:100%; min-width:1200px;">
	<thead>
    <tr>
        <th width="50" onclick="order('CodReformulacion')">C&oacute;digo</th>
	...
	$sql = "SELECT *
			...
			WHERE 1 $filtro
			GROUP BY r.CodOrganismo, CodReformulacion";
	...
	foreach($field as $f) {
		$id = $f['CodOrganismo'].'_'.$f['CodReformulacion'];
		?>
		<tr class="trListaBody" onclick="clk($(this), 'registros', '<?=$id?>');">
			<td align="center"><?=$f['CodReformulacion']?></td>
			...
			<td align="center"><?=printValores('reformulacion-estado',$f['Estado'])?></td>
		</tr>
		<?php
	}
	?>
