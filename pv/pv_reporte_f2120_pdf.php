...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Rect(10, 18, 335, 30, "D");
        ##  
        $this->SetY(48);
        $this->SetFont('Arial','B',6);
        $this->SetDrawColor(0,0,0);
        $this->SetFillColor(255,255,255);
        $this->SetWidths(array(145,10,180));
        $this->SetAligns(array('L','C','L'));
        $this->Row(array(utf8_decode('SECTOR: '.$field_categoria['CodSector'].' PROGRAMA: '.$field_categoria['CodPrograma'].' SUB-PROGRAMA: '.$field_categoria['CodSubPrograma'].' PROYECTO: '.$field_categoria['CodProyecto']),
                         '',
                         utf8_decode('UNIDAD(ES) EJECUTORA(S) '.$field_categoria['UnidadEjecutora'])
                ));
        $this->SetY(53);
        $this->SetFont('Arial','B',6);
        $this->Cell(121, 5, utf8_decode('OBRAS'), 1, 0, 'C');
        ...
        $this->SetFillColor(255,255,255);
        $this->SetWidths(array(18,18,85,12,12,10,20,20,20,20,20,20,20,20,20));
        $this->SetAligns(array('C','C','L','C','C','C','C','C','C','C','C','C','C','C','C'));
        ...
        $this->SetFontSize(6);
        $this->TextWithDirection(138,77,'INICIO','U');
        $this->TextWithDirection(149.5,77,'CONCLUSIÓN','U');
        $this->TextWithDirection(161,77,'SITUACIÓN','U');

        $this->Ln(1);
        $this->SetWidths(array(18,18,85,12,12,10,20,20,20,20,20,20,20,20,20));
        $this->SetAligns(array('C','C','L','C','C','C','R','R','R','R','R','R','R','R','R'));
    }
    ...
    function Footer() {
        ...
        $this->Rect(10, 48, 335, 152, "D");
        $this->Rect(28, 58, 0.1, 142, "FD");
        $this->Rect(46, 58, 0.1, 142, "FD");
        $this->Rect(131, 58, 0.1, 142, "FD");
        $this->Rect(143, 58, 0.1, 142, "FD");
        $this->Rect(155, 58, 0.1, 142, "FD");
        $this->Rect(165, 53, 0.1, 147, "FD");
        $this->Rect(185, 63, 0.1, 137, "FD");
        $this->Rect(205, 63, 0.1, 137, "FD");
        $this->Rect(225, 63, 0.1, 137, "FD");
        $this->Rect(245, 63, 0.1, 137, "FD");
        $this->Rect(265, 63, 0.1, 137, "FD");
        $this->Rect(285, 63, 0.1, 137, "FD");
        $this->Rect(305, 63, 0.1, 137, "FD");
        $this->Rect(325, 63, 0.1, 137, "FD");
        $this->Rect(10, 79, 335, 0.1, "FD");
        ##  
        if ($FlagTotal) {
            $this->SetY(200);
            $this->SetDrawColor(0,0,0);
            $this->SetFont('Arial','B',7);
            $this->Cell(155, 5, utf8_decode('TOTAL'), 1, 0, 'R');
        ...
        $this->SetY(205);
        $this->SetFont('Arial','B',7);
        $this->Cell(195, 5, utf8_decode('FORMA:     2120'), 0, 0, 'L');
    }
}
//---------------------------------------------------
...
foreach ($field_detalle as $f) {
    ...
    $pdf->SetFont('Arial','',7);
    $pdf->SetAligns(array('C','C','L','C','C','C','R','R','R','R','R','R','R','R','R'));
